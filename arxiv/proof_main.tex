...
To study the inverse $(   \Lambda U^\top (Z^{(1)})^\top Z^{(1)} U\Lambda  + V^\top (Z^{(2)})^\top Z^{(2)}V)^{-1}$, we shall use the standard Stieltjes transform method (or the resolvent method) in random matrix theory \cite{bai2009spectral,tao2012topics,erdos2017dynamical}. We now give some preliminaries about 
this method before describing the proof strategy.  

\cor
This has been more formal in work on deterministic equivalents, see e.g., \cite{Hachem2007deterministic,DS18}. 
\nc
