...
In this subsection, we give the proof Theorem \ref{thm_main_informal}. During the proof we shall also give explicit expressions for $\Delta_{\bias}$ and $\Delta_{\vari}$, which might be of interest to some readers. %We shall consider the case where the entries of $\e_1$ and $\e_2$ have the same variance $\sigma_1^2=\sigma_2^2=\sigma^2$. 
...
\end{align}
for $W_1, W_2, \dots, W_t \in \R^r$. Then it is not hard to derive a closed form solution for $B$ as
	\begin{align*}
	...
	\end{align*}
	where $\cal W \in\real^{r\times t}$ is a matrix defined as $\cal W=[W_1, W_2, \dots, W_t]$.
...
We denote $Q := \cal W^{\top} (\cal W\cal W^{\top})^{-1} \cal W \in\real^{t\times t}$, whose $(i,j)$-th entry is given by $W_i^{\top} (\cal W\cal W^{\top})^{-1} W_j$.
	...
	\end{align*}
	From this equation we can solve the minimizer explicitly as $Q_0=U_{r}U_r^{\top}$. On the other hand, let $\hat {\cal W}$ be the minimizer of $g$, and denote $\hat Q:= \hat{\cal W}^{\top} (\hat{\cal W}\hat{\cal W}^{\top})^{-1} \hat{\cal W} $. We claim that $\hat Q$ satisfies
