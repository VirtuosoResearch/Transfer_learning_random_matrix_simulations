...
\begin{theorem}[Multiple tasks with the same covariate]\label{thm_many_tasks}
...
%	Otherwise, we can add a small amount of random perturbation to $A$ (e.g. an inverse exponential in $p$ amount of isotropic Gaussian noise), and the result still holds.
	...
	\end{align}
	Above, we have used that $X^{\top}X$ is invertible by Fact \ref{lem_minv}, and $Y$ was defined as $Y=[Y^{(1)},Y^{(2)},\cdots, Y^{(t)}]$. {\cor(Is $\dag$ a standard notation? It is a bad notation at least for me because $\dag$ is more often used as Hermitian conjugate. Wiki page uses $(AA^{\top})^{+}$ for pseudo-inverse.)}
...
%	The first part measures the bias of hard parameter sharing and the second part measures the variance of hard parameter sharing and scales with $\sigma^2$.
	...
	Recall that $\hat{A}, \hat{B}$ are a global minimum of $f(A, B)$.
