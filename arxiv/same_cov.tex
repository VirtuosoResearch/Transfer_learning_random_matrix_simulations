\section{The Same Covariates Setting}\label{sec_same}
...
\begin{example}[Sample efficiency]
We illustrate the above result in the random-effects model described in Section \ref{sec_hps}.
We further assume that $\Sigma$ is isotropic for simplicity.
Therefore, ${B^{\star}}^{\top}\Sigma B^{\star} = {B^{\star}}^{\top} B^{\star}$ and the $(i, j)$-th entry of this matrix is roughly equal to
\begin{align*}
...
One can verify that the top singular value of ${B^{\star}}^{\top} B^{\star}$ is roughly $p \kappa^2 + d^2$ and the rest of its singular values are all equal to $d^2$.
Therefore, by taking a rank-$r$ approximation of ${B^{\star}}^{\top} B^{\star}$, we get that the average prediction loss of $B^{\star} a_i^{\star}$ is equal to
\[ \frac{1}{t} \sum_{i=1}^t L(B^{\star} a_i^{\star}) = \normFro{B^{\star} A^{\star} {A^{\star}}^{\top} - B^{\star}}^2 = (t - r) d^2. \]
On the other hand, we observe that $\sum_{i=1}^t \norm{a_i^{\star}}^2 = r$.
Therefore, equation \eqref{eq_thm_many} reduces to
\[ \frac{1}{t}\sum_{i=1}^t L(\hat{\beta}_i^{\MTL}) = \bigbrace{1 - \frac{r}{t}} d^2 + \frac{r}{t} \cdot \frac{\sigma^2 p}{n - p} \pm \OO(p^{-c_{\varphi}}). \]
	We compare the prediction loss of hard parameter sharing to single-task learning.
	...
	In this case, hard parameter sharing requires only $p (1 + \sigma^2 / d^2) < n$ samples to achieve comparable loss to single-task learning.

%\begin{corollary}[Derived from Theorem \ref{thm_many_tasks}]\label{prop_same_cov}
...
%	 \sum_{i=1}^t L(\hat{\beta}_i^{\MTL})&= (t-r) \kappa^2 \cdot \frac{\tr\Sigma}{p}  + \sigma^2 \cdot \frac{r}{\rho-1} +\OO\left( p^{-c_\infty} \cdot\frac{\rho t(\kappa^2 + d^2) \|\Sigma\|}{(\sqrt{\rho}-1)^2 \kappa^2  \|\Sigma\| + \sigma^2}   \left( t \kappa^2 \|\Sigma \| + \frac{\sigma^2 r}{\rho-1} \right)  \right) .
	 %& = \sum_{i=r+1}^t \lambda_i^2 (\Sigma^{1/2} B^{\star}) + \sigma^2 \frac{r}{\rho-1} +\OO\left(  p^{-c_\infty} \frac{t(\kappa^2 + d^2)\cdot (p^{-1}\tr \Sigma)}{(\sqrt{\rho}-1)^2 \kappa^2 \cdot (p^{-1}\tr \Sigma) + \sigma^2}   \left( t \kappa^2 \frac{\tr \Sigma}{p} + \frac{\sigma^2 r}{\rho-1} \right) \right) \\
%	 &= (t-r) \kappa^2 \cdot \frac{\tr\Sigma}{p}  + \sigma^2 \cdot \frac{r}{\rho-1} \\
%	 &+\OO\left( p^{-c_\infty}t(\kappa^2+d^2) \cdot p^{-1/2} \|\Sigma\|_F + p^{-c_\infty} \frac{t(\kappa^2 + d^2)\cdot (p^{-1}\tr \Sigma)}{(\sqrt{\rho}-1)^2 \kappa^2 \cdot (p^{-1}\tr \Sigma) + \sigma^2}   \left( t \kappa^2 \frac{\tr \Sigma}{p} + \frac{\sigma^2 r}{\rho-1} \right)  \right) .
%	\end{align*}
%and $\lambda_{r+1} (\Sigma^{1/2} B^{\star}) \ge \cdots\ge  \lambda_{t} (\Sigma^{1/2} B^{\star})$ denote the smallest $(t-r)$ singular values of $\Sigma^{1/2} B^{\star}$.
%\end{corollary}
...
\end{example}
	...
	\begin{claim}\label{lem_exp_opt}
		...
		\begin{align}
			\exarg{\set{\varepsilon^{(j)}}_{j=1}^t, X}{g(A)} = n \bignormFro{\Sigma^{1/2} B^{\star} \bigbrace{A^{\top} (AA^{\top})^{-1} A - \id_{t\times t}}}^2 + \sigma^2 (n\cdot t - p \cdot r). \label{eq_gA}
		\end{align}
