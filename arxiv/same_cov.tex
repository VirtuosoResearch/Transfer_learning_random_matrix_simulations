...
\end{theorem}
	...
	\begin{align}
		\hat{B}(A) \define (X^{\top}X)^{-1} X^{\top} \bigbrace{\sum_{j=1}^t Y^{(j)} A_j^{\top}} (A  A^{\top})^{-1}. \label{eq_Bhat} %\\
	...
	Our next claim shows that the subspace spanned by the rows of $\hat{A}$ is close to that of $U_r$.
	\begin{claim}\label{claim_opt_dist}
		...
		In the setting of Theorem \ref{thm_many_tasks}, we have that
		\[ \bignormFro{U_{\hat{A}} U_{\hat{A}}^{\top} - U_r U_r^{\top}} \lesssim p^{-c} \cdot \frac{\lambda_{\max}({B^{\star}}^{\top}\Sigma B^{\star})}{\lambda_{\min}({B^{\star}}^{\top} \Sigma B^{\star}) + \sigma^2}. \]
	\end{claim}
