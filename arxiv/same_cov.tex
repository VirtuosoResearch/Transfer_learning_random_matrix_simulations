...
\end{theorem}
	...
	\begin{align*}
		\hat{B} \define (X^{\top}X)^{-1} X^{\top} \bigbrace{\sum_{j=1}^t Y^{(j)} A_j^{\top}} (A  A^{\top})^{-1}. %\\
	...
	\begin{lemma}
		...
		\end{align}
		As a result, the minimum solution of $\ex{g(A)}$, denoted by $A^{\star}$, satisfies that $A^{\star}(A^{\star}{A^{\star}}^{\top})^{-1} {A^{\star}}^{\top} = U_r U_r^{\top}$.
	\end{lemma}

	One can see that equation \eqref{eq_gA} provides a bias-variance decomposition of the prediction loss.
	...
	The proof of equation \eqref{eq_gA} can be found in Appendix \ref{app_proof_error_same_cov}.
