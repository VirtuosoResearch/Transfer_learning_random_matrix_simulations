...
%	Consider $t$ data models $Y_i = X\beta_i + \varepsilon_i$, $i=1,2,\cdots, t$, where $X$ has covariance matrix $\Sigma$, and the entries of $\e_i$ are i.i.d. with mean zero and variance $\sigma^2$.
	...
	\begin{align}
		\bigabs{L(\hat{\beta}_i^{\MTL}) - \norm{\Sigma^{1/2} (B^{\star} U_r v_i - \beta_i)}^2 - \frac{\sigma^2 \cdot \norm{v_i}^2 }{\rho - 1}} \lesssim p^{-\frac{c-4}{2c}}\bigbrace{\bigtr{{B^{\star}}^{\top}\Sigma B^{\star}} + \sigma^2}.
	\end{align}
	%\begin{itemize}
...
\end{theorem}
	...
	Our result implies that the variance of hard parameter sharing always reduces compared to single-task learning (cf. Lemma \ref{lem_stl}).
