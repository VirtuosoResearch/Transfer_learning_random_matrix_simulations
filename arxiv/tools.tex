...
This concludes \eqref{show_whZ}.
 ...
 Moreover, we can obtain the following concentration estimates for random noise in the setting of Section \ref{sec_same}.
 
 \begin{corollary}\label{cor_calE}
Suppose the entries of $\cal E=[\epsilon^{(1)},\epsilon^{(2)},\cdots, \epsilon^{(t)}]\in \R^{n\times t}$ are i.i.d. random variables of mean zero, variance $\sigma^2$ and bounded moments up to any order. Then we have that 
