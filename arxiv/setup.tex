...
\begin{assumption}\label{assume_rm}
	...
	\begin{enumerate}
		\item[i)] Bounded moments: for every entry of $Z$, we assume that its $c$-th moment exists, that is, there exist a fixed constant $C > 0$ such that
			\begin{equation}\label{assmAhigh}
				\ex{\abs{Z_{i,j}}^{c}} \le C, \text{ for any } 1\le i \le n, 1\le j \le p.
			\end{equation}
		\item[ii)] Bounded singular values: the singular values of $\Sigma$ are bounded within a fixed range, that is, there exists two fixed values $C > C' > 0$ such that all of $\Sigma$'s singular values are bounded within $C'$ and $C$.
	\end{enumerate}
\end{assumption}

\begin{lemma}[cf. Theorem 2.4 in \citet{isotropic}]\label{lem_minv}
	...
	\[ \bigtr{\Sigma (X^{\top} X)^{-1}} = \frac{\sigma^2 \cdot p}{n - p} + \OO(p^{-\e}). \]
\end{lemma}
...
Finally, for the random noise of every task, we assume that all $l$, the $l$-th moment of the noise entries exists:
