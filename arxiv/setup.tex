...
Given a solution of $f(A, B)$, denoted by $(\hat{A}, \hat{B})$  (which we will specify below), let $\hat{\beta}_i^{\MTL} = \hat{B} \hat{A}_i$ denote the HPS estimator for task $i$.
...
While we no longer have a characterization of $f(A, B)$'s global minimum because of non-convexity, for any local minimizer of $f(A, B)$, we provide a sharp bias-variance tradeoff of both tasks' prediction loss.
...
We believe that our analysis has provided important initial insight and it is an interesting question to tighten our result.
