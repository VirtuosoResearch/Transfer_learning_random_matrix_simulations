...
\begin{figure}[!t]
	...
	\begin{subfigure}[b]{0.5\textwidth}
		...
		\caption{Different patterns of varying sample sizes}
	\end{subfigure}
	\caption{We observe that task two's prediction loss decreases initially before increasing again. Our result shows (provably) that this phenomenon occurs due to different bias-variance tradeoffs as the sample ratio increases. \HZ{rewrite}}
	\label{fig_intro_sample_size}
\end{figure}

As a consequence of our concrete and precise loss estimates, we make qualititative predictions that are related to commonly observed empirical phenomena in hard parameter sharing.
\begin{enumerate}
	\item \textit{Sample efficiency depends on task-specific variance vs. noise variance}:
	\item \textit{Increasing one task's sample size does not always reduce another task's loss}:
	\item \textit{Covariate shift worsens when sample size ratio increases}:
\end{enumerate}
...
\paragraph{Our techniques.} \todo{rewrite} We describe the main ideas for showing the generalization error of hard parameter sharing estimators.
...
\paragraph{Notations.}
Let $B^\star \define [{\beta}_1,{\beta}_2,\dots,{\beta}_{t}] \in \real^{p\times t}$ denote the ground truth model parameters.
Let $\cE \define [\varepsilon_1, \varepsilon_2, \dots, \varepsilon_t] \in \real^{n \times t}$ denote the random noise.
Let $Y$ denote $[Y^{(1)}, Y^{(2)}, \dots, Y^{(t)}]$.
