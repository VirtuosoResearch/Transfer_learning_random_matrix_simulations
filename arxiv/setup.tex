...
The proof of our second result is significantly more involved because of different sample sizes and covariate shifts. Using some novel random matrix theory techniques developed recently \cite{Anisotropic}, we show that inverse of the sum of two sample covariance matrices with arbitrary population covariance matrices converge to a deterministic diagonal matrix asymptotically. Moreover, we can obtain a sharp bound on the concentration error. 
...
To the best of our knowledge, we are not aware of any previous work that provide explicit generalization bounds under different sample sizes and covariate shifts.
...
This result is later extended to non-identity covariance \cite{Anisotropic} and separable covariance matrices \cite{yang2019spiked}.
...
To the best of my knowledge, we are not aware of any previous result for the sum of two sample covariance matrices with arbitrary covariance matrices.
