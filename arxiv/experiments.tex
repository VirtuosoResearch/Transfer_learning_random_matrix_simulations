...
\begin{figure}[!t]
	...
	\caption{Validating Example \ref{ex_same_cov} in the same covariates case: our estimated loss (solid line) matches the empirical loss (dotted line) accurately for various task-specific variance $d^2$ and sample size $n$ settings. The feature dimension $p$ is $200$, and noise variance $\sigma^2$ is $1/4$. \FY{What is $t$ in this setting?}}
	\label{fig_same_cov}
\end{figure}
...
Finally, looking horizontally at the same prediction loss level, we find that hard parameter sharing requires 3x fewer samples per-task than STL. \FY{I do not quite understand this sentence about "3x fewer", because how much data needed depends on $d$ and the prediction loss level we are looking at. For example, at the cross point, this ratio is 1. }
