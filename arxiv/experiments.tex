...
We verify our theoretical results in synthetic settings.
We consider the feature covariates to have dimension $p = 200$.

\paragraph{The same covariates case.}
We first study the setting of Example \ref{ex_same_cov}. We vary the per-task sample size $n$ from $2p$ to $10p$. We set $\sigma = 0.5$. Figure \ref{fig_same_cov} shows the result.
First, we can see that for various different values of $d$ and $n$, our estimated loss matches the empirical loss accurately.
...
On the other hand when $d = 0.2$, this is not always the case, and indeed we observe a crossover point between the average prediction loss of hard parameter sharing and STL.
Finally, we see that to achieve comparable loss to STL, hard parameter sharing requires 3x less samples per-task.

\begin{figure}[!t]
	\centering
	\includegraphics[width=0.45\textwidth]{figures/same_covariates.eps}
	\caption{Validating our result for the same covariates setting. Our estimated loss (solid line) matches the empirical loss (dotted line) accurately, for various values of $d$ and sample sizes.}
	\label{fig_same_cov}
\end{figure}


	\paragraph{The different covarates case.}
...
Sample size: We set $\kappa = 1$ and $d = 0.02$ for model distance.
...
We consider a text classification task as follows.
...
\textit{Towards computing the optimal sample ratio.}
...
Covariate shift: We implement the covariance alignment procedure following \cite{WZR20}.
