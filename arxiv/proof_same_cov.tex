\section{Missing Proof of Theorem \ref{thm_many_tasks}}\label{app_proof_error_same_cov}
	...
	\begin{claim}\label{claim_opt_dist}
		...
		\[ \bignormFro{U_{\hat{A}} U_{\hat{A}}^{\top} - A^{\star} {A^{\star}}^{\top}}^2
				\le  n^{-c_{\varphi}} \cdot \frac{t(  \bignorm{\Sigma^{1/2} B^{\star}}^2 +  \sigma^2 )}{\lambda_r- \lambda_{r+1}}. \]
	\end{claim}
	...
	\begin{claim}\label{claim_pred_err}
		...
		\begin{align*}
			...
			\le  n^{-1/4} \left(\|\Sigma^{1/2}B^{\star} \|^2+ \sigma^2  \cdot\|\hat a_i\|^2\right) .
		\end{align*}
	\end{claim}
...
%	\end{align}
  ...
  % &\le |W_j^{\top} (\cW\cW^{\top})^{-1}W_j|^2 \cdot \sigma^2 p^{\e} \tr \left[(U_X U_X^{\top})^4\right]\le \sigma^2 p^{1/2+\e},
	...
	\begin{align}
		...
		&\lesssim n^{-c_\varphi}\cdot n \bignorm{\Sigma^{1/2} B^{\star}}^2 + n^{-c_\infty}\cdot \sigma^2 \cdot n \cdot t. \label{eq_g_gap}
	\end{align}
	...
	$$\| A^\star {A^\star}^\top -U_{\hat A}U^\top_{\hat A}\|_F^2  =2 \sum_{i=r+1}^t \| U^\top_{\hat A} v_i\|^2 \lesssim \frac{n^{-c_\varphi}\cdot  \bignorm{\Sigma^{1/2} B^{\star}}^2 + n^{-c_\infty}\cdot \sigma^2 t}{\lambda_r- \lambda_{r+1}}  .$$
	...
	\paragraph{Part 2:} The second part is the cross term, whose expectation over $\cE$ is zero. Using Corollary \ref{cor_calE}, the concentration error can be bounded as 
	\begin{align*}
		& \bigabs{\inner{\Sigma^{1/2}(B^{\star} \hat{a}_i - \beta^{(i)})}{\Sigma^{1/2}R_i}} \\
		\le &\sum_{1\le j, k\le t}|\hat a_i(j)| |\hat a_i(k)| \bigabs{ \inner{ X(X^\top X)^{-1}\Sigma \beta^{(j)} }{\epsilon^{(k)}}}  + \sum_{  j =1}^{t}|\hat a_i(j)|\bigabs{ \inner{ X(X^\top X)^{-1}\Sigma \beta^{(i)}}{\epsilon^{(j)}}} \\
		\le & n^{c}\sigma \cdot \bignormFro{ X(X^\top X)^{-1}\Sigma B^{\star} } \cdot \left[ \sum_{j=1}^t |\hat a_i(j)| + \sum_{1\le j,k \le t} |\hat a_i(j)| |\hat a_i(k)| \right] \\
		 \lesssim & n^{c}\sigma   \cdot \|\hat a_i\|\cdot \|\Sigma^{1/2}B^{\star} \| \cdot \| (Z^\top Z)^{-1} \|^{1/2}  \le  n^{-1/2+c}\sigma^2 \|\hat a_i\|^2 + n^{-1/2+c} \|\Sigma^{1/2}B^{\star} \|^2 , 
	\end{align*}	
%	\begin{align*}
...
%	\end{align*}
	with high probability for any small constant $c>0$. Here in the first step, we expand $B^\star \hat a_i=\sum_{1\le j \le t} \hat a_i(j) \beta^{(j)}$ amd $\cal E \hat a_i=\sum_{1\le j \le t} \hat a_i(j) \epsilon^{(j)}$. In the second step, we used equation \eqref{vcalE}. In the third step we used $\sum_j |\hat a_i(j)|\le \sqrt{t}\|\hat a_i\|$ by Cauchy-Schwarz inequality, and 
	\begin{align*}
	\bignormFro{ X(X^\top X)^{-1}\Sigma B^{\star}} &=\left[ {B^{\star}}^\top \Sigma (X^\top X)^{-1}X^\top X(X^\top X)^{-1}\Sigma B^{\star} \right]^{1/2} \\
	&\le  \|\Sigma^{1/2} B^{\star} \|  \cdot \left\| \Sigma^{1/2}(X^\top X)^{-1}\Sigma ^{1/2} \right\|^{1/2}  =   \|\Sigma^{1/2} B^{\star} \|  \cdot \left\| (Z^\top Z)^{-1}\right\|^{1/2},
	\end{align*}
%	\begin{align*}
...
%	\end{align*}
	because $X=Z\Sigma^{1/2}$. In the fourth step, we used Fact \ref{fact_minv} (ii) to bound the operator norm $\left\| (Z^\top Z)^{-1}\right\|$ by $\OO(n^{-1})$, and applied the AM-GM inequality. 
	...
	\paragraph{Part 3:} The final part is $\|R_i\|^2$. We rewrite it as 
	\begin{align}
...
%	with high probability for any small constant $c>0$. On the other hand, if $j=k$, 
	...
	Then using equation \eqref{vcalA2}, we obtain that 
	\begin{align}
	...
	Above, we used Fact \ref{fact_minv} (ii) in the last step to bound $ \bignorm{(Z^{\top} Z)^{-1}}$ by $\OO(n^{-1})$.
	Plugging equation \eqref{jeqk} into equation \eqref{E_i^2}, we obtain that 
	\begin{align*}
		\bigabs{\bignorm{\Sigma^{1/2} R_i}^2 -  \sigma^2 \|\hat a_i\|^2 \cdot\tr\left[\Sigma(X^\top X)^{-1}\right]} \lesssim  \sigma^2 \cdot n^{-1/2+c}  \sum_{1\le j,k\le t}|\hat a_i(j)||\hat a_i(k)|\lesssim  n^{-1/2+c} \sigma^2 \cdot  \|\hat a_i\|^2.
	\end{align*}
