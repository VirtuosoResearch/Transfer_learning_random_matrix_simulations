	\section{Missing Proof of the Same Covariates Case}\label{app_proof_error_same_cov}

	\paragraph{Proof of Claim \ref{lem_exp_opt}.}
	To facilitate the analysis, we consider the following matrix notations.
	Denote
		\[ \cE A^{\top} := {\sum_{k=1}^t \varepsilon^{(k)} A_k^{\top}}, \quad \text{ and } \quad \cW \define X(X^{\top} X)^{-1} X^{\top} \cE A^{\top} (AA^{\top})^{\dag}. \]
	For any $j = 1,2,\dots, t$, let
	\begin{align*}
		H_j &\define B^{\star} A^{\top} (AA^{\top})^{\dagger} A_j - \beta^{(j)}, \quad \text{ and } \quad E_j \define \cW A_j - \varepsilon^{(j)}.
	\end{align*}
	...
	\end{align}
	where $U_{A} U_{A}^{\top} \in\real^{t\times t}$ denotes the subspace $ {A}^{\top} ( {A}{A}^{\top})^{\dagger}  {A}$. Note that the expectation of equation \eqref{eq_gA_p1} over $X$ is $\bignorm{\Sigma^{1/2}(B^{\star} U_A U_A^{\top} - B^{\star})}^2$.
...
%	We consider both terms in the above equation one by one.
	...
	The second equation uses the fact that $\sum_{j=1}^t A_j^{\top} (AA^{\top})^{\dagger} A_j = \bigtr{\id_{r\times r}} = r$, and $\ex{\varepsilon^{(j)} {\varepsilon^{(j)}}^{\top}} = \sigma^2 \cdot \id_{n\times n}$.
...
%	$$g_0(\cal W)= \sum_{j=1}^t\left\|Z v_j \right\|^2,\quad v_j:= \Sigma^{1/2}\left(B^{\star} \cW^{\top} (\cW\cW^{\top})^{-1} W_j -  \beta_j\right).$$
  ...
  % &\le |W_j^{\top} (\cW\cW^{\top})^{-1}W_j|^2 \cdot \sigma^2 p^{\e} \tr \left[(U_X U_X^{\top})^4\right]\le \sigma^2 p^{1/2+\e},
	...
	Next, using triangle inequality, we have that
	\begin{align}
