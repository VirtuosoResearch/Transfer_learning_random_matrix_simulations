...
\begin{theorem}[Two-task case]\label{thm_main_informal}
	...
	\begin{enumerate}
		\item[a)] \textbf{Positive transfer:} If $\Delta_{\bias} < \Delta_{\vari} -\delta$, then w.h.p. over the randomness of $X_1, X_2$, we have
			\[ \te(\hat{\beta}_2^{\MTL}) < \te(\hat{\beta}_2^{\STL}).  \]
		\item[b)] \textbf{Negative transfer:} If $\Delta_{\bias} > \Delta_{\vari} + \delta$, then w.h.p. over the randomness of $X_1, X_2$, we have
			\[ \te(\hat{\beta}_2^{\MTL}) > \te(\hat{\beta}_2^{\STL}). \]
	\end{enumerate}
\end{theorem}

We make several remarks about Theorem \ref{thm_main_informal}.
First, the result shows that provided with a large amount of source task data,
...
\begin{lemma}[Variance asymptotics]\label{lem_cov_shift}
	...
	In the setting of Theorem \ref{thm_main_informal}, let $\Sigma$ be any $p\times p$ fixed and deterministic matrix.
	We have that %for any constant $\e>0$,
	...
	\begin{align}\label{lem_cov_shift_eq}
		\bigtr{(X_1^{\top}X_1 + X_2^{\top}X_2)^{-1} \Sigma} = \frac{1}{n_1 + n_2}\cdot \bigtr{ (a_1 \Sigma_1 + a_2\Sigma_2)^{-1} A} + o(1)%\bigo{\|\| p^{-1/2+\epsilon}}
	\end{align}
...
\begin{corollary}[e.g. Theorem 2.4 in \citet{isotropic}]\label{lem_minv}
	...
	\be\label{XXA}  \bigtr{(X_2^{\top}X_2)^{-1}A} = \frac{1}{n_2 - p} \cdot \bigtr{\Sigma_2^{-1}A} + o(1). \ee
