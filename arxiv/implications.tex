\section{Illustrative Examples}\label{sec_special}
...
\begin{corollary}[Derived from Theorem \ref{thm main RMT}]\label{cor MTL loss}
...
It is well-known since the seminal work of Caruana \cite{C97} that how well multi-task learning performs depends on task relatedness. We formalize this connection in the above isotropic setting, where we can perform explicit calculations. The prediction loss of MTL has been given in Corollary \ref{cor MTL loss}. We can also calculate the prediction STL easily using Fact \ref{lem_minv} (ii). The proof of the following claim will be given in Section \ref{app_iso_cov}.
\begin{claim}
...
\noindent{\bf Varying model distance.} We show that as we increase the distance between $\beta_1$ and $\beta_2$, there is a transition from positive transfer to negative transfer in MTL.
