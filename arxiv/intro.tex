...
Hard parameter sharing works as an inductive transfer mechnism and a regularizer that reduces overfitting, both of which are intuitively appealing \cite{R17}.
...
Previous results based on Rademacher complexity or VC dimensions have considered cases when all tasks' sample sizes are equal to logarithmic factors of the feature dimension \cite{B00,MPR16}, and when all tasks' sample sizes increase simultaneously \cite{AZ05,M06}.
...
\begin{figure*}[!t]
	...
	Increasing $n_1/n_2$ decreases task two's prediction loss initially, but increases afterwards. This phenomenon occurs due to different bias-variance tradeoffs as $n_1/n_2$ increases. Our result provides an estimated loss (solid line) that accurately matches the empirical loss (dotted line).
