...
In random matrix theory, it is much more convenient to rescale the matrices $Z_1$ and $Z_2$ such that their entries have variance $n^{-1}$, where $n:=n_1+n_2$. \HZ{I don't think is the point; I think you want to say that the sample covariance of both tasks $(Z_1^{\top}Z_1 + Z_2^{\top}Z_2)/n$ is what matters} The advantage of this scaling is that the singular eigenvalues of $n^{-1/2}Z_1$ and $n^{-1/2}Z_2$ all lie in a bounded support that does not grow with $n$. For reader's convenience, we now restate the problem setting and introduce a couple of other notations.
...
We assume that $Z_1$ and $Z_2$ are $n_1\times p$ and $n_2\times p$ random matrices with i.i.d. entries satisfying
...
\mathbb{E} \vert (Z_1)_{11} \vert^4 +\mathbb{E} \vert (Z_2)_{11} \vert^4  \le C, \quad k=1,2,
\ee
for some constant $C>0$. 
%In this paper, we regard $N$ as the fundamental (large) parameter and $n \equiv n(N)$ as depending on $N$. 
We assume that $\rho_1= n_1/p$ and $\rho_2=n_2/p$ satisfy that
...
\be\label{eigen}\Sig_1= O_1\Lambda_1 O_1^\top, \ \ \Sig_2= O_2\Lambda_2 O_2^\top,\ \ \Lambda_1=\text{diag}(\lambda_1^{(1)}, \ldots, \lambda^{(1)}_p), \ \ \Lambda_2=\text{diag}( \lambda^{(2)}_1, \ldots, \lambda^{(2)}_p),
\ee
...
We assume that $Z_1$ and $Z_2$ are independent $n_1\times p$ and $n_2\times p$ random matrices with real $i.i.d.$ entries satisfying \eqref{assm1} and \eqref{conditionA2}, $\Sigma_1$ and $\Sigma_2$ are deterministic non-negative definite symmetric matrices satisfying \eqref{eigen}-\eqref{assm32}, and $\rho_{1}$ and $\rho_2$ satisfy \eqref{assm2}.
...
\end{assumption}
\HZ{This is restating what's stated above again. Remove redundant statements (unless there's something critical missing in Sec 2).}
...
We say a random matrix $M$ satisfies the {\it{bounded support condition}} with $q$, if
\begin{equation}
\max_{i,j}\vert M_{ij}\vert \prec q. \label{eq_support}
\end{equation}
...
\end{definition}
\begin{example}
\todo{Talk about the Gaussian case and Bernoulli case.} Note that if the entries of $Z$ have finite moments up to any order, %as in \eqref{assmAhigh}, 
...
For $n^{-1/2}Z_1$ and $n^{-1/2}Z_2$ with bounded support $q$, we have the following estimates on their singular values. We have used it in our previous proofs; see \eqref{eq_isometric}.   
...
Suppose Assumption \ref{assm_big1} holds, and $n^{-1/2}Z_1$ and $n^{-1/2}Z_2$ satisfy the bounded support condition \eqref{eq_support} for some deterministic parameter $q$ satisfying $ n^{-{1}/{2}} \leq q \leq n^{- \phi} $ for a constant $\phi>0$. Then for any constant $\e>0$, we have that with high probability,
...
%, but only the bounds for largest eigenvalues are given there in order to avoid the issue with the smallest eigenvalue when $d_{2}$ is close to 1. However, under the assumption \eqref{assm2}, the lower bound for the smallest eigenvalue follows from the same arguments as in \cite{DY}. Hence we omit the details. 
\end{proof}
\begin{remark}
...
%\end{array}} \right)  ,
\end{align*}
and define the partial traces $m^{(\mathfrak c)}$ and $m^{(\mathfrak c)}_k$, $k=0,1,2,$ by replacing $G$ with $G^{(\mathfrak c)}$ in \eqref{defm}. For convenience, we will adopt the convention that for the resolvent minor $G^{(\mathfrak c)}$ defined as above, $G^{(\mathfrak c)}_{\fa\fb} = 0$ if $\fa =\mathfrak c$ or $\fb =\mathfrak c$.
\end{definition}
...
Notice that using \eqref{mainG}, we can write  \eqref{eigen2extra} as
...
We now describe the asymptotic limit of $G(z)$ and $\cal G(z)$ as $n\to \infty$. First we define the deterministic limits of $(m_1(z), m_{2}(z))$, denoted by $(m_{1 c}(z),m_{2c}(z))$, as the unique solution to the following system of equations
\begin{equation}\label{selfomega}
\begin{split}
& \frac1{m_{1c}} = \frac{\gamma_n}p\sum_{i=1}^p \frac{\lambda_i^2}{  z+\lambda_i^2 r_1 m_{1c} +r_2 m_{2c}  } - 1 ,\  \ \frac1{m_{2c}} = \frac{\gamma_n}p\sum_{i=1}^p \frac{1 }{  z+\lambda_i^2 r_1 m_{1c} +  r_2 m_{2c}  }- 1 ,
\end{split}
\ee
such that $(m_{1 c}(z), m_{2c}(z))\in \C_+^2$ for $z\in \C_+$, where, for simplicity, we  introduced the following ratios 
\be\label{ratios}
...
\be \label{defn_piw}
\Pi(z) := \begin{pmatrix} -(z+r_1 m_{1c}(z)\Lambda^2  +  r_2 m_{2c}(z))^{-1} & 0 & 0 \\ 0 &  m_{1c}(z)\id_{n_1} & 0 \\ 0 & 0 & m_{2c}(z)\id_{n_2}  \end{pmatrix}.\ee
In particular, the matrix limit of $\cal G(z)$ is given by $-(z+r_1 m_{1c}\Lambda^2 + r_2 m_{2c})^{-1}$. 

\vspace{5pt}
\noindent{\bf Proof overview:} Now we give a heuristic derivation of the matrix limit when the entries of $Z_1$ and $Z_2$ are i.i.d. Gaussian. Note that in this case, 
...
where ``$\stackrel{d}{=}$" means ``equal in distribution". Hence it suffices to consider the following resolvent  
 \begin{equation} \nonumber
   ...
   \end{array}} \right)^{-1}.
 \end{equation}
Using Schur complement formula (see (\ref{resolvent2})), we have that %for $i \in \cal I_0$, $\mu \in \cal I_1$ and $\nu\in \cal I_2$,
\begin{align}
...
\end{align}
where we recall the notations in \eqref{labelZ}. For the right-hand side of \eqref{0self_Gii}, $G^{(i)}$ is independent of the entries $z_{\mu i}$ and $z_{\nu i}$. Hence by the concentration inequalities in Lemma \ref{largedeviation}, we have that the  right-hand side of \eqref{0self_Gii} concentrates around the partial expectation over the entries $\{z_{\mu i}: \mu \in \cal I_1\cup \cal I_2\}$, i.e., with high probability,
\begin{align*}
...
\end{align*}
where we used the definition of $m_1^{(i)}$ and $m_2^{(i)}$ in \eqref{defm} with $G$ replaced by $G^{(i)}$. Intuitively, since we have only removed only one column and one row out of the $(p+n)$ columns and rows in $H$, $m_1^{(i)}$ and $m_2^{(i)}$ should be close to the original $m_1$ and $m_2$. Hence we obtain from the above equation that
\begin{align}\label{1self_Gii}
 G_{ii}  = -\frac{1}{ z +\lambda_i^2 r_1  m_1(z) + r_2 m_2(z)+\oo(1)}.
\end{align}
Similarly, we can obtain from \eqref{0self_Gmu1} that for $\mu \in \cal I_1$ and $\nu\in \cal I_2$,
...
\ee
with high probability. Together with the definition of $m$ and $m_0$ in \eqref{defm}, the two equations in \eqref{2self_Gmu} give that 
...
with high probability. Plugging \eqref{1self_Gii} into \eqref{3self_Gmu}, we obtain that
\begin{align*}
...
\end{align*}
with high probability, which give the approximate self-consistent equations for $(m_1,m_2)$. Compare them to the deterministic self-consistent equations in \eqref{selfomega}, one can observe that we should have $(m_1,m_2) =(m_{1c}, m_{2c})+\oo(1)$ with high probability. Inserting this approximate identity into \eqref{1self_Gii}-\eqref{2self_Gmu}, we see that for  $i \in \cal I_0$, $\mu \in \cal I_1$ and $\nu\in \cal I_2$,
$$G_{ii}=-(z +\lambda_i^2 r_1  m_{1c} + r_2 m_{2c}+\oo(1))^{1/2},\quad G_{\mu\mu}=m_{1c}+\oo(1),\quad G_{\nu\nu}=m_{2c}+\oo(1),$$
with high probability. These explain the diagonal entries of $\Pi$ in \eqref{defn_piw}. For the off-diagonal entries, they are close to zero due to concentration. For example, for $i\ne j\in \cal I_1$, by Schur complement formula (see (\ref{resolvent3})), we have
...
If $z=0$, then the equations in \eqref{selfomega} in are reduced to 
...
\end{split}
\ee
where $a_1:=-r_1m_{1c}(0)$ and $a_2:=-r_2m_{2c}(0)$. Note that the function 
...
\ee 
For general $z$ around the origin, the existence and uniqueness of the solution $(m_{1c}(z),m_{2c}(z))$ is given by the following lemma. %Moreover, we also include some basic estimates on it. %{\cor(say something about the previous work)}
\nc
...
\be\label{prior1}
|z|\le c_0, \quad  |m_{1 c}(z) - m_{1 c}(0)| + |m_{2c}(z) - m_{2c}(0)|\le c_0.
\ee
Moreover, the solution satisfies
\be\label{Lipomega}
 |m_{1 c}(z) - m_{1 c}(0)| + |m_{2 c}(z) - m_{2 c}(0)| \le C_0|z|.
\ee
%and
%\be\label{Immcw}
...
% \vert m_{2c}(z) \vert \sim 1,  \quad \left|z^{-1} - (m_{1c}(z)+m_{2c}(z)) + (z-1)m_{1c}(z)m_{2c}(z) \right|\sim 1,
% \ee 
% and
% \be\label{Immcw} 0\le \im m_{2c}(z) \sim \begin{cases}
...
There exist constants $c_0, C_0>0$ depending only on $\tau$ in \eqref{assm2}, \eqref{assm3}, \eqref{assm32} and \eqref{a23} such that the self-consistent equations in \eqref{selfomega} are stable in the following sense. Suppose $|z|\le c_0$ and $m_{k}: \C_+\mapsto \C_+$, $k=1,2$, are analytic functions of $z$ such that 
\be  \label{prior12}
|m_{1}(z) - m_{1 c}(0)| + |m_{2}(z) - m_{2 c}(0)|\le c_0.
\ee
...
where $\delta(z)$ is a deterministic $z$-dependent function satisfying $\delta(z) \le (\log n)^{-1}.$ Then we have 
 \begin{equation}
  \left|m_1(z)-m_{1 c}(z)\right| +  \left|m_2(z)-m_{2 c}(z)\right|\le C_0\delta(z).\label{Stability1}
\end{equation}
\end{lemma}
...
Suppose Assumption \ref{assm_big1} holds, and $Z_1$ and $Z_2$ satisfy the bounded support condition \eqref{eq_support} for a deterministic parameter $q$ satisfying $ n^{-{1}/{2}} \leq q \leq n^{- \phi} $ for some constant $\phi>0$. Then %there exists a sufficiently small constant $c_0>0$ such that 
...
\ee 
for some constant $a>4$. Then \eqref{aniso_law} holds for $q= n^{2/a-1/2}$ on an event $\Omega$ with $\P(\Omega)=1-\oo(1)$.
\end{corollary}
...
Now $\wh Z_1$ and $\wh Z_2$ satisfy the assumptions of Theorem \ref{LEM_SMALL} with $q= n^{-c_a +\e}$, and \eqref{aniso_law} gives that
$$\left| \mathbf u^\top (G(\wh Z_1,\wh Z_2,z)-\Pi(z)) \mathbf v \right|  \prec  q.$$
Then using \eqref{meanshif}, \eqref{EZ norm} and \eqref{priorim} below, it is easy to check that  
$$\left| \mathbf u^\top (G(\wh Z_1,\wh Z_2,z)-G(\wt Z_1,\wt Z_2,z)) \mathbf v \right|  \prec  n^{-1-\e},$$
 This shows that \eqref{aniso_law} also holds for $G(\wt Z_1,\wt Z_2,z)$ with $q= n^{-c_a +\e}$, which concludes the proof by \eqref{XneX} and the fact that $\e$ can be chosen arbitrarily small.
\end{proof}
...
Recall that in the setting of Lemma \ref{lem_cov_shift} we have \eqref{rewrite X as R}. 
%using \eqref{eigen2extra} and \eqref{mainG} we can write 
...
If the entries of $\sqrt{n}Z_1$ and $\sqrt{n}Z_2$ have finite $(4+\e)$-th moments as in \eqref{assmAhigh}, then by Corollary \ref{main_cor}  we have that
...
for $q= n^{-\e/(8+2\e)}$ on an event $\Omega$ with $\P(\Omega)=1-\oo(1)$. By \eqref{defn_piw}, we have
$$\Pi(0)= -(r_1m_{1c}(0)\Lambda^2  +  r_2m_{2c}(0))^{-1}= (a_1 V^\top M^\top M V +  a_2)^{-1},$$
with $(a_1,a_2)$ satisfying \eqref{selfomega0}. Thus from \eqref{G0Pi0} we get that
...
Note that $\cal G^2(0)=\partial_z \cal G|_{z=0}$. Under the moment condition \eqref{assmAhigh}, using Corollary \ref{main_cor} we obtain that
%$Z_1$ and $Z_2$ have bounded support $q=n^{-1/2}$. Using Theorem \ref{LEM_SMALL}, %Now by Corollary \ref{main_cor}, we obtain that for any small constant $\e>0$, %with probability $1-\oo(1)$,
\be\nonumber 
...
\be\label{dervPi}
\bv^\top \Pi'(0)\bv = \bv \frac{1+r_1m_{1c}'(0)\Lambda^2 + r_2m_{2c}'(0)}{(r_1m_{1c}(0)\Lambda^2 + r_2m_{2c}(0))^2}\bv,
\ee
where we need to calculate the derivatives $ m_{1 c}'(0)$ and $ m_{2 c}'(0)$. 
...
\begin{split}
 \frac{m'_{1c}(0) }{m_{1c}^2(0)}=   \frac1n\sum_{i=1}^p \frac{\lambda_i^2\left(1+\lambda_i^2 r_1m'_{1c}(0) + r_2 m'_{2c}(0)\right) }{  ( \lambda_i^2 r_1 m_{1c}(0) +r_2 m_{2c}(0))^2 } ,\quad \frac{m'_{2c}(0)}{m_{2c}^2(0)} &=  \frac1n\sum_{i=1}^p \frac{1+\lambda_i^2 r_1m'_{1c}(0) + r_2 m'_{2c}(0)}{ ( \lambda_i^2 r_1 m_{1c}(0) +r_2 m_{2c}(0))^2  } .
\end{split}
\ee
Recalling that we have denoted  $(a_1, a_2)=(-r_1m_{1c}(0),-r_2m_{2c}(0))$, we can rewrite these two equations as
%\be%\label{dotm34}
%\begin{split}
...
\end{split}
\ee
where we denoted  $(a_3 ,a_4):=(r_2m_{2c}'(0),r_1m'_{2c}(0))$. It is not hard to see that these equations are equivalent to \eqref{eq_a34extra}. Then by \eqref{apply derivlocal} and \eqref{dervPi}, we get
\begin{align*}
...
In this section we give the proof of Lemmas \ref{lem_mbehaviorw} and \ref{lem_stabw} using the contraction principle. 
 ...
 It is easy to check that the equations in \eqref{selfomega} are equivalent to  
\begin{equation}\label{selfalter}
r_1m_{1c}=-(1-\gamma_n) - r_2m_{2c} - z\left( m_{2c}^{-1}+1\right),\quad g_z(m_{2c}(z))=1, 
\ee
where
$$g_z(m_{2c}):= - m_{2c} +\frac1n\sum_{i=1}^p \frac{m_{2c} }{  z -\lambda_i^2(1-\gamma_n)+ (1 - \lambda_i^2) r_2m_{2c} - \lambda_i^2 z\left(  m_{2c}^{-1}+1\right) }.$$
We first show that (i) there exists a unique solution $m_{2c}(z)$ to the equation $g_z(m_{2c}(z))=1$ under the conditions in \eqref{prior1}, and (ii) the solution satisfies \eqref{Lipomega}. We abbreviate $\e(z):= m_{2c}(z) - m_{2c}(0)$. From \eqref{selfalter}, we obtain that 
\begin{equation} \nonumber
0=\left[g_z(m_{2c}(z)) -  g_0(m_{2c}(0)) -g_z'(m_{2c}(0))\e(z)\right] + g_z'(m_{2c}(0))\e(z),
\ee
which implies
\be\nonumber%\label{selfomega1}
 \e(z) =- \frac{ g_z(m_{2c}(0)) - g_0(m_{2c}(0)) }{g_z'(m_{2c}(0))}- \frac{ g_z(m_{2c}(0)+\e(z)) -  g_z(m_{2c}(0))-g_z'(m_{2c}(0))\e(z)}{g_z'(m_{2c}(0))}.
 \ee
%By \eqref{selfomega}, we obtain the self-consistent equations for $(m_{2c}(0),m_{2c}(0))$ and $(m_{2c}(z),m_{2c}(z))$:
...
\be\label{selfomega2}
 \e^{(k+1)} =- \frac{g_z(m_{2c}(0)) - g_0(m_{2c}(0))}{g_z'(m_{2c}(0))} -\frac{g_z(m_{2c}(0)+ \e^{(k)}) -  g_z(m_{2c}(0))-g_z'(m_{2c}(0))\e^{(k)} }{g_z'(m_{2c}(0))} .
 \ee
...
$$g_z'(m_{2c}(0)) = -1 - \frac1n\sum_{i=1}^p \frac{ \lambda_i^2(1-\gamma_n) - z\left[1- \lambda_i^2 \left(  2m_{2c}^{-1}(0)+1\right)\right]  }{  \left[z -\lambda_i^2(1-\gamma_n)+ (1 - \lambda_i^2) r_2m_{2c}(0) - \lambda_i^2 z\left( m_{2c}^{-1}(0)+1\right)\right]^2 }.$$
%Using \eqref{a23}, 
Then it is not hard to check that there exist constants $\wt c, \wt C>0$ depending only on $\tau$ in \eqref{assm32} and \eqref{a23} such that the following estimates hold: for all $z$, $\e_1$ and $\e_2$ such that $|z|\le \wt c$ and $|\e_1|  \le \wt c$, $|\e_2|  \le \wt c$,
\be\label{dust}
\left|[g_z'(m_{2c}(0))]^{-1}\right|\le \wt C, \quad   \left|\frac{g_z(m_{2c}(0)) - g_0(m_{2c}(0))}{g_z'(m_{2c}(0))}\right|  \le \wt C|z| ,
\ee
and 
\be\label{dust222}
\left|\frac{g_z(m_{2c}(0)+ \e_1) -  g_z(m_{2c}(0)+\e_2)-g_z'(m_{2c}(0))(\e_1-\e_2) }{g_z'(m_{2c}(0))}\right|  \le \wt C|\e_1-\e_2|^2 .
\ee
 By \eqref{dust} and \eqref{dust222}, there exists a sufficiently small constant $\delta>0$ depending only on $\wt C$, such that $h$ is a self-mapping 
...
Now it suffices to prove that $h$ restricted to $B_r $ is a contraction, which then implies that ${\e}:=\lim_{k\to\infty} { \e}^{(k)}$ exists and $m_{2c}(0)+\e(z)$ is a unique solution to the second equation of \eqref{selfalter} subject to the condition $\|{\e}\|_\infty \le r$. 
...
$h$ is indeed a contraction mapping on $ B_r$. This proves both the existence and uniqueness of the solution $m_{2c}(z)=m_{2c}(0)+\e(z)$, if we choose $c_0$ in \eqref{prior1} as $c_0=\min\{c_\delta, r\}$. After obtaining $m_{2c}(z)$, we can then find $m_{2c}(z)$ using the first equation in \eqref{selfalter}. 
...
\be\label{endalter}|{ \e}| \le \sum_{k=0}^\infty |{  \e}^{(k+1)}-{ \e}^{(k)}| \le 2\wt C|z| \ \Rightarrow \ |m_{2c}(z)-m_{2c}(0)|\le 2\wt C|z| .\ee
%This gives the bound \eqref{Lipomega} for $m_{2c}(z)$. 
Using the first equation in \eqref{selfalter}, we immediately obtain the bound  $ r_1|m_{1c}(z)-m_{1c}(0)| \le C|z|$, which gives \eqref{Lipomega} as long as if $r_1\gtrsim 1$. To deal with the small $r_1$ case, we go back to the first equation in \eqref{selfomega} and treat $m_{1c}(z)$ as the solution to the following equation:
$$\wt g_z(m_{1c}(z))=1,\quad \wt g_z(x):=- x + \frac{\gamma_n}p\sum_{i=1}^p \frac{\lambda_i^2 x}{  z+\lambda_i^2 r_1 x +r_2 m_{2c}(z) }. $$
...
Then with a similar argument as the one between \eqref{selfalter} and \eqref{endalter}, we can conclude $|m_{2c}(z)-m_{2c}(0)|=\OO(|z|)$, which concludes \eqref{Lipomega} together with \eqref{endalter}. 
...
Under \eqref{prior12}, we can obtain equation \eqref{selfalter} approximately up to some small error
\be\label{selfalter2}r_1 m_{1}=-(1-\gamma_n) - r_2m_{2} - z\left(  {m_{2}^{-1}}+1\right) + \cal E'_1(z),\quad g_z(m_{2}(z))=1+ \cal E'_2(z),\ee
 with $|\cal E'_1(z)|+ |\cal E'_2(z)|=\OO(\delta(z))$. Then we subtract the equations \eqref{selfalter} from \eqref{selfalter2}, and consider the contraction principle for the function $\e (z):= m_{2}(z) - m_{2 c}(z)$.  The rest of the proof is exactly the same as the one for Lemma \ref{lem_mbehaviorw}, so we omit the details.
\end{proof}
...
%\end{array}} \right)\right]^{-1}
%\end{equation}
%provided that $X$ is Gaussian. In particular, the entries of $\Sig^{1/2} X^{\text{Gauss}}\tilde \Sig^{1/2}$ are independent and satisfies the bounded support condition \eqref{eq_support} with $q=N^{-1/2}$, which make the direct proof of Theorem \ref{LEM_SMALL} possible using the methods in \cite{isotropic}. 
...
If the entries of $Z_1$ and $Z_2$ are i.i.d. Gaussian, then by Gaussian concentration $Z_1$ and $Z_2$ have support $q=n^{-1/2}$. Hence combing \eqref{eq in Gauss} with Proposition \ref{prop_diagonal}, we obtain that Theorem \ref{LEM_SMALL} holds when $Z_1$ and $Z_2$ are Gaussian. 
...
Proposition \ref{prop_diagonal} implies that \eqref{aniso_law} holds for Gaussian $Z_1^{\text{Gauss}}$ and $Z_2^{\text{Gauss}}$ as discussed above. Thus it suffices is to prove that for $Z_1$ and $Z_2$ satisfying the assumptions in Theorem \ref{LEM_SMALL}, we have
...
\eqref{aniso_law} through interpolation matrices $Z^\theta$ between $Z^0$ and $Z^1$. We have seen that \eqref{aniso_law} holds for $Z^0$ by Proposition \ref{prop_diagonal}. %as remarked at the beginning of Section \ref{sec_Gauss}.
...
%\end{lemma}
We shall apply \eqref{basic_interp} to $F(Z):=F_{\bu\mathbf v}^s(Z,z)$ for any fixed $s\in 2\N$, where %$F_{\mathbf u\mathbf v}(Z,z)$ defined as
\begin{equation*}%\label{eq_comp_F(X)}
...
%\begin{lemma}\label{lemm_comp_4}
 ...
 \end{equation}
 for some constant $C>0$. If \eqref{lemm_comp_4} holds, then combining \eqref{basic_interp} with  Gr\"onwall's inequality we obtain that for any fixed $s\in 2\N$ and constant $\e>0$, 
 $$\E\left|\bu^\top \left(G(Z^1,z)-\Pi(z)\right)\bv\right|^s  \lesssim (n^\e q)^{s}.$$
Then applying Markov's inequality and noticing that $\e$ can be chosen arbitrarily small, we conclude \eqref{aniso_law}. 
...
\end{split}
\ee
%As in \eqref{SVDW}, $\{\mu_k\}_{1\le k \le p}$ are the eigenvalues of $WW^\top$. 
By Lemma \ref{SxxSyy} and assumption \eqref{assm2}, we have that 
%\be\label{lambdap} 
...
%\end{array}} \right),
\end{align*}
and define the partial traces $m^{(\mathbb T)}$ and $m^{(\mathbb T)}_k$, $k=0,1,2,$ by replacing $G$ with $G^{(\mathbb T)}$ in \eqref{defm}.
...
Note that for any $\fa\in \cal I$, $H^{(\fa)}$ and $G^{(\fa)}$ also satisfies the assumptions for Lemma \ref{lemm apri}. Hence \eqref{priorim} and \eqref{priordiff} also hold for $G^{(\fa)}$. Now applying Lemma \ref{largedeviation} to (\ref{Zi}) and \eqref{Zmu}, %and using the a priori bound \eqref{priorim}, 
...
\end{split}
\end{equation}
where in the last step we used \eqref{priorim} to get that for any $\mu\in \cal I_1\cup \cal I_2$,
...
Note that comibining \eqref{priorim} with \eqref{Zestimate1}, we immediately conclude \eqref{entry_diagonal} for $\fa\ne \fb$.
...
\be\label{Xiz}\Xi(z):=\left\{ |m_{1}(z)-m_{1c}(z)| + |m_{2}(z)-m_{2c}(z)| \le (\log n)^{-1/2}\right\}.\ee
Note that by \eqref{Lipomega}, we have $|m_{1c}+r_1^{-1}a_1|\lesssim (\log n)^{-1}$ and $|m_{2c}+r_2^{-1}a_2|\lesssim (\log n)^{-1}$ (recall that $a_1=-r_1m_{1c}(0)$ and $a_2=-r_2m_{2c}(0)$). Together with \eqref{selfomega}, \eqref{a23} and \eqref{assm32}, we obtain the following estimates 
\be\label{Gsim1}
 |m_{1c}| \sim |m_{2c}| \sim 1, \quad |z+\lambda_i^2 r_1m_{1c} + r_2 m_{2c}|\sim 1  ,\quad \left|1 + \gamma_n m_c\right|\sim  |1+\gamma_n m_{0c}|\sim 1  ,
 \ee
 uniformly in $z\in \mathbf D$, where we abbreviated
 \be\label{defn mc1c}m_c(z):=-\frac1p\sum_{i=1}^p\frac{1}{z+\lambda_i^2 r_1m_{1c} +r_2m_{2c}},\quad m_{0c}(z):=-\frac1p\sum_{i=1}^p\frac{\lambda_i^2}{z+\lambda_i^2 r_1m_{1c} +r_2m_{2c}}.\ee
...
\end{align}
where we recall Definition \ref{defnMinor}, and define %\eqref{m123}, and 
...
With \eqref{Gsim1} and the definition of $\Xi$, we get that $\mathbf 1(\Xi)|z + \lambda_i^2 r_1m_1+r_2m_2|\sim1$. Hence using (\ref{self_Gii}) and \eqref{erri}, we obtain that
\be\label{Gmumu0}
\mathbf 1(\Xi)G_{ii}=\mathbf 1(\Xi)\left[-\frac{1}{z + \lambda_i^2 r_1 m_1+r_2 m_2} +\OO_\prec\left(n^{-1/2}\right)\right].
\ee
Plugging it into the definitions of $m$ and $m_0$ in \eqref{defm}, we get
\begin{align}
...
\end{align}
As a byproduct, we obtain from these two estimates and \eqref{defn mc1c} that  
\be\label{Gsim11}
\mathbf 1(\Xi)\left(|m-m_{c}| +|m_0-m_{0c}| \right)\lesssim (\log n)^{-1/2}, \quad \text{with high probability}. 
\ee
...
\ee
Now combining \eqref{self_Gmu1}, \eqref{self_Gmu2}, \eqref{erri} and \eqref{Gsim2}, we obtain that for $\mu \in \cal I_1$ and $\nu \in \cal I_2,$ %can obtain that with high probability,
...
\noindent{\bf Step 3: $\Xi$ holds with high probability.} In this step, we show that the event $\Xi(z)$ in fact holds with high probability for all $z\in \mathbf D$. Once we have proved this fact, applying Lemma \ref{lem_stabw} to \eqref{selfcons_lemm} immediately shows that $(m_1(z),m_2(z))$ is close to $(m_{1c}(z),m_{2c}(z))$ up to an error of order $\OO_\prec(n^{-1/2})$. 

We claim that it suffices to show
\be\label{Xiz0}
|m_{1}(0)-m_{1c}(0)| + |m_{2}(0)-m_{2c}(0)| \prec n^{-1/2}.
\ee
Note that by \eqref{Lipomega} and \eqref{priordiff} we get 
$$ |m_{1 c}(z)-m_{1 c}(0)|+|m_{2 c}(z)-m_{2 c}(0)|=\OO((\log n)^{-1}),\quad  |m_{1}(z)-m_{1}(0)|+ |m_{2}(z)-m_{2}(0)|=\OO((\log n)^{-1})$$ 
with high probability for all $z\in \mathbf D$. Thus if \eqref{Xiz0} holds, we can obtain that 
\be\label{roughh1} 
\sup_{z\in \mathbf D} \left(|m_{1}(z)-m_{1c}(z)| + |m_{2}(z)-m_{2c}(z)|\right)  \lesssim (\log n)^{-1} \quad \text{with high probability},\ee
and %Moreover, with this estimate and \eqref{Lipomega}, we conclude that 
\be\label{roughh2} \sup_{z\in \mathbf D} \left( |m_{1}(z)-m_{1 c}(0)|+ |m_{2}(z)-m_{2c}(0)|\right) \lesssim (\log n)^{-1} \quad \text{with high probability}.\ee
...
\be\label{Xizz}
|m_{1}(z)-m_{1 c}(z)|+ |m_{2}(z)-m_{2 c}(z)| \prec n^{-1/2} 
\ee
uniformly for all $z\in \mathbf D$. Together with \eqref{Gii0} and \eqref{Gii}, \eqref{Xizz} implies that
\be\label{Xizz2} \max_{\mu\in \cal I_1} |G_{\mu\mu}-m_{1c}|+ \max_{\nu\in \cal I_2} |G_{\nu\nu}-m_{2c}|\prec n^{-1/2}.
\ee
Now plugging \eqref{Xizz} into \eqref{self_Gii} and using \eqref{Xizz2}, we get the diagonal estimate
\be\label{diagest}
\max_{\fa\in \cal I}|G_{\fa\fa}(z)-\Pi_{\fa\fa}(z)| \prec n^{-1/2}. 
\ee
Together with the off-diagonal estimate in \eqref{Zestimate1}, we conclude the proof of Lemma \ref{prop_entry}. 
\end{proof}
...
Together with \eqref{self_Gmu1}, \eqref{self_Gmu2} and \eqref{erri}, we obtain that \eqref{Gii} holds at $z=0$ even without the indicator function $\mathbf 1(\Xi)$. Furthermore, we have that with high probability,
$$  \left|\lambda_i^2 r_1m_1(0)+r_2m_2(0)\right|=\left|\frac{\lambda_i^2 r_1}{ 1+\gamma_n m_0(0)} +\frac{r_2}{1+\gamma_n m(0)}+ \OO_\prec (n^{-1/2})\right| \sim 1 .$$
 Then combining this estimate with (\ref{self_Gii}) and \eqref{erri}, we obtain that \eqref{Gmumu} and \eqref{Gmumu2} hold at $z=0$ even without the indicator function $\mathbf 1(\Xi)$. Finally, plugging \eqref{Gmumu} and \eqref{Gmumu2} into \eqref{Gii}, we conclude that \eqref{selfcons_lemm} holds at $z=0$, that is,
...
Denoting $y_{1}=-m_{1}(0)$ and $y_{2}=-m_{2}(0)$, by \eqref{Gii} we have
...
Also one can verify from \eqref{selfcons_lemma222} that $(r_1y_1,r_2y_2)$ satisfies approximately the same system of equations as \eqref{selfomega0}:
\be\label{selfcons_lemm000}
\begin{split}
r_1y_1+r_2 y_2 = 1-\gamma_n + \OO_\prec (n^{-1/2}),\quad f(r_1y_1)=1 + \OO_\prec (n^{-1/2}).
\end{split}
\ee
The first equation and \eqref{omega12} imply that $y_1 \in [0,r_1^{-1}(1-\gamma_n)]$ with high probability. Since $f$ is strictly increasing and has bounded derivative on $[0,r_1^{-1}(1-\gamma_n)]$, by basic calculus the second equation in \eqref{selfcons_lemm000} gives that $|y_1-r_1^{-1}a_1|\prec n^{-1/2}$. Plugging it into the first equation of \eqref{selfcons_lemm000}, we get $|y_2-r_2^{-1}a_2|\prec n^{-1/2}$. This concludes \eqref{Xiz0}.
\end{proof}
...
%\vspace{10pt}
%
%\noindent{\bf Step 4: Fluctuation averaging.} By \eqref{Gmumu} and using $\Pi_{ii}=-(z + \lambda_i^2 m_{2c}+m_{2c})^{-1}$, to show \eqref{aver_in} it suffices to prove that
%\be \label{avergoal} 
%|m_{2}-m_{2c}|+|m_{3}-m_{2c}|+|[Z]| \prec n^{-1}.
%\ee
...
%\end{split}
%\end{equation}
%Then using Lemma \ref{lem_stabw}, we get $|m_{2}-m_{2c}|+|m_{3}-m_{2c}|\prec n^{-1}$, which concludes \eqref{avergoal}. This concludes the proof of \eqref{aver_in}, and hence Lemma \ref{prop_entry}.
