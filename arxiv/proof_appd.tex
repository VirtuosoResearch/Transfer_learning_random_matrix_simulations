...
\ee
Recall that $M=\Sig_1^{1/2} \Sig_2^{-1/2}$ has a singular value decomposition as in \eqref{eigen2}. We assume that % the singular values satisfy that
\begin{equation}\label{assm32}
\tau \le \lambda_p \le \lambda_1 \le \tau^{-1} .%, \quad \max\left\{\pi_A^{(n)}([0,\tau]), \pi_B^{(n)}([0,\tau])\right\} \le 1 - \tau .
\end{equation}
Finally, recalling the notations in \eqref{labelZ}, we assume that $Z^{(1)}$ and $Z^{(2)}$ satisfy
\begin{equation}
...
\begin{remark}
Whenever \eqref{eq_support} holds, we say $n^{-1/2}Z^{(1)}$ and $n^{-1/2}Z^{(2)}$ satisfy the {\it{bounded support condition}} with $q$, or $n^{-1/2}Z^{(1)}$ and $n^{-1/2}Z^{(2)}$ have support $q$. 
...
By Example \ref{example stoc}, if the entries of $Z^{(1)}$ have finite moments up to any order, %as in equation \eqref{assmAhigh}, 
