...
Then for any constant $\e>0$, we have that with high probability,
...
Suppose Assumption \ref{assm_big1} holds. Then there exists a constant $C>0$ such that the following estimates hold uniformly in $z,z'\in \mathbf D$ with high probability: 
\be\label{priorim}
...
\end{lemma}
\begin{proof}
Our proof is a simple application of the spectral decomposition of $G$. Recall the matrix $A$ defined in \eqref{abbr_A}. Let 
\be\label{SVDW}
...
By Lemma \ref{SxxSyy} and equation \eqref{assm2}, we have that with high probability 
%\be\label{lambdap} 
...
and it suffices to consider the resolvent in \eqref{resolv Gauss1} throughout the whole proof. 
...
\begin{equation*}
  Z_{{a}} :=(1-\mathbb E_{{a}})\big(G_{{a}{a}}\big)^{-1}, %\quad {a}\notin \mathbb T,
\end{equation*}
where $\mathbb E_{{a}}[\cdot]:=\mathbb E[\cdot\mid H^{({a})}]$ denotes the partial expectation over the entries in the ${a}$-th row and column of $H$. Now using (\ref{resolvent2}), we get that for $i \in \cal I_0$, 
\begin{align}
&Z_i =  \frac{\lambda_i^2}{n} \sum_{\mu ,\nu\in \mathcal I_1}  G^{(i)}_{\mu\nu} \left(\delta_{\mu\nu} - Z^{(1)}_{\mu i}Z^{(1)}_{\nu i}\right)+\frac1n \sum_{\mu ,\nu\in \mathcal I_2}  G^{(i)}_{\mu\nu} \left( \delta_{\mu\nu} - Z^{(2)}_{\mu i}Z^{(2)}_{\nu i}\right) - 2 \frac{\lambda_i}{n} \sum_{\mu\in \cal I_1,\nu\in \mathcal I_2} Z^{(1)}_{\mu i}Z^{(2)}_{\nu i}G^{\left( i \right)}_{\mu\nu},  \label{Zi}
\end{align}
and for $\mu\in \cal I_1$ and $\nu\in \cal I_2$, 
\begin{align}
& Z_\mu= \frac{1}{n} \sum_{i,j \in \mathcal I_0}  {\lambda_i \lambda_j}G^{(\mu)}_{ij} \left(\delta_{ij} - Z^{(1)}_{\mu i}Z^{(1)}_{\mu j}\right), \quad Z_\nu = \frac{1}{n} \sum_{i,j \in \mathcal I_0} G^{(\nu)}_{ij} \left( \delta_{ij} - Z^{(2)}_{\nu i}Z^{(2)}_{\nu j}\right).\label{Zmu} 
\end{align}
...
%\end{split}
%\end{equation}
which controls the size of the off-diagonal entries. The following lemma gives the desired large deviation estimate on $\Lambda_o$ and $Z$ variables.
...
\begin{align}
\Lambda_o + \max_{{a}\in \cal I} |Z_{{a}}|  \prec n^{-1/2} \label{Zestimate1}
\end{align}
...
{\cor here} Note that for any ${a}\in \cal I$, $H^{({a})}$ and $G^{({a})}$ also satisfies the assumptions for Lemma \ref{lemm apri}. Hence equation \eqref{priorim} and equation \eqref{priordiff} also hold for $G^{({a})}$. Now applying Lemma \ref{largedeviation} to (\ref{Zi}) and equation \eqref{Zmu}, %and using the a priori bound equation \eqref{priorim}, 
...
\be\label{GG*}\sum_{\nu \in \cal I_1\cup \cal I_2 }  \left| G_{\mu\nu}^{(i)}  \right|^2\le \sum_{{a} \in \cal I }  \left| G_{\mu{a}}^{(i)}  \right|^2 =\left[G^{(i)}(G^{(i)})^* \right]_{\mu\mu} =\OO(1)\ee
...
%\end{equation}
Then we prove the off-diagonal estimate on $\Lambda_o$. For $i\in \mathcal I_1$ and ${a}\in \cal I\setminus \{i\}$, using equation \eqref{resolvent3}, Lemma \ref{largedeviation} and equation \eqref{priorim}, we obtain that 
\begin{align*}
& \left|G_{ii}^{-1}G_{i{a}}\right| \prec n^{-1/2}+\frac{1}{\sqrt{n}} \Big( \sum_{\mu \in \cal I_1\cup \cal I_2}  {\left| G_{\mu {a}}^{(i)}  \right|^2 }  \Big)^{1/2} \prec n^{-1/2}. 
%\\& \left|G_{\mu\mu}^{-1} G_{\mu{b}} \right| \prec n^{-1/2}+  \frac{1}{\sqrt n} \left( \sum_{i \in \cal I_{1}}  {\left| G_{i{b}}^{(\mu)}  \right|^2 }  \right)^{1/2} \prec n^{-1/2}.
 \end{align*}
 We have a similar estimate for $\left|G_{\mu\mu}^{-1} G_{\mu{b}} \right|$ with $\mu \in \mathcal I_1\cup \cal I_2$ and ${b}\in \cal I\setminus \{ \mu\}$.  
...
Note that comibining equation \eqref{priorim} with equation \eqref{Zestimate1}, we immediately conclude equation \eqref{entry_diagonal} for ${a}\ne {b}$.
...
\be
|\Pi_{{a}{a}}(z)| \sim 1 \ \ \text{uniformly in } z\in \mathbf D, \ {a}\in \cal I.
\ee 
