...
\end{align}
on an event $\Omega$ with $\P(\Omega)=1-\oo(1)$, where $ q= n^{-\e/(8+2\e)}$ and $\mathbf e_i$ denotes the standard basis vector along the $i$-th direction. Then with the calculations in equation \eqref{Gi00}, we get that
...
Recall that in the setting of Lemma \ref{lem_cov_derivative}, we have equation \eqref{calculate G'}. 
%\begin{align*}
...
%$Z^{(1)}$ and $Z^{(2)}$ have bounded support $q=n^{-1/2}$. Using Theorem \ref{LEM_SMALL}, %Now by Corollary \ref{main_cor}, we obtain that for any small constant $\e>0$, %with probability $1-\oo(1)$,
\be\nonumber 
\max_{z\in \C:|z|=(\log n)^{-1}}|\mathbf v^\top (G(z)-\Gi(z))\mathbf v| \prec q \|\mathbf v\|^2,\quad q:= n^{-\e/(8+2\e)},
\ee
...
The rest of this section is devoted to the proof of Theorem \ref{LEM_SMALL}. First in this section, we show that the limiting equation \eqref{selfomega_a} has a unique solution $(a_1(z), a_2(z))$ for any $z\in \mathbf D$ in \eqref{SSET1}. Otherwise, Theorem \ref{LEM_SMALL} will be a vacuous statement. 


First observe that when $z=0$, equation \eqref{selfomega_a} can be reduced to \eqref{eq_a12extra}, from which we can derive an equation of $a_1$ only: 
%Note that the function 
$$f(a_1)=0,\quad \text{with}\quad f(a_1):=a_1 +\frac{1}{\rho_1+\rho_2}\cdot\frac{1}p\sum_{i=1}^p \frac{\lambda_i^2 a_1}{ \lambda_i^2 a_1+ (1- (\rho_1+\rho_2)^{-1} - a_1) } .$$
It is not hard to see that $f$ is strictly increasing on $[0,1-(\rho_1+\rho_2)^{-1}]$. {\cor here.....} Moreover, we have $f(0)=0<1$ and $f(1-\gamma_n)=1>r_1$. Hence by mean value theorem, there exists a unique solution $a_1 \in (0,1-\gamma_n)$. Moreover, it is easy to check that $f'(x)=\OO(1)$ for $x\in [0, 1-\gamma_n]$, and $f(r_1)>r_1$ if $r_1\le 1-\gamma_n$. Hence there exists a constant $\tau>0$, such that 
\be\label{a23}
...
\cor
\begin{align*}
		a_1 + a_2 = 1- \frac{1}{\rho_1 + \rho_2},\quad a_1 + \frac1{\rho_1 + \rho_2}\cdot \bigbrace{\frac{1}{p}\sum_{i=1}^p \frac{\lambda_i^2 a_1}{\lambda_i^2 a_1 + a_2}} = \frac{\rho_1}{\rho_1 + \rho_2}. 
\end{align*}
\nc
