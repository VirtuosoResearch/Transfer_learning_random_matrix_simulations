...
Suppose Assumption \ref{assm_big1} holds, and $Z_1,Z_2$ satisfy the bounded support condition \eqref{eq_support} for some deterministic parameter $q$ satisfying $ n^{-{1}/{2}} \leq q \leq n^{- \phi} $ for some constant $\phi>0$. Then for any constant $\e>0$, we have that with high probability,
...
\end{lemma}
\begin{proof}
This lemma essentially follows from \cite[Theorem 2.10]{isotropic}, although the authors considered the case with $q = n^{-1/2}$ only. The results for larger $q$ follows from \cite[Lemma 3.12]{DY}. 
...
\subsubsection{Asymptotic Limit of the Resolvent}
...
We now describe the asymptotic limit of $G(z)$ and $\cal G(z)$ as $n\to \infty$. First we define the deterministic limits of $(m_2(z), m_{3}(z))$, denoted by $(m_{2 c}(z),m_{3c}(z))$, as the unique solution to the following system of equations
...
%\end{itemize}
\end{theorem}
The proof of this theorem will be given in Section \ref{sec_Gauss}. Using a simple cutoff argument, it is easy to obtain from Theorem \ref{LEM_SMALL} the following corollary under weaker moment assumptions. 
...
\subsubsection{Proof of the Bias and Variance Asymptotics}
...
Finally, in this section we give the proof of Lemmas \ref{lem_mbehaviorw} and \ref{lem_stabw} using the contraction principle. 
...
We first show that (i) there exists a unique solution $m_{3c}(z)$ to the equation $g_z(m_{3c}(z))=1$ under the conditions in \eqref{prior1}, and (ii) the solution satisfies \eqref{Lipomega}. We abbreviate $\e(z):= m_{3 c}(z) - m_{3 c}(0)$. From \eqref{selfalter}, we obtain that 
\begin{equation} \nonumber
...
Now it suffices to prove that $h$ restricted to $B_r $ is a contraction, which then implies that ${\e}:=\lim_{k\to\infty} { \e}^{(k)}$ exists and $m_{3c}(0)+\e(z)$ is a unique solution to the second equation of \eqref{selfalter} subject to the condition $\|{\e}\|_\infty \le r$. 
...
$h$ is indeed a contraction mapping on $ B_r$. This proves both the existence and uniqueness of the solution $m_{3c}(z)=m_{3c}(0)+\e(z)$, if we choose $c_0$ in \eqref{prior1} as $c_0=\min\{c_\delta, r\}$. After obtaining $m_{3c}(z)$, we can then find $m_{2c}(z)$ using the first equation in \eqref{selfalter}. 
...
\be\label{endalter}|{ \e}| \le \sum_{k=0}^\infty |{  \e}^{(k+1)}-{ \e}^{(k)}| \le 2\wt C|z| \ \Rightarrow \ |m_{3c}(z)-m_{3c}(0)|\le 2\wt C|z| .\ee
%This gives the bound \eqref{Lipomega} for $m_{3c}(z)$. 
Using the first equation in \eqref{selfalter}, we immediately obtain the bound  $ r_1|m_{2c}(z)-m_{2c}(0)| \le C|z|$, which gives \eqref{Lipomega} as long as if $r_1\gtrsim 1$. To deal with the small $r_1$ case, we go back to the first equation in \eqref{selfomega} and treat $m_{2c}(z)$ as the solution to the following equation:
...
\be\label{selfalter2}r_1 m_{2}=-(1-\gamma_n) - r_2m_{3} - z\left(  {m_{3}^{-1}}+1\right) + \cal E'_2(z),\quad g_z(m_{3}(z))=1+ \cal E'_3(z),\ee
 with $|\cal E'_2(z)|+ |\cal E'_3(z)|=\OO(\delta(z))$. Then we subtract the equations \eqref{selfalter} from \eqref{selfalter2}, and consider the contraction principle for the function $\e (z):= m_{3}(z) - m_{3 c}(z)$.  The rest of the proof is exactly the same as the one for Lemma \ref{lem_mbehaviorw}, so we omit the details.
\end{proof}
...
Note that by \eqref{Lipomega}, we have $|m_{2c}+x_2|\lesssim (\log n)^{-1}$ and $|m_{3c}+x_3|\lesssim (\log n)^{-1}$ (recall that $x_2=-m_{2c}(0)$ and $x_3=-m_{3c}(0)$). Together with \eqref{selfomega}, \eqref{a23} and \eqref{assm32}, we obtain the following estimates 
\be\label{Gsim1}
 ...
 \be\label{defn mc1c}m_c(z):=-\frac1p\sum_{i=1}^p\frac{1}{z+\lambda_i^2 r_1m_{2c} +r_2m_{3c}},\quad m_{1c}(z):=-\frac1p\sum_{i=1}^p\frac{\lambda_i^2}{z+\lambda_i^2 r_1m_{2c} +r_2m_{3c}}.\ee
