...
&=n^{-1} \left|\mathbf e_i^\top \Sigma_2^{-1/2}V \left(\cal G(0)-\Gi(0)\right)V^\top\Sigma_2^{-1/2}\Sigma\mathbf e_i\right| \prec n^{-1}q\|V^\top\Sigma_2^{-1/2} \Sigma\mathbf e_i\| \lesssim n^{-1}q\| \Sigma\mathbf e_i\|,\label{G0Pi0}
\end{align}
...
\begin{align*}
 ...
 \end{align*}
 on event $\Omega$, where we used Fact \ref{lem_stodomin} (i) in the last step. 
%with probability $1-\oo(1)$. 
This concludes equation \eqref{lem_cov_shift_eq} using Definition \ref{stoch_domination} and the fact that $c_{\varphi}$ be any fixed value within $(0, \frac{\varphi - 4}{2\varphi})$.  %if we rename $(r_1X^{(2)},r_2x_3)$ to $(a_1,a_2)$. 
...
Under the constant $\varphi$-th moment condition, using Corollary \ref{main_cor} we obtain that
%$Z^{(1)}$ and $Z^{(2)}$ have bounded support $q=n^{-1/2}$. Using Theorem \ref{LEM_SMALL}, %Now by Corollary \ref{main_cor}, we obtain that for any small constant $\e>0$, %with probability $1-\oo(1)$,
\be\nonumber 
...
There exist constants $c_0, C_0>0$ depending only on $\tau$ in Assumption \ref{assume_rm} such that the following statements hold.
