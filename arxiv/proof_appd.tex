\section{Missing Proof of the Two-task Case} \label{sec_maintools}
...
\subsection{Proof of the Random Matrix Results}
...
\end{definition}
\begin{remark}
Note that since we allow for an $n^\e$ factor in the definition of stochastic domination, we have the seemingly strange relation $(\log n)^C\prec 1$ for any constant $C>0$. 
\end{remark}

\begin{example}\label{example stoc}
...
if we choose $k$ large enough such that $k\e>D$. As a special case, we have $|\xi|\prec 1$ for a Gaussian random variable $\xi$.
\end{example}

%\HZ{Why is this below part of the def. of stochastic domination?}
In the following proof, we say an event $\Xi$ holds with high probability if $1\prec \mathbf 1_{\Xi}$, that is, for any constant $D>0$, $\mathbb P(\Xi)\ge 1- n^{-D}$ for large enough $n$. Moreover, we say $\Xi$ holds with high probability in an event $\Omega$ if for any constant $D>0$, $\mathbb P(\Omega\setminus \Xi)\le n^{-D}$ for large enough $n$.

In this section, we shall prove a more fundamental random matrix result in a little more general setting than the one in Section \ref{sec_prelim}. Then Lemma \ref{lem_cov_shift} and Lemma \ref{lem_cov_derivative} will follow from this result as immediate corollaries. We first state the exact assumptions needed for our purpose.

%We summarize our basic assumptions here for future reference. %Note that this assumption is in accordance with the assumptions of Lemma \ref{lem_cov_shift} and Lemma \ref{lem_cov_derivative}, except that we rescale the entries of $Z_1$ and $Z_2$ by $n^{-1/2}$. %and allow $\rho_1$ to be smaller than 1.
\begin{assumption}\label{assm_big1}
As in Section \ref{sec_prelim}, we consider random matrices $X_1=Z_1\Sigma_1^{1/2}$ and $X_2=Z_2\Sigma_2^{1/2}$, where $Z_1$ and $Z_2$ are independent $n_1\times p$ and $n_2\times p$ random matrices with i.i.d. entries of zero mean and unit variance. Recall that $\rho_1= n_1/p$ and $\rho_2=n_2/p$. We assume that for a small constant $0<\tau<1$,
...
\ee
Assume that $M=\Sig_1^{1/2} \Sig_2^{-1/2}$ has a singular value decomposition as in \eqref{eigen2}, such that % the singular values satisfy that
\begin{equation}\label{assm32}
\tau \le \lambda_p \le \lambda_1 \le \tau^{-1} .%, \quad \max\left\{\pi_A^{(n)}([0,\tau]), \pi_B^{(n)}([0,\tau])\right\} \le 1 - \tau .
\end{equation}
Finally, we assume that $Z_1$ and $Z_2$ satisfy
\begin{equation}
n^{-1/2}\max_{i,j}\vert (Z_1)_{ij}\vert \prec q,\quad n^{-1/2}\max_{i,j}\vert (Z_2)_{ij}\vert \prec q, \label{eq_support}
\end{equation}
...
%rescale the matrices $Z_1$ and $Z_2$ such that their entries have variance $n^{-1}$, where $n:=n_1+n_2$. \HZ{I don't think is the point; I think you want to say that the sample covariance of both tasks $(Z_1^{\top}Z_1 + Z_2^{\top}Z_2)/n$ is what matters} 
...
We have assumed that $Z_1$ and $Z_2$ are $n_1\times p$ and $n_2\times p$ random matrices with i.i.d. entries satisfying
...
%\mathbb{E} \vert (Z_1)_{11} \vert^4 +\mathbb{E} \vert (Z_2)_{11} \vert^4  \le C, 
%\ee
...
\begin{remark}
Whenever \eqref{eq_support} holds, we say $n^{-1/2}Z_1$ and $n^{-1/2}Z_1$ satisfy the {\it{bounded support condition}} with $q$, or $n^{-1/2}Z_1$ and $n^{-1/2}Z_1$ have support $q$. 
...
By Example \ref{example stoc}, we see that if the entries of $Z_1$ have finite moments up to any order, %as in equation \eqref{assmAhigh}, 
then $n^{-1/2}Z_1$ has bounded support $n^{-1/2}$. More generally, if the entries of $Z_1$ have finite $a$-th moment for some constant $a>4$, then using Markov's inequality and a simple union bound we get that
$$\P\left(n^{-1/2}\max_{i,j}|(Z_1)_{ij}|\ge (\log n) n^{2/a-1/2}\right)=\OO((\log n)^{-a}).$$
In other words, $n^{-1/2}Z_1$ has bounded support $q=n^{2/a-1/2}$ on a event with probability $1-\oo(1)$. This fact will be used in the proof of Corollary \ref{main_cor} below. The above discussion shows that the bounded support condition provides great flexibility in dealing with finite moment conditions. 
%is essentially introduced to deal with the moment conditions; see Corollary \ref{main_cor} below for more details.
%\end{example}
\end{remark}
...
Before entering into the formal proof, we state two preparation lemmas, both of which have been used crucially in some previous proofs. First, for $n^{-1/2}Z_1$ and $n^{-1/2}Z_2$ with bounded support $q$, we have the following estimates on their singular values. We have used it in our previous proofs; see equation \eqref{eq_isometric}.   

 
%Then we state the following lemma on the eigenvalues of $Z_1^\top Z_1$ and $Z_2^\top Z_2$, which are denoted as $\lambda_1 (Z_1^\top Z_1) \ge \cdots \ge \lambda_{p} (Z_1^\top Z_1)$ and $\lambda_1 (Z_2^\top Z_2) \ge \cdots \ge \lambda_p (Z_2^\top Z_2)$. We have used it in our previous proofs; see equation \eqref{eq_isometric}. 
...
\lambda_1(Z_1^\top Z_1) \le {(\sqrt{n_1}+\sqrt{p})^2} + n_1^{1+\e} q,
\ee
and
\be\label{op rough2} 
 (\sqrt{n_2}-\sqrt{p})^2  -  n_2^{1+\e} q \le  \lambda_p (Z_2^\top Z_2)  \le  \lambda_1(Z_2^\top Z_2) \le  (\sqrt{n_2}+\sqrt{p})^2 +  n_2^{1+\e} q .
\ee
...
\end{proof}
\begin{remark}
it is well-known that the eigenvalues of $n_2^{-1}Z_2^\top Z_2$ are all inside the support of the Marchenko-Pastur law $[(1-\sqrt{p/n_2})^2-\oo(1) ,(1+\sqrt{p/n_2})^2+\oo(1)]$ with probability $1-\oo(1)$ \cite{No_outside}. Here the estimate \eqref{op rough2} have improved the error to $n_2^\e q$ and the probability to $1-\OO(n^{-D})$.
\end{remark}
...
\Big\vert \sum_{i} (|x_i|^2-\mathbb E|x_i|^2) B_{ii}  \Big\vert  \prec q B_d   , \quad &\Big\vert \sum_{i\ne j} \bar x_i B_{ij} x_j \Big\vert  \prec qB_o + \frac{1}{n}\Big(\sum_{i\ne j} |B_{ij}|^2\Big)^{{1}/{2}} ,
\end{align*}
where $B_d:=\max_{i} |B_{ii} |$ and $B_o:= \max_{i\ne j} |B_{ij}|.$ Moreover, if all the moments of $\sqrt{n}x_i$ and $\sqrt{n}y_j$ exist in the sense of equation \eqref{assmAhigh}, then we have stronger bounds
\begin{align*}
...
which has been proved to be useful in studying the local laws of random matrices of the Gram type \cite{Anisotropic, AEK_Gram, XYY_circular}. We define the following $(p+n)\times (p+n)$ self-adjoint block matrix, which is a linear function of $Z_1$ and $Z_2$:
...
%We define the $(n+N)\times (n+N)$ block matrix
 \begin{equation}\label{linearize_block}
   H \equiv H(Z_1,Z_2): = n^{-1/2}\left( {\begin{array}{*{20}c}
...
and the resolvent of $  n^{-1}\Lambda U^\top Z_1^\top Z_1 U\Lambda  + n^{-1}V^\top Z_2^\top Z_2V$ as
\be\label{mainG}
...
\end{array}} \right)^{-1},\quad \cal G_R:=(W^\top W - z)^{-1} ,
 \end{equation}
 where we abbreviated $W:=n^{-1/2}(\Lambda U^\top Z_1^\top, V^\top Z_2^\top)$, and
...
Now we give a heuristic derivation of the matrix limit when the entries of $Z_1$ and $Z_2$ are i.i.d. Gaussian. Note that in this case,
...
with high probability. Taking average we obtain that 
...
\end{align*}
with high probability, which give the approximate self-consistent equations for $(m_1,m_2)$. Compare them to the deterministic self-consistent equations in equation \eqref{selfomega}, one can observe that we should have $(m_1,m_2) =(M_{1}, M_{2})+\oo(1)$ with high probability. Inserting this approximate identity into equation \eqref{1self_Gii}-equation \eqref{2self_Gmu}, we see that for  $i \in \cal I_0$, $\mu \in \cal I_1$ and $\nu\in \cal I_2$,
$$G_{ii}=-(z +\lambda_i^2 r_1  M_{1} + r_2 M_{2}+\oo(1))^{1/2},\quad G_{\mu\mu}=M_{1}+\oo(1),\quad G_{\nu\nu}=M_{2}+\oo(1),$$
with high probability. These explain the diagonal entries of $\Pi$ in equation \eqref{defn_piw}. For the off-diagonal entries, they are close to zero due to concentration. For example, for $i\ne j\in \cal I_1$, by Schur complement formula (see (\ref{resolvent3})), we have
...
The above arguments are the core of the main proof. To have a rigorous proof, we need to estimate each error carefully, and extend the Gaussian case to the more general case where the entries of $Z_1$ and $Z_2$ only satisfy certain moment assumptions. These will make the real argument rather tedious, but the methods we used are standard in the random matrix literature \cite{erdos2017dynamical,Anisotropic}.

\fi

\subsubsection{Analysis of the Limiting Equations}\label{sec contract}

If $z=0$, then the equations in equation \eqref{selfomega} in are reduced to 
...
\end{split}
\ee
where $a_1:=-r_1M_{1}(0)$ and $a_2:=-r_2M_{2}(0)$. Note that the function 
$$f(a_1):=a_1 +\frac{\gamma_n}p\sum_{i=1}^p \frac{\lambda_i^2 a_1}{ \lambda_i^2 a_1+ (1-\gamma_n - a_1) } $$
is strictly increasing on $[0,(1-\gamma_n)]$. Moreover, we have $f(0)=0<1$ and $f(1-\gamma_n)=1>r_1$. Hence by mean value theorem, there exists a unique solution $a_1 \in (0,1-\gamma_n)$. Moreover, it is easy to check that $f'(x)=\OO(1)$ for $x\in [0, 1-\gamma_n]$, and $f(r_1)>r_1$ if $r_1\le 1-\gamma_n$. Hence there exists a constant $\tau>0$, such that 
\be\label{a23}
 r_1 \tau \le   a_1 \le \min\{ (1-\gamma_n) -r_1 \tau,r_1( 1-\tau)\},\quad \tau<a_1\le 1 -\gamma_n - r_1\tau .
\ee 

\eqref{selfomega_a} reduces to the following equations ...... {\cor Introduce $(M_1(z),M_2(z))$. First we define the deterministic limits of $(m_1(z), m_{2}(z))$, denoted by $(M_{1}(z),M_{2}(z))$ 
%\HZ{This $M_{1}, M_{2}$ notation is awkward; consider changing it to ${M}_1(z), {M}_2(z)$ (or  better, say $a_1(z), a_2(z)$? if they correspond to $a_1, a_2$ when $z=0$)}, 
as the unique solution to the following system of equations
\begin{equation}\label{selfomega}
\begin{split}
...
There exist constants $c_0, C_0>0$ depending only on $\tau$ in equation \eqref{assm2}, equation \eqref{assm3}, equation \eqref{assm32} and equation \eqref{a23} such that the following statements hold. 
...
\be\label{Lipomega}
 |M_{1}(z) - M_{1}(0)| + |m_{2 c}(z) - m_{2 c}(0)| \le C_0|z|.
\ee
%and
%\be\label{Immcw}
...
\end{lemma}
The proof is a standard application of the contraction principle. For reader's convenience, we will include its proof in Section \ref{sec contract}. As a byproduct of the contraction mapping argument there, we also obtain the following stability result that will be used in the proof of Theorem \ref{LEM_SMALL}. 


\begin{lemma} \label{lem_stabw}
There exist constants $c_0, C_0>0$ depending only on $\tau$ in equation \eqref{assm2}, equation \eqref{assm3}, equation \eqref{assm32} and equation \eqref{a23} such that the self-consistent equations in equation \eqref{selfomega} are stable in the following sense. Suppose $|z|\le c_0$ and $m_{k}: \C_+\mapsto \C_+$, $k=1,2$, are analytic functions of $z$ such that 
\be  \label{prior12}
|m_{1}(z) - M_{1}(0)| + |m_{2}(z) - m_{2 c}(0)|\le c_0.
\ee
...
\end{split}
\ee
for some errors satisfying $  |\mathcal E_1| +  |\mathcal E_2| \le \delta(z),$
where $\delta(z)$ is a deterministic $z$-dependent function satisfying $\delta(z) \le (\log n)^{-1}.$ Then we have 
 \begin{equation}
  \left|m_1(z)-M_{1}(z)\right| +  \left|m_2(z)-m_{2 c}(z)\right|\le C_0\delta(z).\label{Stability1}
\end{equation}
\end{lemma}
...
...... In this section we give the proof of Lemmas \ref{lem_mbehaviorw} and \ref{lem_stabw} using the contraction principle. 
 ...
 It is easy to check that the equations in equation \eqref{selfomega} are equivalent to  
\begin{equation}\label{selfalter}
r_1M_{1}=-(1-\gamma_n) - r_2M_{2} - z\left( M_{2}^{-1}+1\right),\quad g_z(M_{2}(z))=1, 
\ee
where
$$g_z(M_{2}):= - M_{2} +\frac1n\sum_{i=1}^p \frac{M_{2} }{  z -\lambda_i^2(1-\gamma_n)+ (1 - \lambda_i^2) r_2M_{2} - \lambda_i^2 z\left(  M_{2}^{-1}+1\right) }.$$
We first show that (i) there exists a unique solution $M_{2}(z)$ to the equation $g_z(M_{2}(z))=1$ under the conditions in equation \eqref{prior1}, and (ii) the solution satisfies equation \eqref{Lipomega}. We abbreviate $\e(z):= M_{2}(z) - M_{2}(0)$. From equation \eqref{selfalter}, we obtain that 
\begin{equation} \nonumber
0=\left[g_z(M_{2}(z)) -  g_0(M_{2}(0)) -g_z'(M_{2}(0))\e(z)\right] + g_z'(M_{2}(0))\e(z),
\ee
which implies
\be\nonumber%\label{selfomega1}
 \e(z) =- \frac{ g_z(M_{2}(0)) - g_0(M_{2}(0)) }{g_z'(M_{2}(0))}- \frac{ g_z(M_{2}(0)+\e(z)) -  g_z(M_{2}(0))-g_z'(M_{2}(0))\e(z)}{g_z'(M_{2}(0))}.
 \ee
...
%\end{split}
%\ee
Inspired by this equation, we define iteratively a sequence ${\e}^{(k)}(z) \in \C$ such that ${\e}^{(0)}=0$, and 
\be\label{selfomega2}
 \e^{(k+1)} =- \frac{g_z(M_{2}(0)) - g_0(M_{2}(0))}{g_z'(M_{2}(0))} -\frac{g_z(M_{2}(0)+ \e^{(k)}) -  g_z(M_{2}(0))-g_z'(M_{2}(0))\e^{(k)} }{g_z'(M_{2}(0))} .
 \ee
...
%\end{split}
%\ee
Then equation \eqref{selfomega2} defines a mapping $h:\C\to \C$, which maps $\e^{(k)}$ to $\e^{(k+1)}=h(\e^{(k)})$.
...
$$g_z'(M_{2}(0)) = -1 - \frac1n\sum_{i=1}^p \frac{ \lambda_i^2(1-\gamma_n) - z\left[1- \lambda_i^2 \left(  2M_{2}^{-1}(0)+1\right)\right]  }{  \left[z -\lambda_i^2(1-\gamma_n)+ (1 - \lambda_i^2) r_2M_{2}(0) - \lambda_i^2 z\left( M_{2}^{-1}(0)+1\right)\right]^2 }.$$
%Using equation \eqref{a23}, 
Then it is not hard to check that there exist constants $\wt c, \wt C>0$ depending only on $\tau$ in equation \eqref{assm32} and equation \eqref{a23} such that the following estimates hold: for all $z$, $\e_1$ and $\e_2$ such that $|z|\le \wt c$ and $|\e_1|  \le \wt c$, $|\e_2|  \le \wt c$,
\be\label{dust}
\left|[g_z'(M_{2}(0))]^{-1}\right|\le \wt C, \quad   \left|\frac{g_z(M_{2}(0)) - g_0(M_{2}(0))}{g_z'(M_{2}(0))}\right|  \le \wt C|z| ,
\ee
and 
\be\label{dust222}
\left|\frac{g_z(M_{2}(0)+ \e_1) -  g_z(M_{2}(0)+\e_2)-g_z'(M_{2}(0))(\e_1-\e_2) }{g_z'(M_{2}(0))}\right|  \le \wt C|\e_1-\e_2|^2 .
\ee
 By equation \eqref{dust} and equation \eqref{dust222}, there exists a sufficiently small constant $\delta>0$ depending only on $\wt C$, such that $h$ is a self-mapping 
...
Now it suffices to prove that $h$ restricted to $B_r $ is a contraction, which then implies that ${\e}:=\lim_{k\to\infty} { \e}^{(k)}$ exists and $M_{2}(0)+\e(z)$ is a unique solution to the second equation of equation \eqref{selfalter} subject to the condition $\|{\e}\|_\infty \le r$. 


From the iteration relation equation \eqref{selfomega2}, using equation \eqref{dust} one can readily check that
\be\label{k1k}
{ \e}^{(k+1)} - { \e}^{(k)}= h({\e}^{(k)}) - h({\e}^{(k-1)}) \le \wt C | { \e}^{(k)}-{ \e}^{(k-1)}|^2.
\ee
Hence as long as $r$ is chosen to be sufficiently small such that $2r\wt C\le 1/2$,  
...
$h$ is indeed a contraction mapping on $ B_r$. This proves both the existence and uniqueness of the solution $M_{2}(z)=M_{2}(0)+\e(z)$, if we choose $c_0$ in equation \eqref{prior1} as $c_0=\min\{c_\delta, r\}$. After obtaining $M_{2}(z)$, we can then find $M_{2}(z)$ using the first equation in equation \eqref{selfalter}. 

Note that with equation \eqref{dust222} and ${\e}^{(0)}= 0$, we get from equation \eqref{selfomega2} that $ |{ \e}^{(1)}| \le \wt C|z| .$
With the contraction mapping, we have the bound 
\be\label{endalter}|{ \e}| \le \sum_{k=0}^\infty |{  \e}^{(k+1)}-{ \e}^{(k)}| \le 2\wt C|z| \ \Rightarrow \ |M_{2}(z)-M_{2}(0)|\le 2\wt C|z| .\ee
%This gives the bound equation \eqref{Lipomega} for $M_{2}(z)$. 
Using the first equation in equation \eqref{selfalter}, we immediately obtain the bound  $ r_1|M_{1}(z)-M_{1}(0)| \le C|z|$, which gives equation \eqref{Lipomega} as long as if $r_1\gtrsim 1$. To deal with the small $r_1$ case, we go back to the first equation in equation \eqref{selfomega} and treat $M_{1}(z)$ as the solution to the following equation:
$$\wt g_z(M_{1}(z))=1,\quad \wt g_z(x):=- x + \frac{\gamma_n}p\sum_{i=1}^p \frac{\lambda_i^2 x}{  z+\lambda_i^2 r_1 x +r_2 M_{2}(z) }. $$
%We can calculate that 
%$$g_z'(M_{2}(0))= -1 +  \frac{\gamma_n}p\sum_{i=1}^p \frac{\lambda_i^2 (z + r_2M_{2}(z))}{  (z+\lambda_i^2 r_1 M_{1}(0) +r_2 M_{2}(z))^2 }.$$
%At $z=0$, we have 
%$$ |g_0'(M_{2}(0))|= \left|1+\frac{\gamma_n}p\sum_{i=1}^p \frac{\lambda_i^2  r_2 x_3}{  (\lambda_i^2 r_1 x_2 + r_2 x_3)^2 }\right|\ge 1,$$
%where $x_2$ and $x_3$ satisfy equation \eqref{a23}. Thus under equation \eqref{prior1} we have $|g_z'(M_{2}(0))|\sim 1$ as long as $c_0$ is taken sufficiently small. 
Then with a similar argument as the one between equation \eqref{selfalter} and equation \eqref{endalter}, we can conclude $|M_{2}(z)-M_{2}(0)|=\OO(|z|)$, which concludes equation \eqref{Lipomega} together with equation \eqref{endalter}. 
...
Under equation \eqref{prior12}, we can obtain equation \eqref{selfalter} approximately up to some small error
\be\label{selfalter2}r_1 m_{1}=-(1-\gamma_n) - r_2m_{2} - z\left(  {m_{2}^{-1}}+1\right) + \cal E'_1(z),\quad g_z(m_{2}(z))=1+ \cal E'_2(z),\ee
 with $|\cal E'_1(z)|+ |\cal E'_2(z)|=\OO(\delta(z))$. Then we subtract the equations equation \eqref{selfalter} from equation \eqref{selfalter2}, and consider the contraction principle for the function $\e (z):= m_{2}(z) - m_{2 c}(z)$.  The rest of the proof is exactly the same as the one for Lemma \ref{lem_mbehaviorw}, so we omit the details.
\end{proof}
...
%\min_{\mu\in \mathcal I_2} \vert 1 + M_{1}(z)\tilde \sigma_\mu \vert \ge \tau', \quad \min_{i\in \mathcal I_1} \vert 1 + M_{2}(z)\sigma_i  \vert \ge \tau',
%\end{equation}
...
%%\end{equation}
%for $z\in S(\tilde c, C_0,-\infty)$. Now we are ready to state the local laws for $G(X,z)$. For the purpose of proving Theorem \ref{main_thm}, we shall relax the condition equation \eqref{assm_3rdmoment} a little bit. 
...
%\end{equation}
%uniformly in $z\in \tilde S(c_0,C_0,\epsilon)\cap \{z=E+\ii\eta: E\ge \lambda_r, N\eta\sqrt{\kappa + \eta} \ge N^\epsilon\}$, where $\kappa$ is defined in equation \eqref{KAPPA}. 
%\end{itemize}
...
Suppose Assumption \ref{assm_big1} holds, and $Z_1$ and $Z_2$ satisfy the bounded support condition equation \eqref{eq_support} for a deterministic parameter $q$ satisfying $ n^{-{1}/{2}} \leq q \leq n^{- \phi} $ for some constant $\phi>0$. Then %there exists a sufficiently small constant $c_0>0$ such that 
...
%\end{itemize}
\end{theorem}
The proof of this theorem will be given in Section \ref{sec_Gauss}. Using a simple cutoff argument, it is easy to obtain from Theorem \ref{LEM_SMALL} the following corollary under certain moment assumptions. 

\begin{corollary}\label{main_cor}
Suppose Assumption \ref{assm_big1} holds. Moreover, assume that the entries of $Z_1$ and $Z_2$ are i.i.d. random variables satisfying equation \eqref{assm1} and  
\be\label{condition_4e} 
\max_{i,j}\mathbb{E}  |\sqrt{n} z^{(k)}_{ij} | ^{a}  =\OO(1),  \quad k=1,2,
\ee 
for some constant $a>4$. Then equation \eqref{aniso_law} holds for $q= n^{2/a-1/2}$ on an event $\Omega$ with $\P(\Omega)=1-\oo(1)$.
\end{corollary}
...
\ee
from which we can also get that 
\be\label{EZ norm}\|\E \wt Z_k\|=\OO(n^{-1-\e}), \quad k=1,2.
\ee
%and
...
Now $\wh Z_1$ and $\wh Z_2$ satisfy the assumptions of Theorem \ref{LEM_SMALL} with $q= n^{-c_a +\e}$, and equation \eqref{aniso_law} gives that
$$\left| \mathbf u^\top (G(\wh Z_1,\wh Z_2,z)-\Pi(z)) \mathbf v \right|  \prec  q.$$
Then using equation \eqref{meanshif}, equation \eqref{EZ norm} and equation \eqref{priorim} below, it is easy to check that  
$$\left| \mathbf u^\top (G(\wh Z_1,\wh Z_2,z)-G(\wt Z_1,\wt Z_2,z)) \mathbf v \right|  \prec  n^{-1-\e},$$
 This shows that equation \eqref{aniso_law} also holds for $G(\wt Z_1,\wt Z_2,z)$ with $q= n^{-c_a +\e}$, which concludes the proof by equation \eqref{XneX} and the fact that $\e$ can be chosen arbitrarily small.
\end{proof}


 
\subsubsection{Proof of the Bias and Variance Asymptotics}\label{sec pf RMTlemma}
...
We now use Corollary \ref{main_cor} to complete the proof of Lemma \ref{lem_cov_shift} and  Lemma \ref{lem_cov_derivative}. 

\begin{proof}[Proof of Lemma \ref{lem_cov_shift}]
Recall that in the setting of Lemma \ref{lem_cov_shift} we have equation \eqref{rewrite X as R}. 
...
If the entries of $\sqrt{n}Z_1$ and $\sqrt{n}Z_2$ have finite $(4+\e)$-th moments as in equation \eqref{assmAhigh}, then by Corollary \ref{main_cor}  we have that
%$Z_1$ and $Z_2$ have bounded support $q=n^{-1/2}$. Using Theorem \ref{LEM_SMALL}, %Now by Corollary \ref{main_cor}, we obtain that for any small constant $\e>0$, %with probability $1-\oo(1)$,
\be\label{G0Pi0}\max_{1\le i \le p}| (A(X_1^\top X_1+X_2^\top X_2)^{-1} - n^{-1}A \Sigma_2^{-1/2}V \Pi(0)V^\top\Sigma_2^{-1/2})_{ii}| \prec q\|A\| ,\ee
for $q= n^{-\e/(8+2\e)}$ on an event $\Omega$ with $\P(\Omega)=1-\oo(1)$. By equation \eqref{defn_piw}, we have
$$\Pi(0)= -(r_1M_{1}(0)\Lambda^2  +  r_2M_{2}(0))^{-1}= (a_1 V^\top M^\top M V +  a_2)^{-1},$$
with $(a_1,a_2)$ satisfying equation \eqref{selfomega0}. Thus from equation \eqref{G0Pi0} we get that
$$ \tr (A(X_1^\top X_1+X_2^\top X_2)^{-1}) = n^{-1}\tr \left[ A(a_1  \Sigma_1  +  a_2\Sigma_2)^{-1}\right] +\OO_\prec(q \|A\|) \quad \text{on $\Omega$}.$$
%with probability $1-\oo(1)$. 
This concludes equation \eqref{lem_cov_shift_eq}. %if we rename $(r_1x_2,r_2x_3)$ to $(a_1,a_2)$. 
...
\begin{align*}
 n^2\bignorm{\Sigma_2^{1/2} ( X_1^{\top}X_1 + X_2^{\top}X_2)^{-1}\beta}^2 =\beta^\top \Sigma_2^{-1/2}  \left(M^\top Z_1^\top Z_1 M +  Z_2^\top Z_2 \right)^{-2}   \Sigma_2^{-1/2}\beta,
\end{align*}
...
$$n^2\bignorm{\Sigma_2^{1/2} ( X_1^{\top}X_1 + X_2^{\top}X_2)^{-1}\beta}^2= \bv^\top \cal G^2(0)  \bv,\quad \bv:=V^\top  \Sigma_2^{-1/2} \beta .$$ 
Note that $\cal G^2(0)=\partial_z \cal G|_{z=0}$. Under the moment condition equation \eqref{assmAhigh}, using Corollary \ref{main_cor} we obtain that
%$Z_1$ and $Z_2$ have bounded support $q=n^{-1/2}$. Using Theorem \ref{LEM_SMALL}, %Now by Corollary \ref{main_cor}, we obtain that for any small constant $\e>0$, %with probability $1-\oo(1)$,
\be\nonumber 
\max_{z\in \C:|z|=(\log n)^{-1}}|\mathbf v^\top (G(z)-\Pi(z))\mathbf v| \prec q \|\mathbf v\|^2,\quad q= n^{-\e/(8+2\e)},
\ee
...
\begin{split}
  \bv^\top \cal G^2(0)\bv  = \frac{1}{2\pi \ii}\oint_{\cal C} \frac{ \bv^\top \cal G(z)\bv }{z^2}\dd z &=  \frac{1}{2\pi \ii}\oint_{\cal C} \frac{ \bv^\top\Pi(z)\bv}{z^2}\dd z +\OO_\prec(q\|\beta\|^2) \\
  &=  \bv^\top \Pi'(0)\bv + \OO_\prec(q\|\beta\|^2),
\end{split}
\ee
where $\cal C$ is the contour $\{z\in \C: |z| = (\log n)^{-1} \}$. Hence it remains to study the derivatives 
\be\label{dervPi}
\bv^\top \Pi'(0)\bv = \bv \frac{1+r_1M_{1}'(0)\Lambda^2 + r_2M_{2}'(0)}{(r_1M_{1}(0)\Lambda^2 + r_2M_{2}(0))^2}\bv,
\ee
where we need to calculate the derivatives $ M_{1}'(0)$ and $ m_{2 c}'(0)$. 
...
Taking implicit differentiation of equation \eqref{selfomega}, we obtain that
\be \nonumber%\label{dotm34}
\begin{split}
 \frac{m'_{1c}(0) }{M_{1}^2(0)}=   \frac1n\sum_{i=1}^p \frac{\lambda_i^2\left(1+\lambda_i^2 r_1m'_{1c}(0) + r_2 m'_{2c}(0)\right) }{  ( \lambda_i^2 r_1 M_{1}(0) +r_2 M_{2}(0))^2 } ,\quad \frac{m'_{2c}(0)}{M_{2}^2(0)} &=  \frac1n\sum_{i=1}^p \frac{1+\lambda_i^2 r_1m'_{1c}(0) + r_2 m'_{2c}(0)}{ ( \lambda_i^2 r_1 M_{1}(0) +r_2 M_{2}(0))^2  } .
\end{split}
\ee
...
\end{split}
\ee
where we denoted  $(a_3 ,a_4):=(r_1M_{1}'(0),r_2M'_{2}(0))$. It is not hard to see that these equations are equivalent to equation \eqref{eq_a34extra}. Then by equation \eqref{apply derivlocal} and equation \eqref{dervPi}, we get
\begin{align*}
n^2\bignorm{\Sigma_2^{1/2} ( X_1^{\top}X_1 + X_2^{\top}X_2)^{-1}\beta}^2&=\beta^\top \Sigma_2^{-1/2} V  \frac{1 + a_4+ a_3\Lambda^2 }{(a_1\Lambda^2 + a_2)^2}  V^\top  \Sigma_2^{-1/2} \beta \\
...
\subsubsection{Proof of the Anisotropic Local Law}\label{sec_Gauss}
...
%We divide the proof of Theorem \ref{LEM_SMALL} into two steps. We first prove Theorem \ref{LEM_SMALL} in the special case where $X$ is Gaussian. Then we use a self-consistent comparison arguments developed in \cite{Anisotropic} to prove Theorem \ref{LEM_SMALL} in the general case. 
 ...
 In this section, we give the proof of Theorem \ref{LEM_SMALL}. The main difficulty in the proof is due to the fact that the entries of $Y_1=Z_1 U\Lambda  $ and $Y_2={Z_2V}$ are not independent. However, notice that if the entries of $Z_1\equiv Z_1^{\text{Gauss}}$ and $Z_2\equiv Z_2^{\text{Gauss}}$ are i.i.d. Gaussian, then by the rotational invariance of the multivariate Gaussian distribution, we have
\be\label{eq in Gauss} Z_1^{\text{Gauss}} U\Lambda \stackrel{d}{=} Z_1^{\text{Gauss}} \Lambda, \quad Z_2^{\text{Gauss}} V \stackrel{d}{=} Z_2^{\text{Gauss}} ,\ee
where ``$\stackrel{d}{=}$" means ``equal in distribution". In this case, the problem is reduced to proving the anisotropic local law for $G$ with $U=\id$ and $V=\id$, such that the entries of $Y_1$ and $Y_2$ are independent. This problem can be handled using the standard resolvent methods as in e.g. \cite{isotropic,yang2019spiked,PY}. 
...
%\end{array}} \right)\right]^{-1}
%\end{equation}
%provided that $X$ is Gaussian. In particular, the entries of $\Sig^{1/2} X^{\text{Gauss}}\tilde \Sig^{1/2}$ are independent and satisfies the bounded support condition equation \eqref{eq_support} with $q=N^{-1/2}$, which make the direct proof of Theorem \ref{LEM_SMALL} possible using the methods in \cite{isotropic}. 
Then we claim the following proposition for $G_0(z)$.
\begin{proposition}\label{prop_diagonal}
Suppose Assumption \ref{assm_big1} holds, and $Z_1$ and $Z_2$ satisfy the bounded support condition equation \eqref{eq_support} with $q= n^{-1/2}$. Suppose $U$ and $V$ are identity. Then the estimate equation \eqref{aniso_law} holds for $G_0(z)$.
...
If the entries of $Z_1$ and $Z_2$ are i.i.d. Gaussian, then by Gaussian concentration $Z_1$ and $Z_2$ have support $q=n^{-1/2}$. Hence combing equation \eqref{eq in Gauss} with Proposition \ref{prop_diagonal}, we obtain that Theorem \ref{LEM_SMALL} holds when $Z_1$ and $Z_2$ are Gaussian. 
...
......... In this subsection, we describe the proof of Theorem \ref{LEM_SMALL} for general $Z_1$ and $Z_2$ satisfying the bounded support condition (\ref{eq_support}) with $q\le n^{-\phi}$ for some constant $\phi>0$. 
%As remarked at the beginning of Section \ref{sec_Gauss},
Proposition \ref{prop_diagonal} implies that equation \eqref{aniso_law} holds for Gaussian $Z_1^{\text{Gauss}}$ and $Z_2^{\text{Gauss}}$ as discussed above. Thus it suffices is to prove that for $Z_1$ and $Z_2$ satisfying the assumptions in Theorem \ref{LEM_SMALL}, we have
\begin{equation*}%\label{Gaussian_starting}
 \mathbf u^\top  \left( G(Z,z) -  G(Z^{\text{Gauss}}, z)\right) \mathbf v  \prec q 
\end{equation*}
for any deterministic unit vectors $\mathbf u,\mathbf v\in{\mathbb R}^{p+n}$ and $z\in \mathbf D$, where we abbreviated that $Z:=\begin{pmatrix}Z_1, Z_2\end{pmatrix}$ and $Z^{\text{Gauss}}:=\begin{pmatrix}Z^{\text{Gauss}}_1, Z^{\text{Gauss}}_2\end{pmatrix}$.
...
%\end{align}
%For simplicity, we denote $Y:=\Sig^{1/2} U^{*}X V\tilde \Sig^{1/2}$. Then with equation \eqref{green2} and equation \eqref{spectral2}, we get that
%\begin{align*}
...
%\end{equation}
%uniformly in $z\in \tilde S(c_0,C_0,\e)$ and any deterministic unit vector $ \mathbf v\in{\mathbb C}^{\mathcal I}$. In fact, we can obtain the more general bound equation \eqref{aniso_law}
...
%\end{lemma}
%
%Combining Lemmas \ref{lemm_boot2} and \ref{lemm_boot}, we conclude that (\ref{eq_comp_Cm}) holds for all $w\in\mathbf S$. Since $\delta$ can be chosen arbitrarily small under the condition (\ref{assm_comp_delta}), we conclude that (\ref{goal_ani2}) holds for all $w\in\mathbf S$, and equation \eqref{aniso_law} follows for all $z\in \tilde S(c_0,C_0,\e)$. What remains now is the proof of Lemma \ref{lemm_boot}. Denote
...
%%\end{proof}
%%\end{subsubsection}
We will prove the above statement using a continuous comparison argument developed in \cite{Anisotropic}, which we shall introduce now. %to prove Lemma \ref{lemm_comp_0}. 
...
We denote $Z^0:=Z^{\text{Gauss}}$ and $Z^1:=Z$. Let $\rho_{\mu i}^0$ and $\rho_{\mu i}^1$ be the laws of $Z_{\mu i}^0$ and $Z_{\mu i}^1$, respectively. For $\theta\in [0,1]$, we define the interpolated law
...
Then applying Markov's inequality and noticing that $\e$ can be chosen arbitrarily small, we conclude equation \eqref{aniso_law}. 
...
%% \end{equation}
%%\end{lemma}
%%Then in Section \ref{subsec_3moment}, we show how to relax equation \eqref{3moment} to equation \eqref{assm_3moment} for $z\in \tilde S(c_0,C_0,\e)$.
...
%%
%%In this subsection, we prove Lemma \ref{lemm_comp_0} under equation \eqref{assm_3moment} for $z\in \tilde S(c_0, C_0,\e)$. 
...
%\end{equation}
%for $z\in S(c_0,C_0,\epsilon)\cap \{z=E+\ii\eta: E\ge \lambda_r, N\eta\sqrt{\kappa + \eta} \ge N^\epsilon\}$. At the end of this section, we will show how to relax equation \eqref{3moment} to equation \eqref{assm_3moment} for $z\in \tilde S(c_0,C_0,\e)$.
%
%Note that
%\be\label{psi2}
...
%Then we prove the averaged local law for $z\in \tilde S(c_0,C_0,\e)$ under equation \eqref{assm_3moment}. By equation \eqref{psi2}, it suffices to prove 
...
%\ee
%For the second term, using equation \eqref{green2}, equation \eqref{spectral1} and recalling that $Y=\Sig^{1/2} U^{*}X V\tilde \Sig^{1/2}$, we can get that
%\begin{align}
...
Now to conclude the proof, it remains to prove Proposition \ref{prop_diagonal}. The core of the proof is  
%The main goal of this subsection is to prove 
the following entrywise local law, Lemma \ref{prop_entry}. 
\begin{lemma}\label{prop_entry}
If the assumptions of Proposition \ref{prop_diagonal} hold, %Fix $C_0>0$ and let $c_0>0$ be a sufficiently small constant. 
...
With Lemma \ref{prop_entry}, we can complete the proof of the anisotropic local law equation \eqref{aniso_law} in Proposition \ref{prop_diagonal}. %using a standard polynomialization method. %as we will explain later. 

\begin{proof}[Proof of Proposition \ref{prop_diagonal}]
With (\ref{entry_diagonal}), one can use the polynomialization method in \cite[Section 5]{isotropic} to get the anisotropic local law (\ref{aniso_law}) for $G_0$ with $q=n^{-1/2}$. The proof is exactly the same, except for some minor differences in notations. Hence we omit the details.
\end{proof}
...
\subsubsection{Entrywise Local Law}\label{sec entry}
 ...
 The rest of the proof is devoted to proving Lemma \ref{prop_entry}. In this subsection, we collect some basic estimates for the proof, while the main argument will be given in Section \ref{sec entry}. We remark that the estimates in this subsection work for general $G$, that is, we do not require $U$ and $V$ to be identity.

First with Lemma \ref{SxxSyy}, we can obtain the following a priori estimate on the resolvent $G(z)$ for $z\in \mathbf D$.% (recall equation \eqref{SSET1}).

\begin{lemma}\label{lemm apri}
Suppose the assumptions of Lemma \ref{SxxSyy} hold. Then there exists a constant $C>0$ such that the following estimates hold uniformly in $z,z'\in \mathbf D$ with high probability: 
\be\label{priorim}
...
\end{lemma}
\begin{proof}
Our proof is a simple application of the spectral decomposition of $G$. Let 
\be\label{SVDW}W= \sum_{k = 1}^{p} {\sqrt {\mu_k} \xi_k } \zeta _{k}^\top ,\quad \mu_1\ge \mu_2 \ge \ldots \ge \mu_{p} \ge 0 =\mu_{p+1} = \ldots = \mu_{n},\ee
be the singular value decomposition of $W$, where
...
By Lemma \ref{SxxSyy} and assumption equation \eqref{assm2}, we have that 
%\be\label{lambdap} 
$\mu_p \ge \lambda_p(Z_2^\top Z_2) \ge c_\tau $ with high probability for some constant $c_\tau>0$ depending on $\tau$. This further implies the estimate that
$ \inf_{z\in \mathbf D}\min_{1\le k \le p}|\mu_k-z| \ge c_\tau/2.$
Then together with equation \eqref{spectral}, we obtain equation \eqref{priorim} and equation \eqref{priordiff}.
\end{proof}
...
\end{equation}
 ...
 \item[(ii)]
 For $i\in \mathcal I_1$, $\mu \in \mathcal I_1\cup \cal I_2$, $\fa\in \cal I\setminus \{i\}$ and $\fb\in \cal I\setminus \{ \mu\}$, we have
\begin{equation}
...
Finally, in this subsection we present the proof of Lemma \ref{prop_entry}, which is the most technical part of the whole proof. Recall that under the assumptions of Lemma \ref{prop_entry}, we have $q=n^{-1/2}$ and
\be\label{diagW}W= (\Lambda Z_1^\top, Z_2^\top).\ee



\begin{proof}[Proof of Lemma \ref{prop_entry}]
The proof is divided into three steps. For simplicity, we will denote $G\equiv G_0$ in the following proof, while keeping in mind that $W$ takes the form in equation \eqref{diagW}. For simplicity, we introduce the following notations: for two (deterministic or random) positive quantities $a$ and $b$, we write $a\lesssim b$ if there exists a constant $C>0$ such that $a\le Cb$, and we write $a\sim b$ if $a\lesssim b$ and $b\lesssim a$.

\vspace{5pt}

\noindent{\bf Step 1: Large deviation estimates.} In this step, we prove some (almost) optimal large deviation estimates on the off-diagonal entries of $G$, and on the following $Z$ variables. In analogy to \cite[Section 3]{EKYY1} and \cite[Section 5]{Anisotropic}, we introduce the $Z$ variables  
\begin{equation*}
  Z_{\fa} :=(1-\mathbb E_{\fa})\big(G_{\fa\fa}\big)^{-1}, \quad \fa\notin \mathbb T,
\end{equation*}
where $\mathbb E_{\fa}[\cdot]:=\mathbb E[\cdot\mid H^{(\fa)}],$ i.e. it is the partial expectation over the randomness of the $\fa$-th row and column of $H$. Using (\ref{resolvent2}), we get that for $i \in \cal I_0$, $\mu\in \cal I_1$ and $\nu\in \cal I_2$, {\cor revise: $Z_i$ has a cross term}
\begin{align}
&Z_i =  \lambda_i^2 \sum_{\mu ,\nu\in \mathcal I_1}  G^{(i)}_{\mu\nu} \left(\frac{1}{n}\delta_{\mu\nu} - z_{\mu i}z_{\nu i}\right)+ \sum_{\mu ,\nu\in \mathcal I_2}  G^{(i)}_{\mu\nu} \left(\frac{1}{n}\delta_{\mu\nu} - z_{\mu i}z_{\nu i}\right),  \label{Zi}\\
& Z_\mu= \sum_{i,j \in \mathcal I_0}  {\lambda_i \lambda_j}G^{(\mu)}_{ij} \left(\frac{1}{n} \delta_{ij} - z_{\mu i}z_{\mu j}\right), \quad Z_\nu = \sum_{i,j \in \mathcal I_0} G^{(\nu)}_{ij} \left(\frac{1}{n} \delta_{ij} - z_{\nu i}z_{\nu j}\right).\label{Zmu} 
\end{align}
...
%\end{split}
%\end{equation}
which controls the size of the off-diagonal entries. The following lemma gives the desired large deviations estimate on $\Lambda_o$ and $Z$ variables.
...
Then the following estimates hold uniformly in all $z\in \mathbf D$:
\begin{align}
...
Note that for any $\fa\in \cal I$, $H^{(\fa)}$ and $G^{(\fa)}$ also satisfies the assumptions for Lemma \ref{lemm apri}. Hence equation \eqref{priorim} and equation \eqref{priordiff} also hold for $G^{(\fa)}$. Now applying Lemma \ref{largedeviation} to (\ref{Zi}) and equation \eqref{Zmu}, %and using the a priori bound equation \eqref{priorim}, 
we get that for any $i\in \cal I_0$, %on $\Xi$,
\begin{equation}\nonumber%\label{estimate_Zi}
\begin{split}
\left| Z_{i}\right|&\lesssim \sum_{k=1}^2 \left| \sum_{\mu ,\nu\in \mathcal I_{k}}  G^{(i)}_{\mu\nu} \left(\frac{1}{n}\delta_{\mu\nu} - z_{\mu i}z_{\nu i}\right)\right| \prec n^{-1/2}+ \frac{1}{n} \Big( \sum_{\mu,\nu \in \cal I_1\cup \cal I_2 }  {\left| G_{\mu\nu}^{(i)}  \right|^2 }  \Big)^{1/2} \prec n^{-1/2} , 
...
with high probability. Similarly, applying Lemma \ref{largedeviation} to $Z_{\mu}$ and $Z_\nu$ in (\ref{Zmu}) and using equation \eqref{priorim}, we obtain the same bound.
...
%\end{equation}
Then we prove the off-diagonal estimate on $\Lambda_o$. For $i\in \mathcal I_1$ and $\fa\in \cal I\setminus \{i\}$, using equation \eqref{resolvent3}, Lemma \ref{largedeviation} and equation \eqref{priorim}, we obtain that 
\begin{align*}
& \left|G_{ii}^{-1}G_{i\fa}\right| \prec n^{-1/2}+\frac{1}{\sqrt{n}} \Big( \sum_{\mu \in \cal I_1\cup \cal I_2}  {\left| G_{\mu \fa}^{(i)}  \right|^2 }  \Big)^{1/2} \prec n^{-1/2}. 
%\\& \left|G_{\mu\mu}^{-1} G_{\mu\fb} \right| \prec n^{-1/2}+  \frac{1}{\sqrt n} \left( \sum_{i \in \cal I_{1}}  {\left| G_{i\fb}^{(\mu)}  \right|^2 }  \right)^{1/2} \prec n^{-1/2}.
 \end{align*}
 We have a similar estimate for $\left|G_{\mu\mu}^{-1} G_{\mu\fb} \right|$ with $\mu \in \mathcal I_1\cup \cal I_2$ and $\fb\in \cal I\setminus \{ \mu\}$.  
...
Note that comibining equation \eqref{priorim} with equation \eqref{Zestimate1}, we immediately conclude equation \eqref{entry_diagonal} for $\fa\ne \fb$.
...
Note that by equation \eqref{Lipomega}, we have $|M_{1}+r_1^{-1}a_1|\lesssim (\log n)^{-1}$ and $|M_{2}+r_2^{-1}a_2|\lesssim (\log n)^{-1}$ (recall that $a_1=-r_1M_{1}(0)$ and $a_2=-r_2M_{2}(0)$). Together with equation \eqref{selfomega}, equation \eqref{a23} and equation \eqref{assm32}, we obtain the following estimates 
\be\label{Gsim1}
 |M_{1}| \sim |M_{2}| \sim 1, \quad |z+\lambda_i^2 r_1M_{1} + r_2 M_{2}|\sim 1  ,\quad \left|1 + \gamma_n m_c\right|\sim  |1+\gamma_n M_{0}|\sim 1  ,
 \ee
 uniformly in $z\in \mathbf D$, where we abbreviated
 \be\label{defn mc1c}m_c(z):=-\frac1p\sum_{i=1}^p\frac{1}{z+\lambda_i^2 r_1M_{1} +r_2M_{2}},\quad M_{0}(z):=-\frac1p\sum_{i=1}^p\frac{\lambda_i^2}{z+\lambda_i^2 r_1M_{1} +r_2M_{2}}.\ee
Plugging equation \eqref{Gsim1} into equation \eqref{defn_piw}, we get
\be
...
Suppose the assumptions of Proposition \ref{prop_diagonal} hold. Then the following estimates hold uniformly in $z \in \mathbf D$: %with $\xi$-high probability:
%\begin{equation}
...
\begin{split}
&{\mathbf 1}(\Xi) \left|\frac{1}{m_{1}} + 1 -\frac1n\sum_{i=1}^p \frac{\lambda_i^2}{  z+\lambda_i^2r_1m_{1} + r_2m_{2}  } \right|\prec n^{-1/2},\\
&  {\mathbf 1}(\Xi)\left|\frac{1}{m_{2}} + 1 -\frac1n\sum_{i=1}^p \frac{1 }{  z+\lambda_i^2 r_1m_{1} +  r_2m_{2}  }\right|\prec n^{-1/2}.
\end{split}
\ee
...
\begin{proof}
 By (\ref{resolvent2}), (\ref{Zi}) and (\ref{Zmu}), we obtain that for $i \in \cal I_0$, $\mu \in \cal I_1$ and $\nu\in \cal I_2$,
\begin{align}
\frac{1}{{G_{ii} }}&=  - z - \frac{\lambda_i^2}{n} \sum_{\mu\in \mathcal I_1} G^{\left( i \right)}_{\mu\mu}- \frac{1}{n} \sum_{\mu\in \mathcal I_2} G^{\left( i \right)}_{\mu\mu} + Z_i =  - z - \lambda_i^2 r_1m_1 - r_2m_2 + \epsilon_i,  \label{self_Gii}\\
\frac{1}{{G_{\mu\mu} }}&=  - 1 - \frac{1}{n} \sum_{i\in \mathcal I_0}\lambda_i^2 G^{\left(\mu\right)}_{ii}+ Z_{\mu} =  - 1  - \gamma_n m_0 + \epsilon_\mu,  \label{self_Gmu1}\\
\frac{1}{{G_{\nu\nu} }}&=  - 1 - \frac{1}{n} \sum_{i\in \mathcal I_0} G^{\left(\nu\right)}_{ii}+ Z_{\nu} =   - 1 - \gamma_n m + \epsilon_\nu,  \label{self_Gmu2}
\end{align}
where we recall Definition \ref{defnMinor}, and define %equation \eqref{m123}, and 
$$\epsilon_i := Z_i + \lambda_i^2 r_1\left(m_1 - m_1^{(i)}\right) + r_2\left(m_2-m_2^{(i)}\right) ,\quad \epsilon_\mu := \begin{cases}Z_{\mu} + \gamma_n(m_0-m_0^{(\mu)}) , \ &\text{if } \mu \in \cal I_1\\ Z_{\mu} +\gamma_n(m-m^{(\mu)}) , \ &\text{if } \mu \in \cal I_2\end{cases}.$$
Using (\ref{resolvent8}), we can bound that
\begin{equation}\nonumber
  |m_1 - m_1^{(i)}| \le   \frac1{n_1}\sum_{\mu\in \mathcal I_1}  \left|\frac{G_{\mu i} G_{i\mu}}{G_{ii}}\right|  \prec n^{-1},
\end{equation}
where we used (\ref{Zestimate1}) in the second step. Similarly, we can get that 
\be\label{higherr} |m - m^{(\mu)}| + |m_0-m_0^{(\mu)}| + |m_1 - m_1^{(i)}| +  |m_2 - m_2^{(i)}| \prec n^{-1}  \ee
for any $i\in \cal I_0$ and $\mu \in \cal I_{1}\cup \cal I_2$. Together with (\ref{Zestimate1}), we obtain that %for all $i$ and $\mu$,
\begin{equation}\label{erri}
\max_{i\in \cal I_0} |\epsilon_i | +\max_{\mu \in \cal I_1\cup \cal I_2} |\epsilon_\mu|  \prec n^{-1/2}.
\end{equation}
...
With equation \eqref{Gsim1} and the definition of $\Xi$, we get that $\mathbf 1(\Xi)|z + \lambda_i^2 r_1m_1+r_2m_2|\sim1$. Hence using (\ref{self_Gii}) and equation \eqref{erri}, we obtain that
\be\label{Gmumu0}
\mathbf 1(\Xi)G_{ii}=\mathbf 1(\Xi)\left[-\frac{1}{z + \lambda_i^2 r_1 m_1+r_2 m_2} +\OO_\prec\left(n^{-1/2}\right)\right].
\ee
Plugging it into the definitions of $m$ and $m_0$ in equation \eqref{defm}, we get
\begin{align}
...
\be\label{Gsim11}
\mathbf 1(\Xi)\left(|m-M| +|m_0-M_{0}| \right)\lesssim (\log n)^{-1/2}, \quad \text{with high probability}. 
\ee
Together with equation \eqref{Gsim1}, we get that %from equation \eqref{self_Gmu1} and equation \eqref{self_Gmu2} that
\be\label{Gsim2}
|1+\gamma_nm_0|\sim 1, \quad |1+\gamma_nm|\sim 1, \quad \text{with high probability on } \Xi.
\ee
Now combining equation \eqref{self_Gmu1}, equation \eqref{self_Gmu2}, equation \eqref{erri} and equation \eqref{Gsim2}, we obtain that for $\mu \in \cal I_1$ and $\nu \in \cal I_2,$ %can obtain that with high probability,
...
\end{align}
Taking average over $\mu\in \cal I_1$ and $\nu\in \cal I_2$, we get that with high probability,
...
%\ee
Finally, plugging equation \eqref{Gmumu} and equation \eqref{Gmumu2} into equation \eqref{Gii}, we conclude (\ref{selfcons_lemm}). 
\end{proof}

%The following lemma gives the stability of the equation $ f(z,m)=0$. Roughly speaking, it states that if $f(z, m_{2}(z))$ is small and $m_2(\tilde z)-M_{2}(\tilde z)$ is small for $\Im\, \tilde z \ge \Im\, z$, then $m_{2}(z)-M_{2}(z)$ is small. For an arbitrary $z\in S(c_0,C_0, \e)$, we define the discrete set
...
\noindent{\bf Step 3: $\Xi$ holds with high probability.} In this step, we show that the event $\Xi(z)$ in fact holds with high probability for all $z\in \mathbf D$. Once we have proved this fact, applying Lemma \ref{lem_stabw} to equation \eqref{selfcons_lemm} immediately shows that $(m_1(z),m_2(z))$ is close to $(M_{1}(z),M_{2}(z))$ up to an error of order $\OO_\prec(n^{-1/2})$. 

We claim that it suffices to show
\be\label{Xiz0}
|m_{1}(0)-M_{1}(0)| + |m_{2}(0)-M_{2}(0)| \prec n^{-1/2}.
\ee
Note that by equation \eqref{Lipomega} and equation \eqref{priordiff} we get 
$$ |M_{1}(z)-M_{1}(0)|+|m_{2 c}(z)-m_{2 c}(0)|=\OO((\log n)^{-1}),\quad  |m_{1}(z)-m_{1}(0)|+ |m_{2}(z)-m_{2}(0)|=\OO((\log n)^{-1})$$ 
with high probability for all $z\in \mathbf D$. Thus if equation \eqref{Xiz0} holds, we can obtain that 
\be\label{roughh1} 
\sup_{z\in \mathbf D} \left(|m_{1}(z)-M_{1}(z)| + |m_{2}(z)-M_{2}(z)|\right)  \lesssim (\log n)^{-1} \quad \text{with high probability},\ee
and %Moreover, with this estimate and equation \eqref{Lipomega}, we conclude that 
\be\label{roughh2} \sup_{z\in \mathbf D} \left( |m_{1}(z)-M_{1}(0)|+ |m_{2}(z)-M_{2}(0)|\right) \lesssim (\log n)^{-1} \quad \text{with high probability}.\ee
The condition equation \eqref{roughh1} shows that $\Xi$ holds with high probability, and the condition equation \eqref{roughh2} verifies the condition equation \eqref{prior1} of Lemma \ref{lem_stabw}. Then applying Lemma \ref{lem_stabw} to equation \eqref{selfcons_lemm}, we obtain that
\be\label{Xizz}
|m_{1}(z)-M_{1}(z)|+ |m_{2}(z)-m_{2 c}(z)| \prec n^{-1/2} 
\ee
uniformly for all $z\in \mathbf D$. Together with equation \eqref{Gii0} and equation \eqref{Gii}, equation \eqref{Xizz} implies that
\be\label{Xizz2} \max_{\mu\in \cal I_1} |G_{\mu\mu}-M_{1}|+ \max_{\nu\in \cal I_2} |G_{\nu\nu}-M_{2}|\prec n^{-1/2}.
\ee
Now plugging equation \eqref{Xizz} into equation \eqref{self_Gii} and using equation \eqref{Xizz2}, we get the diagonal estimate
\be\label{diagest}
\max_{\fa\in \cal I}|G_{\fa\fa}(z)-\Pi_{\fa\fa}(z)| \prec n^{-1/2}. 
\ee
Together with the off-diagonal estimate in equation \eqref{Zestimate1}, we conclude the proof of Lemma \ref{prop_entry}. 
\end{proof}
Now we give the proof of equation \eqref{Xiz0}.
...
Together with equation \eqref{self_Gmu1}, equation \eqref{self_Gmu2} and equation \eqref{erri}, we obtain that equation \eqref{Gii} holds at $z=0$ even without the indicator function $\mathbf 1(\Xi)$. Furthermore, we have that with high probability,
$$  \left|\lambda_i^2 r_1m_1(0)+r_2m_2(0)\right|=\left|\frac{\lambda_i^2 r_1}{ 1+\gamma_n m_0(0)} +\frac{r_2}{1+\gamma_n m(0)}+ \OO_\prec (n^{-1/2})\right| \sim 1 .$$
 Then combining this estimate with (\ref{self_Gii}) and equation \eqref{erri}, we obtain that equation \eqref{Gmumu} and equation \eqref{Gmumu2} hold at $z=0$ even without the indicator function $\mathbf 1(\Xi)$. Finally, plugging equation \eqref{Gmumu} and equation \eqref{Gmumu2} into equation \eqref{Gii}, we conclude that equation \eqref{selfcons_lemm} holds at $z=0$, that is,
...
\be\label{omega12} c \le y_1 \le 1, \quad  c\le y_2\le 1, \quad \text{with high probability}.\ee
...
Also one can verify from equation \eqref{selfcons_lemma222} that $(r_1y_1,r_2y_2)$ satisfies approximately the same system of equations as equation \eqref{selfomega0}:
\be\label{selfcons_lemm000}
\begin{split}
r_1y_1+r_2 y_2 = 1-\gamma_n + \OO_\prec (n^{-1/2}),\quad f(r_1y_1)=1 + \OO_\prec (n^{-1/2}).
\end{split}
\ee
The first equation and equation \eqref{omega12} imply that $y_1 \in [0,r_1^{-1}(1-\gamma_n)]$ with high probability. Since $f$ is strictly increasing and has bounded derivative on $[0,r_1^{-1}(1-\gamma_n)]$, by basic calculus the second equation in equation \eqref{selfcons_lemm000} gives that $|y_1-r_1^{-1}a_1|\prec n^{-1/2}$. Plugging it into the first equation of equation \eqref{selfcons_lemm000}, we get $|y_2-r_2^{-1}a_2|\prec n^{-1/2}$. This concludes equation \eqref{Xiz0}.
\end{proof}
...
%%\bigcap_{z \in S(c_0,C_0,L)} \left\{ \Lambda(z) \leq C\varphi^{20\tilde \xi}\left(q+ \sqrt{\frac{\operatorname{Im} M_{2}(z) }{N \eta}}+ \frac{1}{N\eta}\right) \right\},
%%\end{equation}
%%with $\xi$-high probability. %For notational convenience, we shall denote $m_c:=M_{1}+M_{2}$. 
%
