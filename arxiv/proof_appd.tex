...
for any $z\in \mathbf D$, and any deterministic unit vectors $\mathbf u, \mathbf v \in \mathbb R^{p+n_1+n_2}$, the following holds with high probability
...
\todo{check}
In the setting of Theorem \ref{main_cor}, assume that $Z^{(1)}$ and $Z^{(2)}$ satisfy the more general bounded support condition with $q = n^{-\frac{\varphi - 4}{2\varphi}}$.
Then we have that the anisotropic local law in equation \eqref{aniso_law} holds uniformly for any $z \in \mathbf D$, and deterministic unit vectors $\mathbf u, \mathbf v \in \mathbb R^{p+n_1+n_2}$.
%\begin{itemize}
