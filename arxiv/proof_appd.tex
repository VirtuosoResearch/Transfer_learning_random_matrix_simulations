...
\be\label{one_event}\left\{ \max_{1\le i\le n, 1\le j \le p}|Z^{(1)}_{i, j}| \le (\log n) n^{\frac{2}{\varphi}}, \ \max_{1\le i\le n, 1\le j \le p}|Z^{(2)}_{i, j}|\le (\log n) n^{\frac{2}{\varphi}} \right\}.\ee
\end{theorem}
...
\end{align*}
where in the third step we used condition \eqref{assmAhigh} \HZ{check; I don't see which one} and Markov's inequality. Similarly, we can obtain that
\begin{align*}
...
with overwhelming probability, where we also used Fact \ref{fact_minv}(ii) to bound the operator norm of $\wt Z^{(k)}$. Together with equation \eqref{priorim} below, this bound implies that % check that
