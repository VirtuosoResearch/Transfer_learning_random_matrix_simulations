\section{Proof of Bias and Variance Asymptotics} \label{sec_maintools}
...
We assume that $Z_1$ and $Z_2$ are independent $n_1\times p$ and $n_2\times p$ random matrices with real $i.i.d.$ entries satisfying equations \eqref{assm1} and \eqref{conditionA2}, $\Sigma_1$ and $\Sigma_2$ are deterministic non-negative definite symmetric matrices satisfying equations \eqref{eigen}-\eqref{assm32}, and $\rho_{1}$ and $\rho_2$ satisfy equation \eqref{assm2}.
...
%\HZ{I would merge this def below to the earlier eqs. This seems to be a standard and simple assumption.}
...
\end{equation}
Here $q\equiv q(n)$ is a deterministic parameter and usually satisfies $ n^{-{1}/{2}} \leq q \leq n^{- \phi} $ for some (small) constant $\phi>0$. Whenever (\ref{eq_support}) holds, we say that $M$ has support $q$. 
...
\end{definition}
\begin{example}
\todo{Talk about the Gaussian case and Bernoulli case.} Note that if the entries of $Z$ have finite moments up to any order, %as in equation \eqref{assmAhigh}, 
then with Markov's inequality one can check that $n^{-1/2}Z$ has bounded support $n^{-1/2}$. More generally, if the entries of $Z$ have finite $a$-th moment for some fixed $a>4$, then using a standard cut-off argument and a simple union bound, it is easy to see that $n^{-1/2}Z$ has bounded support $q=n^{2/a-1/2}$ on a event with probability $1-\oo(1)$. Hence the bounded support condition is essentially introduced to deal with the moment conditions; see Corollary \ref{main_cor} below for more details.
\end{example}
...
For general $z$ around the origin, the existence and uniqueness of the solution $(M_{1}(z),M_{2}(z))$ is given by the following lemma. Introduce $(M_1(z),M_2(z))$. For simplicity, we  introduced the following ratios 
\be\label{ratios}
 \gamma_n :=\frac{p}{n}=\frac{1}{\rho_1+\rho_2},\quad r_1 :=\frac{n_1}{n}=\frac{\rho_1}{\rho_1+\rho_2},\quad r_2 :=\frac{n_2}{n}=\frac{\rho_2}{\rho_1+\rho_2}.
\ee
%Moreover, we also include some basic estimates on it. %{\cor(say something about the previous work)}
...
\end{split}
\ee
where we denoted  $(a_3 ,a_4):=(r_2M_{2}'(0),r_1m'_{2c}(0))$. It is not hard to see that these equations are equivalent to equation \eqref{eq_a34extra}. Then by equation \eqref{apply derivlocal} and equation \eqref{dervPi}, we get
\begin{align*}
n^2\bignorm{\Sigma_2^{1/2} ( X_1^{\top}X_1 + X_2^{\top}X_2)^{-1}\beta}^2&=\beta^\top \Sigma_2^{-1/2} V  \frac{1 + a_3+ a_4\Lambda^2 }{(a_1\Lambda^2 + a_2)^2}  V^\top  \Sigma_2^{-1/2} \beta \\
&=\beta^\top \Sigma_2^{-1/2} \frac{1 + a_3+ a_4M^\top M }{(a_1M^\top M + a_2)^2} \Sigma_2^{-1/2} \beta,
\end{align*}
...
The following lemma gives (almost) sharp concentration bounds for linear and quadratic forms of bounded supported random variables. Besides the proof of Lemma \ref{prop_entry}, we also use it in several other places of this paper, including the proofs in Section \ref{sec_proof_general} and Section \ref{app_iso_cov}.
