...
\begin{lemma} \label{LEM_SMALL} %[Results on covariance matrices with small support]
...
%uniformly for any $z \in \mathbf D$, and 
...
\begin{proof}[Proof of Theorem \ref{main_cor}]%[Proof of Corollary \ref{main_cor}]
...
\begin{align*}
&  \mathbb E \left| \mathbf 1\left( |Z^{(1)}_{\mu i}|> q n^{1/2}\log n \right)Z^{(1)}_{\mu i}\right| = \int_0^\infty \P\left( \left| \mathbf 1\left(  |Z^{(1)}_{\mu i}|> q n^{1/2}\log n \right)Z^{(1)}_{\mu i}\right| > s\right)\dd s \\
...
\begin{align*}
&  \mathbb E \left| \mathbf 1\left( |Z^{(1)}_{\mu i}|> qn^{1/2} \log n \right)Z^{(1)}_{\mu i}\right|^2 = 2\int_0^\infty s \P\left( \left| \mathbf 1\left( |Z^{(1)}_{\mu i}|> q n^{1/2}\log n \right)Z^{(1)}_{\mu i}\right| > s\right)\dd s \\
...
Now $\wh Z^{(1)}$ and $\wh Z^{(2)}$ satisfy the assumptions of Lemma \ref{LEM_SMALL} with bounded support $\sqrt{n}q = n^{ \frac{2}{\varphi}}$, so we get that %and equation \eqref{aniso_law} gives that
...
with overwhelming probability, where we also used Fact \ref{fact_minv}(ii) to bound the operator norm of $\wt Z^{(k)}$. Together with estimate \eqref{priorim} below, this bound implies that % check that
...
Now we are ready to complete the proof of Theorem \ref{thm_main_RMT} and Theorem \ref{thm_main_RMT_app} using Theorem \ref{main_cor}.
...
\begin{align}
\left| \left[\Sigma^{(2)}\hat \Sigma^{-1} - n^{-1} (\Sigma^{(2)})^{1/2}V \Gi(0)V^\top(\Sigma^{(2)})^{-1/2} \right]_{ii}\right|  & = n^{-1} \left|\mathbf e_i^\top (\Sigma^{(2)})^{1/2}V \left(\cal G(0)-\Gi(0)\right)V^\top (\Sigma^{(2)})^{-1/2} \mathbf e_i\right| \nonumber\\
...
\begin{proof}[Proof of Theorem \ref{thm_main_RMT_app}]
...
\ee
on the event \eqref{one_event} with $ q:= n^{-\frac{\varphi - 4}{2\varphi}}$. Now combining this estimate with Cauchy's integral formula, we get that %on the event \eqref{one_event}, %with probability $1-\oo(1)$, 
...
When $z=0$, the system of equations \eqref{selfomega_a} reduces to equations \eqref{eq_a12extra000} and  \eqref{eq_a12extra}, from which we can derive an equation of $a_1$ only: 
...
\be\label{a23}
\frac{n_1}{n_1+n_2} \tau \le   a_1 \le \min\left\{ 1-\frac{p}{n_1+n_2}  -\frac{n_1}{n_1+n_2}  \tau,\frac{n_1}{n_1+n_2} ( 1-\tau)\right\},\quad \tau<a_2\le 1 -\frac{p}{n_1+n_2}  - \frac{n_1}{n_1+n_2} \tau .
\ee 
...
\subsection{Beyond Multivariate Gaussian Random Matrices: an Anisotropic Local Law}\label{sec_Gauss}
...
For this case, we use the standard resolvent methods in \cite{isotropic,yang2019spiked,PY} and prove the following result.
...
\begin{proposition}\label{prop_diagonal}
 In the setting of Lemma \ref{LEM_SMALL}, assume further that the entries of $ Z^{(1)}$ and $ Z^{(2)}$ are i.i.d. Gaussian random variables. %which satisfy the bounded support condition \eqref{eq_support} with $q= n^{-1/2}$. 
...
equation \eqref{aniso_law} through interpolation matrices $Z^\theta$ between $Z^0$ and $Z^1$. We have seen that equation \eqref{aniso_law} holds for $G(Z^0,z)$ by Proposition \ref{prop_diagonal}. %as remarked at the beginning of Section \ref{sec_Gauss}.
...
Underlying the proof of the estimate (\ref{lemm_comp_4}) is an expansion approach, which is the same as the ones for Lemma 7.10 of \cite{Anisotropic} and Lemma 6.11 of \cite{yang2019spiked}. So we omit the details.
\end{proof}
...
%We first collect some preliminary results, Lemmas \ref{lemm apri}-\ref{lemm_resolvent}, that will be used in the proof. We remark that theses results work under the general setting in Lemma \ref{LEM_SMALL}, that is, we do not require $Z^{(1)}$ and $Z^{(2)}$ to be Gaussian as in Lemma \ref{prop_entry}.
...
%we write $\xi\lesssim \zeta$ if there exists a constant $C>0$ such that $\xi\le C\zeta$, and 
...
\begin{equation*}
 \cal  Z_{{a}} :=(1-\mathbb E_{{a}})\left[\big(G_{{a}{a}}\big)^{-1}\right], %\quad {a}\notin \mathbb T,
\end{equation*}
...
%Suppose $\Xi$ holds, then we have $|G_{{a}{a}} -\Pi|\lesssim (\log N)^{-1}$ on $\Xi$, and  
 Note that for any ${a}\in \cal I$, $H^{({a})}$ and $G^{({a})}$ also satisfies the assumptions in Lemma \ref{lemm apri}. Hence equations \eqref{priorim} and \eqref{priordiff} also hold for $G^{({a})}$ with overwhelming probability. Now applying equations \eqref{eq largedev1} and \eqref{eq largedev2} to equation (\ref{Zi}), %and using the a priori bound equation \eqref{priorim}, 
...
\begin{align*}
& \left|G_{ii}^{-1}G_{i{a}}\right| \le {n^{-1/2}}\left|  {\lambda_i}\sum_{\mu \in \cal I_1} Z^{(1)}_{\mu i} G^{(i)}_{\mu a}(z)\right| + n^{-1/2}\left|\sum_{\mu \in \cal I_2} Z^{(2)}_{\mu i} G^{(i)}_{\mu a}(z)  \right| \prec  n^{-1/2}\left( \sum_{\mu \in \cal I_1\cup \cal I_2}  {| G_{\mu {a}}^{(i)} |^2 }  \right)^{1/2} \prec n^{-1/2}. 
...
More precisely, we will show that $(m_1(z),m_2(z))$ satisfies equation \eqref{selfomegaerror} for some small errors satisfying $|\cal E_{1}|+ |\cal E_{2}|\prec n^{-1/2}$. Later in Step 3, we will apply Lemma \ref{lem_stabw} to show that $(m_1(z),m_2(z))$ is close to $(M_{1}(z),M_{2}(z))$. %---this will discussed .
