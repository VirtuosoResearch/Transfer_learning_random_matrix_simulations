...
\be\label{GG*}\sum_{\nu \in \cal I_1\cup \cal I_2 }  | G_{\mu\nu}^{(i)} |^2\le \sum_{{a} \in \cal I } | G_{\mu{a}}^{(i)} |^2 =\left[G^{(i)}(G^{(i)})^* \right]_{\mu\mu} =\OO(1),\quad \text{with high probability,}\ee
where $(G^{(i)})^*$ denotes the complex conjugate transpose of $G^{(a)}$. Similarly, applying bounds \eqref{eq largedev1} and \eqref{eq largedev2} to $Z_{\mu}$ and $Z_\nu$ in equation (\ref{Zmu}) and using the estimate \eqref{priorim}, we can obtain the same bound.
...
\ee
 ...
 which are the asymptotic limit of $m(z)$ and $m_0(z)$, respectively. {\cor (start here)} Plugging equation \eqref{Gsim1} into equation \eqref{defn_piw}, we get
\be
