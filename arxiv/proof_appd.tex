...
Note that $\cal G^2(0)=\partial_z \cal G|_{z=0}$. Under the moment condition \eqref{assmAhigh}, by Corollary \ref{main_cor} we have that
%$Z_1$ and $Z_2$ have bounded support $q=n^{-1/2}$. Using Theorem \ref{LEM_SMALL}, %Now by Corollary \ref{main_cor}, we obtain that for any small constant $\e>0$, %with probability $1-\oo(1)$,
\be\nonumber 
\max_{z\in \C:|z|=(\log n)^{-1}}|\mathbf v^\top (G(z)-\Pi(z))\mathbf v| \prec q \|\mathbf v\|^2,
\ee
for $q= n^{-\e/(8+2\e)}$ on an event $\Omega$ with $\P(\Omega)=1-\oo(1)$. {\cor(here)} Now combining this estimate with Cauchy's integral formula, we get that %with probability $1-\oo(1)$, 
...
\be\label{dervPi}
\bv^\top \Pi'(0)\bv = \bv \frac{1+r_1m_{2c}'(0)\Lambda^2 + r_2m_{2c}'(0)}{(r_1m_{2c}(0)\Lambda^2 + r_2m_{2c}(0))^2}\bv,
\ee
...
\begin{split}
 \frac{m'_{2c}(0) }{m_{2c}^2(0)}=   \frac1n\sum_{i=1}^p \frac{\lambda_i^2\left(1+\lambda_i^2 r_1m'_{2c}(0) + r_2 m'_{3c}(0)\right) }{  ( \lambda_i^2 r_1 m_{1c}(0) +r_2 m_{2c}(0))^2 } ,\quad \frac{m'_{3c}(0)}{m_{2c}^2(0)} &=  \frac1n\sum_{i=1}^p \frac{1+\lambda_i^2 r_1m'_{2c}(0) + r_2 m'_{3c}(0)}{ ( \lambda_i^2 r_1 m_{1c}(0) +r_2 m_{2c}(0))^2  } .
\end{split}
\ee
If we rename $(-r_1m_{2c}(0),-r_2m_{2c}(0))$ to $(a_1, a_2)$ and  $(r_2m_{2c}'(0),r_1m'_{2c}(0))$ to $(a_3 ,a_4)$, then these equations become
%\be%\label{dotm34}
%\begin{split}
...
The main difficulty in the proof of Theorem \ref{LEM_SMALL} is due to the fact that the entries of $Y_1=Z_1 U\Lambda  $ and $Y_2={Z_2V}$ are not independent. However, notice that if the entries of $Z_1\equiv Z_1^{\text{Gauss}}$ and $Z_2\equiv Z_2^{\text{Gauss}}$ are i.i.d. Gaussian, then by the rotational invariance of the multivariate Gaussian distribution, we have
...
& Z_\mu= \sum_{i,j \in \mathcal I_1}  {\lambda_i \lambda_j}G^{(\mu)}_{ij} \left(\frac{1}{n} \delta_{ij} - z_{\mu i}z_{\mu j}\right), \quad Z_\nu = \sum_{i,j \in \mathcal I_1} G^{(\nu)}_{ij} \left(\frac{1}{n} \delta_{ij} - z_{\nu i}z_{\nu j}\right).\label{Zmu} 
\end{align}
...
%\end{split}
%\end{equation}
The following lemma gives the desired large deviations estimate on $\Lambda_o$ and $Z$ variables.

\begin{lemma}\label{Z_lemma}
Suppose the assumptions of Proposition \ref{prop_diagonal} hold. 
...
\end{split}
\end{equation}
where in the last step we used \eqref{priorim} to get that for any $\mu\in \cal I_2\cup \cal I_3$,
...
\noindent{\bf Step 2: Self-consistent equations.} This is the key step of the proof for Proposition \ref{prop_entry}, where we shall derive approximate self-consistent equations satisfied by $m_2(z)$ and $m_3(z)$. More precisely, we will show that $(m_2(z),m_3(z))$ satisfies \eqref{selfomegaerror} for some small errors $|\cal E_{2}|+ |\cal E_{3}|\prec n^{-1/2}$. Then in Step 3 we will apply Lemma \ref{lem_stabw} to show that $(m_2(z),m_3(z))$ is close to $(m_{2c}(z),m_{2c}(z))$. %---this will discussed .
...
\be\label{Xiz}\Xi(z):=\left\{ |m_{2}(z)-m_{2c}(z)| + |m_{3}(z)-m_{2c}(z)| \le (\log n)^{-1/2}\right\}.\ee
Note that by \eqref{Lipomega}, we have $|m_{2c}+x_2|\lesssim (\log n)^{-1}$ and $|m_{2c}+x_3|\lesssim (\log n)^{-1}$ (recall that $x_2=-m_{2c}(0)$ and $x_3=-m_{2c}(0)$). Together with \eqref{selfomega}, \eqref{a23} and \eqref{assm32}, we obtain the following estimates 
\be\label{Gsim1}
 |m_{2c}| \sim |m_{2c}| \sim 1, \quad |z+\lambda_i^2 r_1m_{2c} + r_2 m_{2c}|\sim 1  ,\quad \left|1 + \gamma_n m_c\right|\sim  |1+\gamma_n m_{1c}|\sim 1  ,
 \ee
 uniformly in $z\in \mathbf D$, where we abbreviated
 \be\label{defn mc1c}m_c(z):=-\frac1p\sum_{i=1}^p\frac{1}{z+\lambda_i^2 r_1m_{2c} +r_2m_{2c}},\quad m_{1c}(z):=-\frac1p\sum_{i=1}^p\frac{\lambda_i^2}{z+\lambda_i^2 r_1m_{2c} +r_2m_{2c}}.\ee
...
$$\epsilon_i := Z_i + \lambda_i^2 r_1\left(m_2 - m_2^{(i)}\right) + r_2\left(m_3-m_3^{(i)}\right) ,\quad \epsilon_\mu := \begin{cases}Z_{\mu} + \gamma_n(m_1-m_1^{(\mu)}) , \ &\text{if } \mu \in \cal I_2\\ Z_{\mu} +\gamma_n(m-m^{(\mu)}) , \ &\text{if } \mu \in \cal I_3\end{cases}.$$
...
\max_{i\in \cal I_1} |\epsilon_i | +\max_{\mu \in \cal I_2\cup \cal I_3} |\epsilon_\mu|  \prec n^{-1/2}.
\end{equation}
...
With \eqref{Gsim1} and the definition of $\Xi$, we get that $\mathbf 1(\Xi)|z + \lambda_i^2 r_1m_2+r_2m_3|\sim1$. Hence using (\ref{self_Gii}) and \eqref{erri}, we obtain that
\be\label{Gmumu0}
...
\ee
Now combining \eqref{self_Gmu1}, \eqref{self_Gmu2}, \eqref{erri} and \eqref{Gsim2}, we obtain that for $\mu \in \cal I_2$ and $\nu \in \cal I_3,$ %can obtain that with high probability,
\begin{align}\label{Gii0} 
&\mathbf 1(\Xi)\left(G_{\mu\mu}+\frac{1}{1 + \gamma_nm_1}\right) =\OO_\prec\left(n^{-1/2}\right) , \quad \mathbf 1(\Xi)\left(G_{\nu\nu}+\frac{1}{1 + \gamma_nm}\right)= \OO_\prec\left(n^{-1/2}\right) .
...
\noindent{\bf Step 3: $\Xi$ holds with high probability.} In this step, we show that the event $\Xi(z)$ in fact holds with high probability for all $z\in \mathbf D$. Once we have proved this fact, applying Lemma \ref{lem_stabw} to \eqref{selfcons_lemm} immediately shows that $(m_2(z),m_3(z))$ is close to $(m_{2c}(z),m_{2c}(z))$ up to an error of order $\OO_\prec(n^{-1/2})$. 

We claim that it suffices to show
\be\label{Xiz0}
|m_{2}(0)-m_{2c}(0)| + |m_{3}(0)-m_{2c}(0)| \prec n^{-1/2}.
\ee
Note that by \eqref{Lipomega} and \eqref{priordiff} we get 
$$ |m_{2 c}(z)-m_{2 c}(0)|+|m_{3 c}(z)-m_{3 c}(0)|=\OO((\log n)^{-1}),\quad  |m_{2}(z)-m_{2}(0)|+ |m_{3}(z)-m_{3}(0)|=\OO((\log n)^{-1})$$ 
with high probability for all $z\in \mathbf D$. Thus if \eqref{Xiz0} holds, we can obtain that 
\be\label{roughh1} 
\sup_{z\in \mathbf D} \left(|m_{2}(z)-m_{2c}(z)| + |m_{3}(z)-m_{2c}(z)|\right)  \lesssim (\log n)^{-1} \quad \text{with high probability},\ee
and %Moreover, with this estimate and \eqref{Lipomega}, we conclude that 
\be\label{roughh2} \sup_{z\in \mathbf D} \left( |m_{2}(z)-m_{2 c}(0)|+ |m_{3}(z)-m_{2c}(0)|\right) \lesssim (\log n)^{-1} \quad \text{with high probability}.\ee
...
\be\label{Xizz2} \max_{\mu\in \cal I_2} |G_{\mu\mu}-m_{2c}|+ \max_{\nu\in \cal I_3} |G_{\nu\nu}-m_{2c}|\prec n^{-1/2}.
\ee
...
$$ m(0)=\frac1p\sum_{i\in \cal I_1}G_{ii}(0) = \frac1p\sum_{i=\in \cal I_1}\sum_{k = 1}^{p} \frac{|\xi_k(i)|^2 }{\lambda_k} \ge \lambda_1^{-1} \gtrsim 1. $$
Similarly, we can also get that $m_1(0)$ is positive and has size $m_1(0)\sim 1$. Hence we have the estimates
$$1+\gamma_n m(0)\sim 1,\quad 1+\gamma_n m_1(0)\sim 1.$$ 
...
Denoting $y_{2}=-m_{2}(0)$ and $y_{3}=-m_{2}(0)$, by \eqref{Gii} we have
$$y_2= \frac{1}{1+\gamma_n m_{1}(0)} +\OO_\prec(n^{-1/2}),\quad y_3= \frac{1}{1+\gamma_n m(0)}+\OO_\prec(n^{-1/2}).$$ 
Hence there exists a constant $c>0$ such that 
\be\label{omega12} c \le y_2 \le 1, \quad  c\le y_3\le 1, \quad \text{with high probability}.\ee
...
Also one can verify from \eqref{selfcons_lemma222} that $(y_2,y_3)$ satisfies approximately the same system of equations as \eqref{selfomega0}:
\be\label{selfcons_lemm000}
\begin{split}
r_1y_2+r_2 y_3 = 1-\gamma_n + \OO_\prec (n^{-1/2}),\quad f(y_2)=1 + \OO_\prec (n^{-1/2}).
\end{split}
\ee
The first equation and \eqref{omega12} imply that $y_2 \in [0,r_1^{-1}(1-\gamma_n)]$ with high probability. Since $f$ is strictly increasing and has bounded derivative on $[0,r_1^{-1}(1-\gamma_n)]$, by basic calculus the second equation in \eqref{selfcons_lemm000} gives that $|y_2-x_2|\prec n^{-1/2}$. Plugging it into the first equation of \eqref{selfcons_lemm000}, we get $|y_3-x_3|\prec n^{-1/2}$. This concludes \eqref{Xiz0}.
\end{proof}
...
Proposition \ref{prop_diagonal} implies that \eqref{aniso_law} holds for Gaussian $Z_1^{\text{Gauss}}$ and $Z_2^{\text{Gauss}}$ as discussed at the beginning of Section \ref{sec_Gauss}. Thus the basic idea is to prove that for $Z_1$ and $Z_2$ satisfying the assumptions in Theorem \ref{LEM_SMALL}, %we have
...
%\end{lemma}
We shall apply \eqref{basic_interp} to $F(Z):=F_{\bu\mathbf v}^s(Z,z)$ for any fixed $s\in 2\N$ and %$F_{\mathbf u\mathbf v}(Z,z)$ defined as
\begin{equation*}%\label{eq_comp_F(X)}
...
%\begin{lemma}\label{lemm_comp_4}
 ...
 \begin{equation}\label{lemm_comp_4}
  \sum_{i\in\mathcal I_1}\sum_{\mu\in\mathcal I_2\cup \cal I_3}\left[\mathbb EF_{\bu\mathbf v}^s\left(Z^{\theta,Z_{\mu i}^1}_{(\mu i)},z\right)-\mathbb EF_{\bu\mathbf v}^s\left(Z^{\theta,Z_{\mu i}^0}_{(\mu i)},z\right)\right]\le (n^\e q)^{s}+C\E F_{\bu\mathbf v}^s\left(Z^{\theta},z\right) 
 \end{equation}
 ...
 $$\E\left|\bu^\top \left(G(Z^1,z)-\Pi(z)\right)\bv\right|^p  \le (n^\e q)^{p}.$$
