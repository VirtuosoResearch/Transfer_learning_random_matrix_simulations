...
%\begin{align*}
%	 \hat{B}(W_1, W_2) &= (W_1^2 (X^{(1)})^{\top}X^{(1)} + W_2^2 (X^{(2)})^{\top}X^{(2)})^{-1} (W_1 (X^{(1)})^{\top}Y_1 + W_2 (X^{(2)})^{\top}Y_2)\\
...
%$$\hat v\Sigma_1^{1/2} (\hat{v}^2 (X^{(1)})^{\top}X^{(1)} + (X^{(2)})^{\top}X^{(2)})^{-1} \Sigma_2 (\hat{v}^2 (X^{(1)})^{\top}X^{(1)} + (X^{(2)})^{\top}X^{(2)})^{-1}\Sigma_1^{1/2}.$$
...
\item[(ii)] Using the approximate isometry property of $Z^{(1)}$ in \eqref{eq_isometric}, we will approximate the bias term $ \delta_{\bias}(v)$ by
...
%	Let $\alpha = \bignorm{\Sigma_2^{1/2} K \Sigma_1 (\beta_1 - \hat{w}\beta_2)}^2$.
	%We have
	\begin{align}
%		& \bignorm{\Sigma_2^{1/2}((X^{(1)})^{\top}X^{(1)} + (X^{(2)})^{\top}X^{(2)})^{-1}(X^{(1)})^{\top}X^{(1)}(\beta_1 - \hat{w}\beta_2)}^2 \nonumber \\
...
\paragraph{Step IV: An $\e$-net argument.} Finally, it remains to extend the above result \eqref{dicho_varbeta} to $v=\hat v$, which is random and depends on $X^{(1)}$ and $X^{(2)}$. We first show that for any fixed constant $C_0>0$, there exists a high probability event $\Xi$ on which \eqref{dicho_varbeta}
...
\end{equation}
Finally, we assume that $Z^{(1)}$ and $Z^{(2)}$ satisfy
\begin{equation}
...
\end{equation}
for some deterministic parameter $q$ satisfying $ n^{-{1}/{2}} \leq q \leq n^{- \phi} $ for a small constant $\phi>0$. Here we recall the notations in \eqref{labelZ}.
...
\begin{remark}
Whenever \eqref{eq_support} holds, we say $n^{-1/2}Z^{(1)}$ and $n^{-1/2}Z^{(1)}$ satisfy the {\it{bounded support condition}} with $q$, or $n^{-1/2}Z^{(1)}$ and $n^{-1/2}Z^{(1)}$ have support $q$. 
...
By Example \ref{example stoc}, we see that if the entries of $Z^{(1)}$ have finite moments up to any order, %as in equation \eqref{assmAhigh}, 
then $n^{-1/2}Z^{(1)}$ has bounded support $n^{-1/2}$. More generally, if the entries of $Z^{(1)}$ have finite $a$-th moment for some constant $a>4$, then using Markov's inequality and a simple union bound we get that
$$\P\left(n^{-1/2}\max_{i,j}|(Z^{(1)})_{ij}|\ge (\log n) n^{2/a-1/2}\right)=\OO((\log n)^{-a}).$$
In other words, $n^{-1/2}Z^{(1)}$ has bounded support $q=n^{2/a-1/2}$ on a event with probability $1-\oo(1)$. This fact will be used in the proof of Corollary \ref{main_cor} below. The above discussion shows that the bounded support condition provides great flexibility in dealing with finite moment conditions. 
...
Before entering into the formal proof, we state two preparation lemmas, both of which have been used crucially in some previous proofs. First, for $n^{-1/2}Z^{(1)}$ and $n^{-1/2}Z^{(2)}$ with bounded support $q$, we have the following estimates on their singular values. We have used it in our previous proofs; see equation \eqref{eq_isometric}.   
...
Suppose Assumption \ref{assm_big1} holds, and $n^{-1/2}Z^{(1)}$ and $n^{-1/2}Z^{(2)}$ satisfy the bounded support condition equation \eqref{eq_support} for some deterministic parameter $q$ satisfying $ n^{-{1}/{2}} \leq q \leq n^{- \phi} $ for a constant $\phi>0$. Then for any constant $\e>0$, we have that with high probability,
\be\label{op rough1} %(1-\sqrt{d_1})^2 - \e \le  \lambda_p((Z^{(1)})^{\top} Z^{(1)})   \le  
\lambda_1((Z^{(1)})^{\top} Z^{(1)}) \le {(\sqrt{n_1}+\sqrt{p})^2} + n_1^{1+\e} q,
\ee
and
\be\label{op rough2} 
 (\sqrt{n_2}-\sqrt{p})^2  -  n_2^{1+\e} q \le  \lambda_p ((Z^{(2)})^\top Z^{(2)})  \le  \lambda_1((Z^{(2)})^\top Z^{(2)}) \le  (\sqrt{n_2}+\sqrt{p})^2 +  n_2^{1+\e} q .
\ee
...
\end{proof}
\begin{remark}
it is well-known that the eigenvalues of $n_2^{-1}(Z^{(2)})^\top Z^{(2)}$ are all inside the support of the Marchenko-Pastur law $[(1-\sqrt{p/n_2})^2-\oo(1) ,(1+\sqrt{p/n_2})^2+\oo(1)]$ with probability $1-\oo(1)$ \cite{No_outside}. Here the estimate \eqref{op rough2} have improved the error to $n_2^\e q$ and the probability to $1-\OO(n^{-D})$.
\end{remark}
...
$$\cal I_0:=\llbracket 1,p\rrbracket, \quad  \cal I_1:=\llbracket p+1,p+n_1\rrbracket, \quad \cal I_2:=\llbracket p+n_1+1,p+n_1+n_2\rrbracket ,\quad \cal I:=\cal I_0\cup \cal I_1\cup \cal I_2  .$$
 We will consistently use the latin letters $i,j\in\sI_{0}$, greek letters $\mu,\nu\in\sI_{1}\cup \sI_{2}$, and $\fa,\fb,\mathfrak c\in \cal I$. Correspondingly, the indices of the matrices $Z^{(1)}$ and $Z^{(2)}$ are labelled as 
 \be\label{labelZ}
...
%In this subsection, 
We now state the main random matrix result---Theorem \ref{LEM_SMALL}---which gives an almost optimal estimate on the resolvent $G$. It is conventionally called the {\it anisotropic local law} \cite{Anisotropic}. We define a domain of the spectral parameter $z$ as
\begin{equation}
...
\end{equation}
where recall that $\Gi$ was defined in \eqref{defn_piw}.
...
%\end{itemize}
\end{theorem}
Using a simple cutoff argument, it is easy to obtain from Theorem \ref{LEM_SMALL} the following corollary under certain moment assumptions. 

\begin{corollary}\label{main_cor}
Suppose Assumption \ref{assm_big1} holds. Moreover, assume that the entries of $Z^{(1)}$ and $Z^{(2)}$ are i.i.d. random variables satisfying equation \eqref{assm1} and  
...
Fix any sufficiently small constant $\e>0$. We choose $q= n^{-c_a +\e}$ with $c_a:=1/2-2/a $. Then we introduce the truncated matrices $\wt Z^{(1)}$ and $\wt Z^{(2)}$, with entries
...
\mathbb P(\wt Z^{(1)} = Z^{(1)},  \wt Z^{(2)} = Z^{(2)}) =1-\OO ( n^{-a\e}).
\end{equation}
...
Now $\wh Z^{(1)}$ and $\wh Z^{(2)}$ satisfy the assumptions of Theorem \ref{LEM_SMALL} with $q= n^{-c_a +\e}$, and equation \eqref{aniso_law} gives that
$$\left| \mathbf u^\top (G(\wh Z^{(1)},\wh Z^{(2)},z)-\Pi(z)) \mathbf v \right|  \prec  q.$$
Then using equation \eqref{meanshif}, equation \eqref{EZ norm} and equation \eqref{priorim} below, it is easy to check that  
$$\left| \mathbf u^\top (G(\wh Z^{(1)},\wh Z^{(2)},z)-G(\wt Z^{(1)},\wt Z^{(2)},z)) \mathbf v \right|  \prec  n^{-1-\e},$$
 This shows that equation \eqref{aniso_law} also holds for $G(\wt Z^{(1)},\wt Z^{(2)},z)$ with $q= n^{-c_a +\e}$, which concludes the proof by equation \eqref{XneX} and the fact that $\e$ can be chosen arbitrarily small.
\end{proof}
...
If the entries of $\sqrt{n}Z^{(1)}$ and $\sqrt{n}Z^{(2)}$ have finite $(4+\e)$-th moments as in equation \eqref{assmAhigh}, then by Corollary \ref{main_cor}  we have that
...
\be\label{G0Pi0}\max_{1\le i \le p}| (A((X^{(1)})^\top X^{(1)}+(X^{(2)})^\top X^{(2)})^{-1} - n^{-1}A \Sigma_2^{-1/2}V \Pi(0)V^\top\Sigma_2^{-1/2})_{ii}| \prec q\|A\| ,\ee
...
$$ \tr (A((X^{(1)})^\top X^{(1)}+(X^{(2)})^\top X^{(2)})^{-1}) = n^{-1}\tr \left[ A(a_1  \Sigma_1  +  a_2\Sigma_2)^{-1}\right] +\OO_\prec(q \|A\|) \quad \text{on $\Omega$}.$$
...
\begin{align*}
 n^2\bignorm{\Sigma_2^{1/2} ( (X^{(1)})^{\top}X^{(1)} + (X^{(2)})^{\top}X^{(2)})^{-1}\beta}^2 =\beta^\top \Sigma_2^{-1/2}  \left(M^\top (Z^{(1)})^{\top} Z^{(1)} M +  (Z^{(2)})^\top Z^{(2)} \right)^{-2}   \Sigma_2^{-1/2}\beta,
\end{align*}
...
{\cor Introduce $(M_1(z),M_2(z))$. First we define the deterministic limits of $(m_1(z), m_{2}(z))$, denoted by $(M_{1}(z),M_{2}(z))$ 
%\HZ{This $M_{1}, M_{2}$ notation is awkward; consider changing it to ${M}_1(z), {M}_2(z)$ (or  better, say $a_1(z), a_2(z)$? if they correspond to $a_1, a_2$ when $z=0$)}, 
as the unique solution to the following system of equations
\begin{equation}\label{selfomega}
\begin{split}
...
%We divide the proof of Theorem \ref{LEM_SMALL} into two steps. We first prove Theorem \ref{LEM_SMALL} in the special case where $X$ is Gaussian. Then we use a self-consistent comparison arguments developed in \cite{Anisotropic} to prove Theorem \ref{LEM_SMALL} in the general case. 
 ...
 In this section, we give the proof of Theorem \ref{LEM_SMALL}. The main difficulty in the proof is due to the fact that the entries of $Y_1=Z^{(1)} U\Lambda  $ and $Y_2={Z^{(2)}V}$ are not independent. However, notice that if the entries of $Z^{(1)}\equiv (Z^{(1)})^{\text{Gauss}}$ and $Z^{(2)}\equiv (Z^{(2)})^{\text{Gauss}}$ are i.i.d. Gaussian, then by the rotational invariance of the multivariate Gaussian distribution, we have
...
Suppose Assumption \ref{assm_big1} holds, and $Z^{(1)}$ and $Z^{(2)}$ satisfy the bounded support condition equation \eqref{eq_support} with $q= n^{-1/2}$. Suppose $U$ and $V$ are identity. Then the estimate equation \eqref{aniso_law} holds for $G_0(z)$.
...
......... In this subsection, we describe the proof of Theorem \ref{LEM_SMALL} for general $Z^{(1)}$ and $Z^{(2)}$ satisfying the bounded support condition (\ref{eq_support}) with $q\le n^{-\phi}$ for some constant $\phi>0$. 
%As remarked at the beginning of Section \ref{sec_Gauss},
Proposition \ref{prop_diagonal} implies that equation \eqref{aniso_law} holds for Gaussian $(Z^{(1)})^{\text{Gauss}}$ and $(Z^{(2)})^{\text{Gauss}}$ as discussed above. Thus it suffices is to prove that for $Z^{(1)}$ and $Z^{(2)}$ satisfying the assumptions in Theorem \ref{LEM_SMALL}, we have
\begin{equation*}%\label{Gaussian_starting}
 \mathbf u^\top  \left( G(Z,z) -  G(Z^{\text{Gauss}}, z)\right) \mathbf v  \prec q 
\end{equation*}
for any deterministic unit vectors $\mathbf u,\mathbf v\in{\mathbb R}^{p+n}$ and $z\in \mathbf D$, where we abbreviated that $Z:=\begin{pmatrix}Z^{(1)}, Z^{(2)}\end{pmatrix}$ and $Z^{\text{Gauss}}:=\begin{pmatrix}Z^{\text{Gauss}}_1, Z^{\text{Gauss}}_2\end{pmatrix}$.
...
%\be\label{lambdap} 
$\mu_p \ge \lambda_p((Z^{(2)})^\top Z^{(2)}) \ge c_\tau $ with high probability for some constant $c_\tau>0$ depending on $\tau$. This further implies the estimate that
...
\be\label{diagW}W= (\Lambda (Z^{(1)})^{\top}, (Z^{(2)})^\top).\ee
