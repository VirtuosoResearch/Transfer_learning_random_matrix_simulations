...
\ee
Recall that $M=\Sig_1^{1/2} \Sig_2^{-1/2}$ has a singular value decomposition as in equation \eqref{eigen2}. We assume that % the singular values satisfy that
...
\begin{remark}\label{rem_athmoment}
...
\be\label{Ptrunc}
\P\left(n^{-1/2}\max_{i,\mu}|(Z^{(1)})_{\mu i}|\ge (\log n) n^{2/a-1/2}\right)=\OO((\log n)^{-a}).\ee
...
%We have used it in our previous proofs; see equation \eqref{eq_isometric}.   
...
%In this subsection, 
We now state the main random matrix result---Theorem \ref{LEM_SMALL}---which gives an almost optimal estimate on the resolvent $G(z)$ defined in equation \eqref{eqn_defG}. It is conventionally called the {\it anisotropic local law} \cite{Anisotropic}. We define a domain of the spectral parameter $z$ as
\begin{equation}
...
Assume that the entries of $Z^{(1)}$ and $Z^{(2)}$ are i.i.d. random variables of zero mean, unit variance, and finite $a$-th moment
...
\ee 
for a constant $a>4$. Assume that conditions \eqref{assm2} and \eqref{assm32} hold. Then equation \eqref{aniso_law} holds for $q= n^{2/a-1/2}$ on an event $\Omega$ with $\P(\Omega)=1-\oo(1)$.
\end{corollary}
...
&\lesssim \int_0^{qn^{1/2}\log n}\left(q  n^{1/2}\log n \right)^{-a}\dd s +\int_{qn^{1/2}\log n}^\infty s^{-a}\dd s \le n^{-2(a-1)/a}.
\end{align*}
where in the third step we used condition \eqref{condition_4e} and Markov's inequality. Similarly, we can obtain that 
\begin{align*}
...
&\lesssim \int_0^{qn^{1/2}\log n}s\left(q  n^{1/2}\log n \right)^{-a}\dd s +\int_{qn^{1/2}\log n}^\infty s^{-a+1}\dd s \le n^{-2(a-2)/a}.
\end{align*}
Plugging the above two estimates into equation \eqref{EwtZ} and using $a>4$, we get that
...
\be\label{GwhZZ}\left| \mathbf u^\top (G(\wh Z^{(1)},\wh Z^{(2)},z)-\Pi(z)) \mathbf v \right|  \prec  q,\ee
where $G(\wh Z^{(1)},\wh Z^{(2)},z)$ is defined in the same way as $G(z)\equiv G(Z^{(1)}, Z^{(2)},z)$ in \eqref{eqn_defG}, but with $(Z^{(1)}, Z^{(2)})$ replaced by $(\wh Z^{(1)},\wh Z^{(2)})$.
...
with high probability, where we also used Lemma \ref{SxxSyy} to bound the operator norm of $\wt Z^{(k)}$. Together with equation \eqref{priorim} below, this bound implies that % check that  
...
with high probability on the event $\{\wt Z^{(1)} = Z^{(1)},  \wt Z^{(2)} = Z^{(2)}\}$. Combining this estimate with equation \eqref{GwhZZ}, we obtain that estimate \eqref{aniso_law} also holds for $G(z)$ on the event $\{\wt Z^{(1)} = Z^{(1)},  \wt Z^{(2)} = Z^{(2)}\}$, which concludes the proof by equation \eqref{XneX}.
\end{proof}
...
Now we are ready to complete the proof of Lemma \ref{lem_cov_shift} and Lemma \ref{lem_cov_derivative} using Corollary \ref{main_cor}. 
...
and that the entries of $Z^{(1)}$ and $Z^{(2)}$ have finite $(4+\e)$-th moments as in equation \eqref{assmAhigh}. 
...
\begin{align}\label{G0Pi0}
& \left| \left[\left((X^{(1)})^\top X^{(1)}+(X^{(2)})^\top X^{(2)}\right)^{-1}\Sigma - n^{-1}  \Sigma_2^{-1/2}V \Pi(0)V^\top\Sigma_2^{-1/2}\Sigma\right]_{ii}\right| \\
&=n^{-1} \left| \left[ \Sigma_2^{-1/2}V \left(\cal G(0)-\Pi(0)\right)V^\top\Sigma_2^{-1/2}\Sigma \right]_{ii}\right| 
\prec q\|A\| ,
\end{align}
for $q= n^{-\e/(8+2\e)}$ on an event $\Omega$ with $\P(\Omega)=1-\oo(1)$. Here $\mathbf e_i$ denotes the standard basis vector along the $i$-th direction. By equation \eqref{defn_piw}, we have
...
%\end{equation}
%%Fix a constant $\xi>0$. Suppose $q\le \varphi^{-5\xi}$ and that there exists $\wt  S\subseteq S(c_0,C_0,L)$ with $L\ge 18\xi$ such that with $\xi$-high probability,
%%\begin{equation} 
