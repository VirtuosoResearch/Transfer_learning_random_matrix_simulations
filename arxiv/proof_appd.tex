...
Note that if the entries of $\sqrt{n}Z$ have finite moments up to any order as in \eqref{assmAhigh}, then using Markov's inequality one can show that $Z$ has bounded support $n^{-1/2}$. More generally, if the entries of $\sqrt{n}Z$ have finite $a$-th moment for some $a>4$, then by Markov's inequality and a simple union bound, it is easy to see that $Z$ has bounded support $q=n^{2/a-1/2}$ on a event with probability $1-\oo(1)$. Hence the bounded support condition allows us to relax the moment conditions using a simple cut-off argument. 
