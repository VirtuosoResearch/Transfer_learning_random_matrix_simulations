...
\begin{corollary}[Derived from Theorem \ref{thm_many_tasks}]\label{prop_same_cov}
	In the setting described above, the total prediction loss of the $t$ tasks satisfies that
	\begin{align*} 
...
\begin{corollary}[Derived from Theorem \ref{thm_main_RMT}]\label{cor_MTL_loss}
	In the setting described above, assume that there exists a small constant $c_0>0$ such that the signal-to-noise ratios satisfy that
	...
	%-\left[1- \left( 1-\frac{1}{\sqrt{\rho_1}}\right)^4\right] pd^2\cdot \frac{\rho_1^2 (\rho_1+\rho_2)}{(\rho_1 + \rho_2 - 1)^3} +\OO(p^{-c}\sigma^2)  \le 
	 & \left|L(\hat{\beta}_2^{\MTL}) -\frac{\sigma^2}{\rho_1+\rho_2-1} - pd^2\cdot \frac{\rho_1^2 (\rho_1+\rho_2)}{(\rho_1 + \rho_2 - 1)^3}\right|  \le  \left[\left( 1+\frac{1}{\sqrt{\rho_1}}\right)^4-1\right] pd^2\cdot \frac{\rho_1^2 (\rho_1+\rho_2)}{(\rho_1 + \rho_2 - 1)^3} +\OO(p^{-c_{\varphi}}\sigma^2 ).
	 \end{align}
\end{corollary}
