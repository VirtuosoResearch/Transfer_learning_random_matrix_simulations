...
\begin{align}
	G_{\mu,\mu}^{-1}(z) &=  - 1 - \frac{1}{n} \sum_{i,j\in \mathcal I_0}\lambda_i \lambda_j Z^{(1)}_{\mu i}Z^{(1)}_{\mu j} G^{\left(\mu\right)}_{ij}, \\
	{G_{\nu,\nu}^{-1}(z) &=  - 1 - \frac{1}{n} \sum_{i,j\in \mathcal I_0}  Z^{(2)}_{\nu i}Z^{(2)}_{\nu j}  G^{\left(\nu\right)}_{ij}.
\label{0self_Gmu1}
\end{align}
To see why  holds, using Schur complement formula,
$G_{ii}^{-1}(z) = -z - H_i G^{(i)}(z) H_{i}^\top$, where $H_i$ is the $i$-th row of $H$ with the $(i,i)$-th entry removed.
Equation \eqref{0self_Gii} follows by expanding this formula.

These two expressions are derived similarly using the Schur complement formula.
...
Compare \eqref{approximate m1m2} to the deterministic self-consistent equations in equation \eqref{selfomega_a}, one can observe that we should have
...
Combining equation \eqref{1self_Gii} and equation \eqref{3self_Gmu},

For the right hand side of equation \eqref{0self_Gii}, recall that the resolvent minor $G^{(i)}$ is defined such that it is independent of the $i$-th row and column of $Z^{(1)}$ and $Z^{(2)}$.
Hence by standard concentration inequalities (cf. Lemma \ref{largedeviation}), we have that the right hand side of equation \eqref{0self_Gii} is approximately equal to the expectation over $\{Z^{(1)}_{\mu i}: \mu \in \cal I_1 \}\cup \{Z^{(2)}_{\nu i}: \nu \in \cal I_2\}$.
More precisely, we have that with high probability the following holds
\begin{align*}
	\frac{1}{{G_{ii} }} &=  - z - \frac{\lambda_i^2}{n} \sum_{\mu \in \mathcal I_1}  G^{\left( i \right)}_{\mu\mu} - \frac{1}{n} \sum_{\mu\in \mathcal I_2} G^{\left( i \right)}_{\mu\mu} +\oo(1) \\
	&= - z - \lambda_i^2 \frac{\rho_1}{\rho_1+\rho_2} m_1^{(i)}(z)-  \frac{\rho_2}{\rho_1+\rho_2} m_2^{(i)}(z)+\oo(1),
\end{align*}
where we used the definition of $m_1^{(i)}(z)$ and $m_2^{(i)}(z)$ in equation \eqref{defm} with $G(z)$ replaced by $G^{(i)}(z)$.
...
\begin{align}\label{1self_Gii}
	G_{ii}(z)  = -\left( z +\lambda_i^2 \frac{\rho_1}{\rho_1+\rho_2} m_1(z) +  \frac{\rho_2}{\rho_1+\rho_2} m_2(z) + \oo(1)\right)^{-1}.
\end{align}
Similarly, we can obtain from equation \eqref{0self_Gmu1} that for $\mu \in \cal I_1$ and $\nu\in \cal I_2$, with high probability the following holds
\be\label{1self_Gmu} G_{\mu \mu }=-\left(1+\frac{p}{n} m_0 + \oo(1)\right)^{-1},\quad G_{\nu\nu}=-\left(1+\frac{p}{n} m+\oo(1)\right)^{-1},\ee
The above inequalities are stated more precisely in the proof of Lemma \ref{lemm_selfcons_weak} (cf.  equations \eqref{self_Gii}-\eqref{erri}).
By averaging equation \eqref{1self_Gmu} over $\mu \in \cal I_1$ and $\nu \in \cal I_2$, we obtain that with high probability
\be\label{2self_Gmu}
	m_1(z) = \frac{1}{n_1}\sum_{\mu \in \cal I_1}G_{\mu\mu}=-\left(1+\frac{p}{n} m_0 + \oo(1)\right)^{-1},
	\quad m_2(z) =\frac{1}{n_2}\sum_{\nu \in \cal I_2}G_{\nu\nu}=-\left(1+\frac{p}{n} m+\oo(1)\right)^{-1}.
\ee
Combining equation \eqref{1self_Gmu} and \eqref{2self_Gmu}, we obtain that for $\mu \in \cal I_1$ and $\nu\in \cal I_2$, with high probability
\be\label{2.5self_Gmu}
	G_{\mu \mu }=m_1 +\oo(1),\quad G_{\nu\nu}=m_2+\oo(1).
\ee
Together with the definition of $m$ and $m_0$ in equation \eqref{defm}, the two equations in equation \eqref{2self_Gmu} give that with high probability
\be\label{3self_Gmu}
	\frac1{m_1}= -1- \frac{1}{n} \sum_{i=1}^p \lambda_i^2 G_{ii}+ \oo(1),\quad \frac1{m_2}=-1-\frac{1}{n} \sum_{i=1}^p G_{ii}  + \oo(1).
\ee
