...
\begin{theorem}\label{thm_main_RMT}
	Suppose that $X^{(1)}$ and $X^{(2)}$ are both random matrices that satisfy Assumption \ref{assume_rm}.
...
\begin{corollary}\label{cor_MTL_loss}
	...
	(i) task two's sample size is at least $n_2 \ge \frac{3}{2}p$;
...
We show that as a function of $n_1$, the limit equation always decreases first from $\frac{\sigma^2 p}{n_2 - p}$ with $n_1$ being zero, and then increases to $d^2$ with $n_1$ going to infinity.
We describe an informal proof.
...
Moreover, near the point where $n_1$ is zero, one can verify that bias increases less than the decrease in variance, and there is only one positive root for the derivative being zero.
...
We omit the details.
Thus, we conclude that if $n_1 \ge n_2$, then $f(\lambda) > f(1)$; if $n_1< n_2$, then $f(\lambda)< f(1)$.
\end{example}
...
As a remark, by combining the bias and variance asymptotics, we can also obtain a generalization bound for any local minimum solutions $(A, B)$.
...
For the rest of this section, we present the main ideas for proving Theorem \ref{thm_main_RMT}.
