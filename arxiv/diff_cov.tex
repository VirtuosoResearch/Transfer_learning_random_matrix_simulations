...
The matrix $\hat{\Sigma}$ adds up both tasks' sample covariance matrices, and the expectation of $\hat{\Sigma}$ is equal to a mixture of their population covariance matrices, with mixing proportions determined by their sample sizes.
 ...
 To derive the bias and variance equation, we consider the expected loss conditional on the covariates as follows (the empirical loss is actually close to this expectation as will be shown in equation \eqref{claim_largedev2}):
...
\end{align}
Equations \eqref{eq_bias_2task} and \eqref{eq_variance_2task} correspond to the bias and variance of hard parameter sharing for two tasks, respectively.
...
\begin{theorem}\label{thm_main_RMT}
	...
	the variance equation \eqref{eq_variance_2task} satisfies the following estimate with high probability:
			\begin{align}
				...
				\le & p^{-c_{\varphi}}, \label{lem_cov_shift_eq} %\norm{\Sigma^{(2)}},
			\end{align}
			...
			\end{align}
\end{theorem}
The bias limit can be found in equation \eqref{lem_cov_derv_eq}.
Our result extends Fact \ref{fact_tr} to the inverse of the sum of two sample covariance matrices.
To see this, when $n_1$ is zero, we solve equations \eqref{eq_a12extra000} and  \eqref{eq_a12extra} to obtain that $a_1 = 0$ and $a_2 = (n_2-p) / n_2$, and apply these solutions to equation \eqref{lem_cov_shift_eq}.
We also remark that for general $A_1$ and $A_2$, we need to replace $\hat \Sigma$ with $\frac{A_1^2}{A_2^2}{X^{(1)}}^{\top} X^{(1)} + {X^{(2)}}^{\top} X^{(2)}$.
...
One can see that both the variance limit and the bias limit (in equation \eqref{lem_cov_derv_eq}) depend intricately on both tasks' samples sizes and covariate shift.
...
\begin{example}[Sample size ratio]\label{ex_sample_ratio}
	...
	Applying Theorem \ref{thm_main_RMT} to the above setting, we first solve the self-consistent equations \eqref{eq_a12extra000} and \eqref{eq_a12extra} using $\lambda_i = 1$ for all $1\le i\le p$.
	This gives us the following variance limit by equation \eqref{eq_a12extra000}: 
	\begin{align*}
	...
	Similarly, we can calculate the bias limit using equation \eqref{lem_cov_derv_eq}. 
...
%	(iii) the sample sizes $n_1$ and $n_2$ are greater than $(1 + \tau) p$.
	...
	The prediction loss of hard parameter sharing for task two satisfies that with high probability,
	\begin{align}
...
As a remark, in equation \eqref{cor_MTL_error}, the predication loss $L(\hat{\beta}_2^{\MTL}) $ was obtained using the global minimizer $(\hat A,\hat B)$. By combining the bias and variance limits, we can also obtain a generalization bound for any local minimizer of $f(A, B)$ in a similar way.
...
The variance term always reduces as $n_1$ increases, while the bias term always increases as $n_1$ increases (which can be verified by calculating the derivative of the bias term).
...
(ii) When $\frac{d^2}{\sigma^2} > \frac{p}{2n_2 - 3p}$, the prediction loss always decreases first from $\frac{\sigma^2 p}{n_2 - p}$ (when $n_1 = 0$), and then increases to $d^2$ (when $n_1 \rightarrow \infty$).
	To see this, near the point where $n_1$ is zero, one can verify that the bias increases slower than the variance decreases, and there is only one critical point where the derivative is zero, which corresponds to the \textit{optimal sample size ratio}.
%\end{enumerate}
...
Our second example focuses on how varying covariate shifts impacts the \textit{variance} limit in equation \eqref{lem_cov_shift_eq}, which gives that
...
With equation \eqref{eq_a12extra000} and a straightforward calculation (details omitted), we can verify that
%\begin{align*}
...
\end{align*}
where $g(\lambda, a_1) \ge 0$ is a fixed function. 
...
%\end{align*}
This implies $n_1>n_2$. Similarly, if $a_1<a_2$, equations  \eqref{eq_a12extra000} and  \eqref{eq_a12extra} give that $a_1 < \frac{n_1 + n_2-p}{2 (n_1 + n_2)}$ and $n_1<n_2$. 
...
For any probability measure $\mu$ supported on $[0,\infty)$ and complex number $ z\in \C\setminus [0,\infty)$, the Stieltjes transform of $\mu$ is a complex function defined as
...
\begin{figure*}[!t]
	...
	\textbf{Middle.} Validating Example \ref{ex_sample_ratio} for two tasks: we discover several interesting phenomena by fixing task two's sample size and increasing task one's sample size.
...
We will prove the convergence of $W$'s resolvent using the so-called ``local law'' with a sharp convergence rate \cite{isotropic,erdos2017dynamical,Anisotropic}. The full details will be given in Section \ref{appendix RMT}.
