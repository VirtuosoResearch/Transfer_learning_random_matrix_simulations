...
Suppose that $X^{(1)} = Z^{(1)}{\Sigma^{(1)}}^{1/2} \in \real^{n_1 \times p}$ and $X^{(2)} = Z^{(2)}{\Sigma^{(2)}}^{1/2} \in \real^{n_2 \times p}$, where every entry of $Z^{(1)}, Z^{(2)}$ is drawn independently from a one dimensional distribution with zero mean, unit variance, and constant $\varphi$-th moment for $\varphi > 4$.
...
%				= b_0.
			\end{align}
\end{theorem}
Our result extends Fact \ref{fact_tr} on the inverse of a sample covariance matrix.
When $X_1$ is zero, we solve equation \eqref{eq_a12extra} and obtain $a_1 = 0$ and $a_2 = (n_2-p) / n_2$, and apply these solutions to the variance limit.
