...
$$\tr \left\{\left[ (Y-z\id)^{-1}-R(z)\right]B\right\}\to 0.$$
If the above equation holds for all $p$, then we say that $(Y-z\id)^{-1}$ has a matrix limit $R$.
For our result, we shall prove the convergence of the resolvent $(Y-z\id)^{-1}$ using ``local laws'' with a sharp convergence rate.

\medskip
\noindent\textbf{Resolvents.} For our purpose, we use a convenient linearization trick in linear algebra, that is, the SVD of a rectangular matrix $A$ is equivalent to the study of the eigendecomposition of the symmetric block matrix
