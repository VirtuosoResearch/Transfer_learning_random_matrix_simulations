...
Let $U\Lambda V^\top$ denote the singular value decomposition of $M$, where $\Lambda\in\real^{p\times p}$ consists of the singular values of $M$, $\lambda_1, \lambda_2, \dots, \lambda_p$ in descending order.
	...
	\begin{enumerate}
		\item[i)]  The variance equation converges to the following asymptotic limit
			\begin{align}\label{lem_cov_shift_eq}
				\bigabs{\bigtr{\Sigma^{(2)} \bigbrace{\frac{\hat{\Sigma}^{-1}}{n_1 + n_2} - (a_1 \Sigma^{(1)} + a_2 \Sigma^{(2)})^{-1}}}} \lesssim p^{-c_{\varphi}}\norm{\Sigma^{(2)}},
			\end{align}
			...
			\end{align}
		\item[ii)] For any fixed vector $w\in\real^p$, the bias equation converges to the following asymptotic limit
