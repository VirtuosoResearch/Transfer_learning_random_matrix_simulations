...
Therefore, we consider an arbitrary local minimum and provide generalization bounds that scale with key properties of task data such as sample sizes and covariate shifts.
...
Without loss of generality, we consider task two.
For this setting, we can obtain the hard parameter sharing estimator $\hat{\beta}_2^{\MTL} = B A_2 = B$ by solving $B$ in equation \eqref{eq_mtl} as follows:
\begin{align*}
	\hat{\beta}_2^{\MTL} = {\hat{\Sigma}}^{-1} ({X^{(1)}}^{\top} Y^{(1)} + {X^{(2)}}^{\top} Y^{(2)}), \text{ where } \hat{\Sigma} = {X^{(1)}}^{\top} X^{(1)} + {X^{(2)}}^{\top} X^{(2)}
\end{align*}
Next, we derive the prediction loss of hard parameter sharing in expectation over the random noise, since the empirical quantity is close to the expectation similar to what we have shown in Claim \ref{claim_pred_err}:
\begin{align*}
...
%In Section \ref{app_proof_main_thm},
