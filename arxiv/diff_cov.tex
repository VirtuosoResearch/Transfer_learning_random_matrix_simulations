\section{Bias and Variance Limits when Tasks have Different Sample Sizes and Covariate Shifts}\label{sec_diff}
...
Next, we illustrate how varying sample sizes and covariate shifts impact hard parameter sharing, respectively.
...
We will prove the convergence of $Y$'s resolvent matrix using the so-called ``local laws'' with a sharp convergence rate \cite{isotropic,erdos2017dynamical,Anisotropic}.
...
Using equation \eqref{eigen2extra}, we have that when $p$ goes to infinity, % the trace of $$ converges to
\[ \bigtr{\Sigma^{(2)} \hat{\Sigma}} \rightarrow \bigtr{(a_1 \Lambda^2 + a_2\id)^{-1}} = \bigtr{(a_1 M^{\top}M + a_2 \id)^{-1}} = \bigtr{\Sigma^{(2)} (a_1 \Sigma^{(1)} + a_2 \Sigma^{(2)})^{-1}}. \]
...
As a remark, for $\frac{\dd \cal G(z)}{\dd z}$ to stay close to its limit at $z = 0$, we not only need the limit of $\cal G(0)$, but also the limit of $\cal G(z)$ within a small neighborhood of $0$.
...
Finally, we extend our result to general random matrices under the moment condition in Assumption \ref{assume_rm}.
