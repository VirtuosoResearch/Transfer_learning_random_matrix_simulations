...
The different covariates case differs from the same covariates case in two key aspects.
...
Unlike the previous section, we no longer have an optimal solution for $A$.
...
\begin{corollary}\label{cor_MTL_loss}
	...
	(i) task one's sample size is at least $n_1 \ge \max(\frac{17}{\varepsilon^2}, 50) \cdot p$ for a fixed positive value $\varepsilon$, and task two's sample size is at least $n_2 \ge \frac{3}{2}p$;
...
We show that as a function of $n_1$, the limit equation always decreases first from $\frac{\sigma^2 p}{n - p}$ with $n_1$ being zero, and then increases to $d^2$ with $n_1$ going to infinity.
