...
&+  A^2 \exarg{ \epsilon^{(2)}}{ (\epsilon^{(2)} )^\top X^{(2)} \Sigma(A)^{-1}  (X^{(1)})^\top X^{(1)}\Sigma(A)^{-1} (X^{(2)})^\top \epsilon^{(2)}}  +A^2 \exarg{ \epsilon^{(1)}} { (\epsilon^{(1)})^\top X^{(1)} \Sigma(A)^{-1}  (X^{(2)})^\top X^{(2)}\Sigma(A)^{-1} (X^{(1)})^\top \epsilon^{(1)}  }\\
		&= \left\| X^{(1)} \Sigma(A)^{-1} (X^{(2)})^\top X^{(2)} (A\beta_2-\beta_1)\right\|^2 +\left\| X^{(2)} \Sigma(A)^{-1} (X^{(1)})^\top X^{(1)} (A\beta_1-A^2\beta_2)\right\|^2  \\
...
& h_2(A) := {(\epsilon^{(1)})^\top \left(A^2 X^{(1)}\Sigma(A)^{-1} (X^{(1)})^\top-\id_{n_1}\right)^2\epsilon_1}  ,\\
...
For $h_1(A)$, using Lemma \ref{largedeviation} in Appendix \ref{app_tool} and the fact that the entries of $\beta_1-A\beta_2\in \R^p$ are i.i.d. Gaussian random variables with mean zero and variance $p^{-1}[(A-1)^2\kappa^2 + (A^2+1)d^2/2]$, we obtain the following estimate with high probability:
\begin{align*}
...
&\lesssim p^{1/2+e_1}\cdot \frac{(A-1)^2\kappa^2 + (A^2+1)d^2/2}{(A^2+1)^2}\lesssim p^{1/2+e_1}(\kappa^2 + d^2),
\end{align*}
for any small constant $\e_1>0$. Here in the last step we used \eqref{op_X12} and \eqref{op_Sig1} to bound the operator norm:
\begin{align*}
