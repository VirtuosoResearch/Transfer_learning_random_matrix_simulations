...
We now give the proof of Proposition \ref{cor_MTL_loss}. As in the proof of Theorem \ref{thm_many_tasks}, we first derive a bias-variance decomposition of the prediction loss $L(\hat \eta_2^{\MTL})$. Then we can calculate the bias term and the variance term using Theorem \ref{thm_main_RMT}. 
...
Under the two-task setting of Corollary \ref{cor_MTL_loss}, the optimization objective \eqref{eq_mtl} becomes
 ...
 \begin{align}
		& g(A) \define  \left\| X^{(1)} \hat \Sigma^{-1} (X^{(2)})^\top X^{(2)} (A\beta_2-\beta_1)+ \left(A^2 X^{(1)}\hat \Sigma^{-1} (X^{(1)})^\top-\id_{n_1}\right)\epsilon_1 + A X^{(1)}\hat \Sigma^{-1} (X^{(2)})^\top \epsilon_2 \right\| \nonumber\\
		&  +\left\| X^{(2)} \hat \Sigma^{-1} (X^{(1)})^\top X^{(1)} (A\beta_1-A^2\beta_2)+ \left(X^{(2)}\hat \Sigma^{-1} (X^{(2)})^\top-\id_{n_2}\right)\epsilon_2 + A X^{(2)}\hat \Sigma^{-1} (X^{(1)})^\top \epsilon_1 \right\|, 
		\label{eq_mtl_A12}
	\end{align}
	where we denote
...
\begin{align*}	
\exarg{\epsilon_1, \epsilon_2}{g(A) \mid X_1, X_2 ,\beta_1,\beta_2} = 
\end{align*}
As in Claim \ref{lem_exp_opt}, we first calculate the mean 
...
Plugging $Y^{(1)}$ and $Y^{(2)}$ in \eqref{model_YvsX} into \eqref{eq_Bhat_2task}, we can rewrite that 
 \begin{align}
		...
		%&= (B^\star A ^{\top}) (A A^{\top})^{-1} + (X^{\top}X)^{-1}X^{\top}   \bigbrace{\sum_{j=1}^t \varepsilon_i A_i^{\top}} (A  A^{\top})^{-1}.
	\end{align}
\todo{next derive the prediction loss} 
 \cob
