...
Under the two-task setting of Corollary \ref{cor_MTL_loss}, the optimization objective \eqref{eq_mtl} becomes
 ...
 \begin{align}
		 g(x) \define & \left\| X^{(1)} \Sigma(x)^{-1} (X^{(2)})^\top X^{(2)} (x\beta_2-\beta_1)+ \left(x^2 X^{(1)}\Sigma(x)^{-1} (X^{(1)})^\top-\id_{n_1}\right)\epsilon^{(1)} \right. \nonumber\\
		& \left. + x X^{(1)}\Sigma(A)^{-1} (X^{(2)})^\top \epsilon^{(2)} \right\|^2 \nonumber\\
		 + & \left\| X^{(2)} \Sigma(x)^{-1} (X^{(1)})^\top X^{(1)} (x\beta_1-x^2\beta_2)+ \left(X^{(2)}\Sigma(x)^{-1} (X^{(2)})^\top-\id_{n_2}\right)\epsilon^{(2)} \right. \nonumber\\
		 &\left. + x X^{(2)}\Sigma(x)^{-1} (X^{(1)})^\top \epsilon^{(1)} \right\|^2, 
		\label{eq_mtl_A12}
	\end{align}
	where we denote {\cor revise}
	$$ \Sigma(x):= x^2 (X^{(1)})^\top X^{(1)}  + (X^{(2)})^\top X^{(2)} .$$
...
\paragraph{Part $2$: approximating the global minimum.}
	Let $\hat{A}$ denote the minimizer of $g(A)$.\HZ{change $\hat{A}$ to say $\hat{x}$.}
	We show that in the setting of Corollary \ref{cor_MTL_loss}, $\hat A$ is close to 1.% This will be used to calculate the predication loss $\te(\hat{\beta}_2^{\MTL})$ loss.
...
\end{align}
where in the first step we used that $|A-1|^2\gtrsim 1+A^2$ for $|A-1|\ge \delta$, $1+\tau\le \rho_1\le \tau^{-1}$ and $1+\tau\le \rho_2\le \tau^{-1}$, and in the second step we use equation \eqref{choiceofpara0} \HZ{I'm not sure which condition we use? put it here}. Hence using Claim \ref{claim_largedev1} and equations \eqref{eq_h1}-\eqref{eq_hA}, we obtain that with high probability,
\begin{align}
...
\end{align}
under condition \eqref{choiceofpara0} \HZ{again, I don't see which condition we use here?}.
This gives that
%\be\label{rough A-1}
...
under equation \eqref{choiceofpara0} \HZ{I don't see which condition we want to use here?}, which implies the inequality \eqref{gA>g1}. In sum, we obtain that the minimizer $\hat A$ must satisfy equation \eqref{hatw_add1}.
