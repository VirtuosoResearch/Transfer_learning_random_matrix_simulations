...
For the isotropic model in Section \ref{sec_similarity}, we actually have an easier and sharper bound than Theorem \ref{thm_model_shift} as follows.
\begin{lemma}[The isotropic model]\label{prop_model_shift_tight}
		...
		$\Delta^\star_{\bias} := \bigbrace{(1 - \hat{v})^2 \kappa^2 + d^2} \bigtr{\Pi(\hat v)}.$
...
The proof of Lemma \ref{prop_model_shift_tight} is similar to that for Theorem \ref{thm_model_shift}, except that we can replace \eqref{bounddelta-} with a tighter bound. We only describe the main difference.
...
\end{align*}
Bounding $(X_1^\top X_1)^2=(Z_1^\top Z_1)^2$ using \eqref{eq_isometric} again, we obtain that
...
Note that $\delta^\star_{\bias}(v) \sim p$, hence combining \eqref{anotherbeta} and \eqref{anotherwtbeta} we get
\be\label{replaceest}
...
\be\label{choiceofpara}
	\frac{pd^2}{\sigma^2} \le \OO(1),\quad p^{-1+c_0} \le \frac{\kappa^2}{\sigma^2 }  \le p^{-\e_0-c_0}.
\ee
Then we have
\begin{align}
...
\begin{proof}
	...
	\be\label{boundv-w}
		g(v)=\left[N_2\cdot h(v) + (N_1\sigma^2_1+N_2\sigma^2_2)\right]\cdot \left( 1+\OO(p^{-(1-\e_0)/2+\e})\right)
	\ee
...
\subsection{Sample Size} \label{app_proof_32}
