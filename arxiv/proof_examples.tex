...
\paragraph{Proofs for two tasks.}  We now give the proof of Proposition \ref{cor_MTL_loss}. As in the proof of Theorem \ref{thm_many_tasks}, we first derive a bias-variance decomposition of the prediction loss $L(\hat \eta_2^{\MTL})$. Then we can calculate the bias term and the variance term using Theorem \ref{thm_main_RMT}. 
...
\end{align*}
where we used that the entries of $\beta_1-A\beta_2\in \R^p$ are i.i.d. random variables with mean zero and variance $p^{-1}[(A-1)^2\kappa^2 + (A^2+1)d^2/2]$.
...
Then we calculate the predication loss ...
