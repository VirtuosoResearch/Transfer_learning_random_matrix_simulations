...
\end{align*}
 \end{claim}
 \begin{proof}
 The proof of this claim is the same as the one for Claim \ref{claim_largedev1}, so we only give an outlier of proof without writing down all the details. We expand $L(\hat{\beta}_2^{\MTL}) $ in \eqref{Lbeta_HPS} as:
 \begin{align*}
\cor L(\hat{\beta}_2^{\MTL}) &=  \left\| (\Sigma^{(2)})^{1/2}\hat \Sigma^{-1} \left[(X^{(1)})^\top X^{(1)} (\hat A\beta_1-\hat A^2\beta_2)+ (X^{(2)})^\top \epsilon^{(2)} + \hat A   (X^{(1)})^\top \epsilon^{(1)} \right]\right\|^2,
\end{align*}
 \end{proof}
