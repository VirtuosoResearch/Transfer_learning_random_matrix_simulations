...
Using the concentration estimates in Lemma \ref{largedeviation}, we first show that $g(x)$ and its conditional expectation over $X^{(1)}, X^{(2)}$ are close. %$\exarg{\epsilon^{(1)}, \epsilon^{(2)}}{g(x) \mid X_1, X_2 ,\beta_1,\beta_2}$.
...
%\HZ{restructure this part. merge the result of B.17 and B.18, and state it upfront. separate its proof into two parts}
%\HZ{just say that this calculation of B.97 is similar to B.81?}
...
\end{align}
%where we denoted $$\hat \Sigma:= \hat x^2 (X^{(1)})^\top X^{(1)}  + (X^{(2)})^\top X^{(2)} .$$
The main goal of this step is to prove the following two claims.  
\begin{claim}
...
\end{align}
for any small constant $c>0$, where we recall that $\hat \Sigma=\hat \Sigma(1)$ as defined in \eqref{def hatsig}.
\end{claim}
\begin{proof}
{\cor simplify} To illustrate the idea of bias-variance decomposition, we assume that $\hat x$ is independent of $\epsilon^{(1)}$, $\epsilon^{(2)}$, $\beta_1$ and $\beta_2$ for now. Then we can calculate the partial expectation of $L(\hat{\beta}_2^{\MTL})$ over the randomness in $\epsilon_1$ and $\epsilon_2$:
 \begin{align}
...
%control the difference between $g(x)$ and its partial expectation. 
 ...
 In the setting of Proposition \ref{cor_MTL_loss}, if $\hat x=\OO(1)$, then we have that with high probability,
$$\left| L(\hat{\beta}_2^{\MTL})- \cal L(\hat x)\right| \le p^{-1/2+c} \left(\sigma^2 +\kappa^2 +d^2 \right),$$
for any small constant $c>0$. Here we introduced the function
\begin{align*}
...
In the setting of Proposition \ref{cor_MTL_loss}, we can further simplify $\cal L(\hat x)$ using Claim \ref{lem_hat_v} and $\Sigma^{(1)}=\Sigma^{(2)}=\id_p$.
...
\end{claim}
\begin{proof}
Using Lemma \ref{lem_hat_v} and equations \eqref{op_X12}-\eqref{op_Sig1}, we can obtain that with high probability,
\begin{align}
...
\be\label{eq_var112}\tr(\hat\Sigma^{-1})  = \frac{p}{n_1+n_2} \cdot \frac{\rho_1+\rho_2}{\rho_1+\rho_2-1}+\OO(p^{-c_\varphi})=  \frac{1}{\rho_1+\rho_2-1}+\OO(p^{-c_\varphi})\ee
...
\end{align}
with high probability for some constant $C>0$. Then using Claim \ref{claim_largedev2} and the conditions $\sigma^2 \lesssim  \kappa^2$ and $d^2 \le p^{-c_{\varphi}}{\kappa^2}$, we conclude \eqref{cor_MTL_error} by noticing that the constant $c>0$ can be arbitrarily small. %, we conclude \eqref{cor_MTL_error} 
\end{proof}
%\begin{align}
