...
Under the two-task setting of Corollary \ref{cor_MTL_loss}, the optimization objective \eqref{eq_mtl} becomes
	...
	\begin{align}
		\hat{B}(A) \define  (A_1^2 (X^{(1)})^{\top}X^{(1)} + A_2^2 (X^{(2)})^{\top}X^{(2)})^{-1} (A_1 (X^{(1))})^{\top}Y^{(1)} + A_2 (X^{(2)})^{\top}Y^{(2)}). \label{eq_Bhat_2task} %\\
...
&=[(A-1)^2\kappa^2 + (A^2+1)d^2/2] \cdot p^{-1} \tr\left[ (X^{(1)})^\top X^{(1)} \hat \Sigma^{-1}(X^{(2)})^\top X^{(2)}\right]+\sigma^2(n_1+n_2-p), \label{EgA}
\end{align}
where in the first step we used that the entries of $\beta_1-A\beta_2\in \R^p$ are i.i.d. random variables with mean zero and variance $p^{-1}[(A-1)^2\kappa^2 + (A^2+1)d^2/2]$, and in the second step we used  %Using $(X^{(2)})^\top X^{(2)}=\hat\Sigma- A^2 (X^{(1)})^\top X^{(1)}$, we can further simplify that
\begin{align*}
 ...
 &=\tr\left[  (X^{(2)})^\top X^{(2)} \hat \Sigma^{-1}(X^{(1)})^\top X^{(1)} \hat \Sigma^{-1} (X^{(2)})^\top X^{(2)}\right]+ A^2 \tr\left[  (X^{(1)})^\top X^{(1)} \hat \Sigma^{-1}(X^{(1)})^\top X^{(1)} \hat \Sigma^{-1}(X^{(2)})^\top X^{(2)} \right] \\
 & = \tr\left[ (X^{(1)})^\top X^{(1)} \hat \Sigma^{-1}(X^{(2)})^\top X^{(2)}\right] .
\end{align*}
 In the first step of the above derivation, we used that 
 $$(X^{(1)})^\top X^{(1)} \hat \Sigma^{-1}(X^{(2)})^\top X^{(2)} =\left( A^2 [(X^{(2)})^\top X^{(2)}]^{-1} + [(X^{(1)})^\top X^{(1)}]^{-1} \right)^{-1} = (X^{(2)})^\top X^{(2)} \hat \Sigma^{-1}(X^{(1)})^\top X^{(1)}.$$
...
\begin{proof}
Corresponding to \eqref{EgA}, we define the function
\begin{align*}
h(A):= [(A-1)^2\kappa^2 + (A^2+1)d^2/2] \cdot p^{-1} \tr\left[ (X^{(1)})^\top X^{(1)} \hat \Sigma^{-1}(X^{(2)})^\top X^{(2)}\right].
\end{align*}
...
&\preceq (X^{(1)})^\top X^{(1)} \hat \Sigma^{-1}(X^{(2)})^\top X^{(2)} =\left( A^2 [(X^{(2)})^\top X^{(2)}]^{-1} + [(X^{(1)})^\top X^{(1)}]^{-1} \right)^{-1} \nonumber\\
...
\end{align}
Then we get that
\be\label{eq_h1}h(1)= d^2 \cdot p^{-1} \tr\left[ (X^{(1)})^\top X^{(1)} \hat \Sigma^{-1}(X^{(2)})^\top X^{(2)}\right]\lesssim pd^2\left(\rho_1^{-1} + \rho_2^{-1}\right)^{-1}\lesssim pd^2.\ee
...
\end{align}
where in the first step we used that $|A-1|^2\gtrsim 1+A^2$ for $|A-1|\ge \delta$, $1+\tau\le \rho_1\le \tau^{-1}$ and $1+\tau\le \rho_2\le \tau^{-1}$, and in the second step we used \eqref{choiceofpara0}. Hence using Claim \ref{claim_largedev1}, \eqref{eq_h1} and \eqref{eq_hA}, we obtain that with high probability,
\begin{align}
...
\end{align}
under \eqref{choiceofpara0}.
This gives that
\be\label{rough A-1}
|\hat A-1|\le \delta
\ee
for any small constant $\delta>0$.
...
\begin{align*}
h(A):= [(1-A^{-1})^2\kappa^2 + (1+A^{-2})d^2/2] \cdot p^{-1} \tr\left([(X^{(2)})^\top X^{(2)}]^{-1} + A^{-2} [(X^{(1)})^\top X^{(1)}]^{-1}\right).
\end{align*}
Notice that
$$\tr\left([(X^{(2)})^\top X^{(2)}]^{-1} + A^{-2} [(X^{(1)})^\top X^{(1)}]^{-1}\right).$$
is an increasing functions in $A$. Hence taking derivative of $h(w)$ with respect to $w$, we obtain that
\begin{align}\label{h'(A)1}
h'(A) \ge \left[2(1-A^{-1})\frac{\kappa^2}{A^2} - \frac{d^2}{A^3}\right] \cdot p^{-1} \tr\left([(X^{(2)})^\top X^{(2)}]^{-1} + A^{-2} [(X^{(1)})^\top X^{(1)}]^{-1}\right) \ge 0,
\end{align}
...
$$\tr\left(A^2[(X^{(2)})^\top X^{(2)}]^{-1} +  [(X^{(1)})^\top X^{(1)}]^{-1}\right).$$
is a decreasing functions in $A$. Hence taking derivative of $h(A)$, we obtain that
\begin{align}\label{h'(A)2}
	h'(A)\le [-2(1-A)\kappa^2 + Ad^2] \cdot p^{-1} \tr\left(A^2[(X^{(2)})^\top X^{(2)}]^{-1} + [(X^{(1)})^\top X^{(1)}]^{-1}\right)\le 0,
\end{align}
...
Now we are ready to prove \eqref{hatw_add1}. First using \eqref{op_uplow} and \eqref{h'(A)1}, we obtain that for $1+d^2/\kappa^2 \le A\le 1+\delta$,
$$h'(A)\ge \frac{2(A-1)\kappa^2 - d^2}{A^3} \cdot p^{-1} \tr\left([(X^{(2)})^\top X^{(2)}]^{-1} + A^{-2} [(X^{(1)})^\top X^{(1)}]^{-1}\right) \gtrsim p\left(d^2 + |A-1|\kappa^2\right).$$
...
under \eqref{choiceofpara0}, which implies the inequality as in \eqref{gA>g1}. Second using \eqref{op_uplow} and \eqref{h'(A)2}, we obtain that for $1-\delta \le A \le 1-d^2/\kappa^2$,
$$- h'(A)\ge  [2(1-A)\kappa^2 - Ad^2] \cdot p^{-1} \tr\left(A^2[(X^{(2)})^\top X^{(2)}]^{-1} + [(X^{(1)})^\top X^{(1)}]^{-1}\right)  \gtrsim p\left(d^2 + |A-1|\kappa^2\right).$$
...
under \eqref{choiceofpara0}, which implies the inequality as in \eqref{gA>g1}. In sum, we obtain that the minimizer $\hat A$ must satisfy \eqref{hatw_add1}.
...
\paragraph{Calculating the prediction loss.} Now we calculate the predication loss using the optimal $\hat B(\hat A) $ in \eqref{eq_Bhat_2task}. ......
...
\paragraph{Calculating the variance and bias terms.} For the isotropic model in Section \ref{sec_similarity}, we actually have an easier and sharper bound than Lemma \ref{thm_model_shift}. %as follows.
