...
\begin{align}\label{eq_mtl}
			f(A, B) = \sum_{i=1}^t \norm{X^{(i)} B A_i - Y^{(i)}}^2.
\end{align}
Given a solution $(\hat{A}, \hat{B})$ to the above optimization problem, let $\hat{\beta}_i^{\MTL} = \hat{B} \hat{A}_i$ denote the hard parameter sharing (HPS) estimator for task $i$.
...
\begin{enumerate}
	...
	Our results further imply that HPS's sample efficiency depends on model-specific variance vs. noise variance. It is generally high when the noise variance is large compared to model-specific variance across different tasks.
...
An influential paper by \citet{BBCK10} provides uniform convergence bound that combines multiple datasets in certain settings.
