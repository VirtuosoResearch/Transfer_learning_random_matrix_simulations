...
Here we have assumed that \eqref{assmAhigh} holds solely for simplicity of representation.  On the other hand, if the entries of $Z$ only have finite $a$-th moment for some $a>4$, then all the results below still hold except that we need to replace $\OO(p^{-1/2+\e})$ with $\OO( p^{-\frac12+\frac2a +\epsilon})$ in the error bounds. We will not get deeper into this issues in this section, but refer the reader to Corollary \ref{main_cor} below. 
...
Suppose $X$ satisfies assumption \ref{assm_secA1}. Let $A$ be any $p\times p$ matrix that is independent of $X$. We have that for any constant $\e>0$,
	\be\label{XXA}  \bigtr{(X^{\top}X)^{-1}A} = \frac{1}{\rho - 1} \frac1p\tr(A\Sigma^{-1}) +\bigo{ \|A\|p^{-1/2+\epsilon}} \ee
