...
\end{align}
Inserting \eqref{hatB} into \eqref{eq_mtl_2tasktilde}, one can see that $g$ only depends on the ratio $v:=w_1/w_2$. Hence we will also write $g(v)$ in the following discussion.

Let $\hat v=\hat{w_1}/\hat{w_2}$ be the global minimizer of $g(v)$. We will define the multi-task learning estimator for the target task as
...
Then the test loss of using $\hat{\beta}_2^{\MTL}$ for the target task is
\begin{align}
...
\begin{align*}
	L(\hat{\beta}_2^{\STL}) = \sigma_2^2 \cdot \bigtr{\Sigma_2(X_2^{\top}X_2)^{-1}}, \ \text{ where } \ \hat{\beta}_2^{\STL} = (X_2^{\top}X_2)^{-1} X_2^{\top}Y_2.
\end{align*}
...
In fact using concentration of random vectors with i.i.d. entries, we can reduce $g$ to an expression that is easier to handle. 
...
Suppose $Y_i = X_i\beta_i + \varepsilon_i$ and $\wt Y_i = \wt X_i\beta_i + \wt\varepsilon_i$, $i=1,2$, all satisfy Assumption \ref{assm_secA2}. Then we rewrite \eqref{eq_mtl_2tasktilde} as
...
\begin{align*}
		&\val(v):= \exarg{\varepsilon_1,\e_2} {\sum_{i=1}^2 \left\|\Sigma_i^{1/2}( \hat B w_i - \beta_i) \right\|^2} \\
...
\ee
Hence to minimize $g$, it suffices to minimize $\val(v)$ over $v$.
