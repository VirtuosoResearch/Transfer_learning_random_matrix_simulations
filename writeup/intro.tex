...
A similar result on the bias of $\hat{\beta}_t^{\MTL}$ that scales with task similarity in addition to sample size and covariate shift holds.
\end{theorem}

%Concretely, we show a tight bound on the trace of $(X_1^{\top}X_1 + X_2^{\top}X_2)^{-1}$, which
Theorem \ref{lem_cov_shift_informal} allows us to analyze of the bias-variance tradeoff of the multi-task estimator for two settings:
...
It is crucial that the concentration error of Theorem \ref{lem_cov_shift_informal} is small so that we can explain the transition phenomena in Figure \ref{fig_model_shift} and \ref{fig_size}.
...
Theorem \ref{lem_cov_shift_informal} allows us to compare MTL performance under different covariate shift.
