...
Next, we explain the performance of multi-task learning by varying the three components above in isotropic and covariate shifted settings.
...
\squishlist
	\item In Proposition \ref{prop_dist_transition}, Our theoretical results in  and  provide predictions that match the empirical result of (a) and (b).
%	It is crucial that the concentration result in Theorem \ref{lem_cov_shift_informal} is sufficiently precise so that we can explain the transition phenomena in Figure \ref{fig_model_shift} and \ref{fig_size}.
	...
	\item In Proposition \ref{prop_data_size},
	%Theorem \ref{lem_cov_shift_informal} allows us to compare MTL performance under different covariate shifts.
	\item In Proposition \ref{prop_covariate}, When task models are the same and the covariate shift matrix belongs to a certain bounded set, we show that having no covariate shift yields the optimal MTL performance.
	When $\beta_1 = \beta_2$, Lemma \ref{lem}
\squishend
