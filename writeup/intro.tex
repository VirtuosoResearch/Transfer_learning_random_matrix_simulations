...
%	It is crucial that the concentration result in Theorem \ref{lem_cov_shift_informal} is sufficiently precise so that we can explain the transition phenomena in Figure \ref{fig_model_shift} and \ref{fig_size}.
	...
	\item We study a special case of having the same models and show that MTL  performs best without covariate shift (Proposition \ref{prop_covariate}).
...
Using our incremental training schdule, we reduce the computational cost by $55\%$ compared to baseline multi-task training over six sentiment analysis dataset while keeping the accuracy the same.
Third, we provide a fine-grained insight on a covariange alignment procedure proposed in \cite{WZR20}.
