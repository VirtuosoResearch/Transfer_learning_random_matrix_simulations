...
Let $n_i = \rho_i \times p$ denote the data size and $X_i\in\real^{n_i\times p}$ denote the features of task $i$.
...
Let $\hat{\beta}_t^{\MTL} = B W_t$ denote the optimal multi-task estimator for the target task, which is defined precisely in Section \ref{sec_setup}.
...
\begin{theorem}[Informal]\label{lem_cov_shift_informal}
	In the setting of two tasks, let $a_1, a_2$ be fixed values solved from deterministic equations given by $\rho_1, \rho_2$ and the singular values of $\Sigma_1^{1/2}\Sigma_2^{-1}\Sigma_1^{1/2}$.
...
We extend our result to transfer learning in the setting of high-dimensional linear regression.
We pool learnt source task representations into the shared body of the hard parameter sharing architecture, as in Taskonomy by Zamir et al. \cite{ZSSGM18}.
We show that the bias of the transfer learning estimator can be captured by the projection of $\beta_t$ to the orthogonal subspace spanned by $\set{\beta_i}_{i=1}^{t-1}$.
