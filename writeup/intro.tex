...
\vspace{-0.05in}
	...
	Yet, there is little rigorous understanding of when multi-task learning ouperforms single-task learning.
	In this work, we provide a rigorous study of this question in the setting of high-dimensional linear regression.
	We show that the bias-variance tradeoff of multi-task learning determines the  effect of transfer and develop new concentration bounds to analyze the tradeoff.
	The high-dimensional linear regression setting allows us to define three components to measure the difference between tasks, including \textit{task similarity}, \textit{sample size}, and \textit{covariate shift}.
%	and show that the bias-variance tradeoff of multi-task estimators determines the transfer effect.
	We relate each component to the bias and variance of multi-task learning to explain three negative effects as a result of decreased task similarity,	increased source sample size, and covariate shift under increased source sample size.
...
Second, we explain the performance of multi-task learning observed in Figure \ref{fig_model_shift_phasetrans} by varying the three components above in isotropic and covariate shifted settings.
