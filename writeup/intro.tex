...
%When does multi-task learning provide positive (or negative) information transfer, compared to single-task learning?
	...
	In this work, we study this question by considering three components including \textit{task model distance}, \textit{sample size} and \textit{covariate shift} in the setting of high-dimensional linear regression.
...
%a multi-task learning architecture that uses a shared layer for all tasks and a separate output layer for each task.
	We show that whether multi-task learning performs better than single-task learning is determined by the bias-variance tradeoff of the multi-task estimator.
...
%	We develop a technical tool on the inverse of the sum of two random matrices that naturally arises from the analysis.
	...
	Finally, we validate the above results on text classification tasks.
	Inspired by our theory, we show that single-task performance can help understand multi-task performance. We propose an incremental training scheduler for improving the efficiency of multi-task training for predicting a particular task.
