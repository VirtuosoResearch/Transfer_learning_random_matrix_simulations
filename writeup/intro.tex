...
We consider multi-task learning in the setting of high-dimensional linear regression.
First, we provide a sharp analysis of the bias-variance tradeoff for multi-task learning, and extend the result to the related setting of transfer learning.
We develop a technical tool on the trace of $(X_1^{\top}X_1 + X_2^{\top}X_2)^{-1}$ for two random matrices $X_1, X_2$ in the setting of two tasks.
...
For a target task $t$, let $\hat{\beta}_t^{\MTL} = B W_t$ denote the optimal multi-task (MTL) estimator (defined precisely in Section \ref{sec_setup}).
Let $\hat{\beta}_t^{\STL}$ denote the standard single-task (STL) estimator for task $t$.
...
Our first contribution is to develop a new technical tool that provides a tight bound on the trace of $(X_1^{\top}X_1 + X_2^{\top}X_2)^{-1}$, which arises naturally from the setting of two tasks.
...
We apply our tool to both multi-task and transfer learning settings.
We present the following results in Section \ref{sec_main}.
\squishlist
