...
%When does multi-task learning provide positive (or negative) information transfer, compared to single-task learning?
	...
	In this work, we consider three components including \textit{task model distance}, \textit{sample size} and \textit{covariate shift} to determine positive vs. negative transfer.
...
%	We develop a technical tool on the inverse of the sum of two random matrices that naturally arises from the analysis.
	Then, we show that the performance of multi-task learning can be negatively affected as a result of: (a) increased task model distance;
...
\squishlist
	\item \textbf{Task model distance:} Assume that each task follows a linear model with parameters $\beta_i\in\real^p$, for $1\le i\le t$.
	Hence, the distance between two tasks can be measured by the distance of their model parameters.
	...
	Hence, for two tasks $i$ and $j$, their data ratio is measured by $n_i / n_j = \rho_i / \rho_j$.
	Importantly, we assume that for a particular (say $t$-th) task, $\rho_t$ is a small constant (say 2), to capture the need for adding more labeled data.
	...
	Given two tasks $i$ and $j$, covariate shift by $\Sigma_i^{1/2}\Sigma_j^{-1/2}$.
\squishend
...
\begin{figure}[!t]
	...
	(b) transitions from positive to negative transfer as source/target \text{sample size} increases (Prop.\;\ref{prop_data_size});
