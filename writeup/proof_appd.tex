...
\end{proof}
With this corollary, we can easily extend Lemma \ref{lem_cov_shift} and Lemma \ref{lem_cov_derivative} to the case with weaker moment assumptions. Due to length constraints, we will not go into further details here. 
...
In Section \ref{sec tools}, we collect some a priori estimates and resolvent identities that will be used in the proof of Theorem \ref{LEM_SMALL} and Proposition \ref{prop_diagonal}. Then in Section \ref{sec entry} we give the proof of Proposition \ref{prop_diagonal}, which concludes Theorem \ref{LEM_SMALL} for i.i.d. Gaussian $Z_1$ and $Z_2$. Finally, in Section \ref{sec_comparison}, we describe how to extend the result in Theorem \ref{LEM_SMALL} from the Gaussian case to the case with generally distributed entries of $Z_1$ and $Z_2$. In the proof, we always denote the spectral parameter by $z=E+\ii\eta$. 
...
With this lemma, we can obtain the following a priori estimate on the resolvent $G(z)$ for $z\in \mathbf D$.
...
%\end{equation}
 ...
 \item[(iii)]
 For $\fa \in \mathcal I$ and $\fb, \fc \in \mathcal I \setminus \{\fa\}$,
\begin{equation}
