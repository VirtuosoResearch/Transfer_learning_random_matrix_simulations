...
\subsection{Task Model Similarities}
...
\begin{proposition}\label{prop_dist_transition}
	In the example described above, assume that $c > 100$ is a fixed constant.
	...
	\begin{itemize}
		\item If ${p\cdot d^2}/{\sigma^2} < \frac 2 3 \frac{(c_1 + c_2 -1)^2}{c_1 (c_1 + c_2)(c_2 - 1)} - o(1)$, then whp we have that $\te(\hat{\beta}_t^{\MTL}) < \te(\hat{\beta}_t^{\STL})$.
		\item If ${p\cdot d^2}/{\sigma^2} \ge \frac 3 2 \frac{(c_1 + c_2 -1)^2}{c_1(c_1 + c_2)(c_2 - 1)} + o(1)$, then whp we have that $\te(\hat{\beta}_t^{\MTL}) \ge \te(\hat{\beta}_t^{\STL})$.
	\end{itemize}
\end{proposition}
