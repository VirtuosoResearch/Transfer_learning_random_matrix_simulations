...
\begin{proposition}\label{prop_covariate}
	In the setting of Example \ref{ex_basic}, assume that $d^2 < \sigma^2 / (c_1 p)$ and $c_1 > ??$.
	As a parameter of $M \in \cS_{\mu}$, we have that $(\te(\hat{\beta}_t^{\MTL}))(M)$ as a function of $M$ satisfies that 
$$ (\te(\hat{\beta}_t^{\MTL}))(\mu \id) \le \left[1+ {\cor\text{some errors}}\right]\cdot\inf_{M\in \cal S_{\mu}}(\te(\hat{\beta}_t^{\MTL}))(M) .$$	
