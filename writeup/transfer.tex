\section{Explaining Negative Transfer in an Isotropic Setting}\label{sec_insight}
...
We show how negative transfer occurs by varying task similarity or data size.
Then we show that when source task data size becomes large, covariate shift causes more negative effects.
...
\begin{proposition}[Task model distance]\label{prop_dist_transition}
	...
	\squishlist
		...
		\item If $\Psi(\beta_1, \beta_2) \ge \gamma_{-}^{-1}  \Phi(\rho_1, \rho_2)$, then w.h.p. over the randomness of $X_1,X_2$, $\te(\hat{\beta}_t^{\MTL}) \ge \te(\hat{\beta}_t^{\STL})$.
	\squishend
...
We can in fact extend the result to the cases where the noise variance is different.
...
\begin{proposition}[Source/target data size]\label{prop_data_size}
	In the isotropic model, assume that $\rho_1 > 40$ and $\rho_2 > 110$ are fixed constants, $\Psi(\beta_1, \beta_2) > 2/(\rho_2 - 1)$.
	We have that
	\squishlist
...
For $i = 1, 2$, let $\hat{\beta}_i^{\MTL}(x)$ denote the estimator trained using $R \cdot n_i$ datapoints from every task.
The data efficiency ratio roughly scales as
\[ \argmin_{x\in(0, 1)} ~~
...
Compared with Proposition \ref{prop_dist_transition}, we see that when $\Psi(\beta_1, \beta_2)$ is smaller than $\rho_1^{-1} + \rho_2^{-1}$, the transfer is positive.
Hence the data efficiency ratio quantifies how effective is the positive transfer using MTL.
...
The precise statement can be found in Appendix \ref{app_proof_32}.
...
To compare different choices of $M$ on the performance of $\hat{\beta}_t^{\MTL}$, let $\lambda_1, \lambda_2, \dots, \lambda_p$ be the singular values of $M$ in decreasing order.
Let $\mu_{\min} < \mu < \mu_{\max}$ are fixed values that do not grow with $p$.
Consider the following bounded set
\begin{align*}
		\cS_{\mu}\define\bigset{M \mid \prod_{i=1}^p \lambda_i \le \mu^p, \mu_{\min} \le \lambda_p, \lambda_1 \le \mu_{\max}},
\end{align*}
...
\end{proposition}
The following proposition shows that when $n_1 \gg n_2$, $\te(\hat{\beta}^{\MTL})$ is minimized when there is no covariate shift between source and target task.
