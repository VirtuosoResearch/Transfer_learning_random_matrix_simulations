...
\begin{proposition}\label{prop_data_size}
	...
	\begin{itemize}
		\item If $c_2 < \frac{2\sigma^2}{3p d^2} + 1$, then whp $\te(\hat{\beta}_t^{\MTL}) < \te(\hat{\beta}_t^{\STL})$, for any $c_1$ larger than $??$.
		\item If $c_2 > \frac{3\sigma^2}{2p d^2} + 1$, then we have the following transition depending on $c_1$:
		\begin{itemize}
