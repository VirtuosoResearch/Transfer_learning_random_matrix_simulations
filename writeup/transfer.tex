...
\begin{proposition}\label{prop_dist_transition}
	In the example described above, assume that $c > 50$ is a fixed constant.
	...
	\begin{itemize}
		\item If ${p\cdot d^2}/{\sigma^2} < \frac 1 4 \frac{(2c-1)^2}{(c-1)c^2} - o(1)$, then whp we have that $\te(\hat{\beta}_t^{\MTL}) < \te(\hat{\beta}_t^{\STL})$.
		\item If ${p\cdot d^2}/{\sigma^2} \ge \frac{(2c-1)^2}{(c-1)c^2} + o(1)$, then whp we have that $\te(\hat{\beta}_t^{\MTL}) \ge \te(\hat{\beta}_t^{\STL})$.
	\end{itemize}
\end{proposition}

We obtain the following insight from Proposition \ref{prop_dist_transition}.
