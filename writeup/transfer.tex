...
\begin{proposition}[Source/task data ratio]\label{prop_data_size}
	...
	We have that
	\squishlist
		\item If $\frac{n_1}{n_2} = \frac{\rho_1}{\rho_2} < \frac{1 - 2\rho_2^{-1}}{\gamma_+ \cdot \Psi(\beta_1, \beta_2) (1 - \rho_2^{-1}) - \rho_2^{-1}}$, then w.h.p. $\te(\hat{\beta}_t^{\MTL}) < \te(\hat{\beta}_t^{\STL})$.
		\item If $\frac{n_1}{n_2} = \frac{\rho_1}{\rho_2} > \frac{1 - 2\rho_2^{-1}}{\gamma_- \cdot \Psi(\beta_1, \beta_2) (1 - 3\rho_2^{-1}) - \rho_2^{-1}}$, then w.h.p. $\te(\hat{\beta}_t^{\MTL}) > \te(\hat{\beta}_t^{\STL})$.
	\squishend
\end{proposition}
...
We use our tools to explain a key result of Taskonomy by Zamir et al.'18 \cite{ZSSGM18}, which shows that MTL can reduce the amount of labeled data needed from each task to achieve comparable performance to STL.
