...
For the high-dimensional setting where the data size of the target task is a small constant times dimension $p$, we quantify the trade-off as a function of the task models $\set{\beta_i}_{i=1}^k$, covariance matrices $\set{\Sigma_i}_{i=1}^k$, and number of data points $\set{n_i}_{i=1}^k$.
...
\subsection{Two Effects: Model Shift Bias and Variance Reduction}\label{sec_main}
...
%\textbf{The High-Dimensional Setting.}
...
\textbf{Setting one: Two tasks with general covariance matrices.}
...
\textbf{Setting two: Many tasks with the same covariates.}
