...
(a) \textit{Task model distance}: we explain the phenomenon of negative transfer precisely as task models become different.
...
\subsection{Task Model Distance}\label{sec_similarity}
...
\textit{The isotropic model.}
	...
	Each task has sample size $n_i = \rho_i \cdot p$, for $i = 1, 2$.
	...
	The labels are given by $Y_i = X_i\beta_i + \varepsilon_i$, where $\e_i$ consists of i.i.d. entries with mean zero and variance $\sigma^2$.
...
We can see that our result is able to predict positive or negative transfer  accuractly that matches the empirical curve.
...
We apply the above result to the parameter setting of Figure \ref{fig_size} (described in Appendix \ref{app_synthetic}).
Our theory accuractly predicts a curious phenomenon, where increasing the sample size of the source task results in negative transfer!
