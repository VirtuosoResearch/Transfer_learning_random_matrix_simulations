...
We explain three contributing causes of negative transfer, including model dissimilarity, large source data size and covariate shift.
On the positive side, we describe two benefits of multi-task learning, including improving labeled data efficiency and de-noising via less noisy source labels.
...
As a warmup, we first describe an example that rigorously formalizes the belief using our setup.
...
\begin{figure}
	...
	(c) As covariate shift becomes more severe, test performance gets worse (Proposition \ref{prop_covariate}).}
...
On the other hand, if $n_1$ is comparable or even smaller than $n_2$, we show an example where having ``complementary'' covariance matrices is better performing than having the same covariance matrices.
	...
	Similar to the isotropic model, we assume that $\beta_2$ has i.i.d. entries with mean zero and variance $\kappa^2$ and $\beta_1$ equals $\beta_2$ plus i.i.d. entries with mean $0$ and variance $d^2$.
...
\begin{proposition}\label{prop_covariate}
	In the setting described above, assume that $\rho_1 > 3$ and $\rho_2>3$,
and $\|\Sigma_1\|\le C_1$ for some constant $C_1>0$.
	...
	We have that \[ g(\mu\id) \le \bigbrace{1+ \bigo{\frac{\rho_2}{\rho_1} + \frac{1}{\sqrt {\rho_1}}}} \min_{M\in\cS_{\mu}} g(M). \]
\end{proposition}
...
\subsection{Implications on the Benefits of Multi-task Learning}\label{sec_benefit}
