...
\be\label{eq_deltaextra}\delta:=\frac{\al_+^2(\rho_1) - 1 }{\al_-^{2}(\rho_1)} \cdot \kappa^4(\hat M)\cdot \norm{\Sigma_1^{1/2}(\beta_s - \hat{w}\beta_t)}^2,\ee
...
The proof is divided into the following four steps. We first consider $\hat M= v\Sigma_1^{1/2}\Sigma_2^{-1/2}$ for a fixed $v\ge 0$. 
\begin{itemize}
\item[(i)] We use Lemma \ref{lem_minv} and Lemma \ref{lem_cov_shift} to calculate 
$$\sigma^2 \cdot \bigtr{(X_2^{\top}X_2)^{-1}},\quad \sigma^2 \cdot \bigtr{(\hat{v}^2 X_1^{\top}X_1 + X_2^{\top}X_2)^{-1} \Sigma_2},$$
and the difference between them will lead to the $\Delta_{\vari}$ term.

\item[(ii)] Using the approximate isometry property of $X_1$ (see \eqref{eq_isometric} below), we will bound the bias term 
$${v}^2 \bignorm{\Sigma_2^{1/2}({v}^2 X_1^{\top}X_1 + X_2^{\top}X_2)^{-1} X_1^{\top}X_1 (\beta_s - {v} \beta_t)}^2$$
through 
\be\label{deltabetapf}{v}^2 n_1^2\bignorm{\Sigma_2^{1/2}({v}^2 X_1^{\top}X_1 + X_2^{\top}X_2)^{-1} \Sigma_1 (\beta_s - {v} \beta_t)}^2.\ee
...
Finally, we have 
\begin{itemize}
\item[(iv)] We use a standard $\e$-net argument to extend the above results to $\hat M= \hat v\Sigma_1^{1/2}\Sigma_2^{-1/2}$ for a possibly random $\hat v$.
\end{itemize}

\paragraph{Part I: Variance reduction.} Now fix a constant $v>0$. We have \todo{add details}

\paragraph{Part II: Bounding the bias from model shift.}
