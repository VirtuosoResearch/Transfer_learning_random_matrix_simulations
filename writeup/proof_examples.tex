\section{Proofs for Isotropic and Covariate Shifted Settings}\label{app_iso_cov}

For the isotropic model, we will use Lemma \ref{prop_model_shift_tight} to analyze the bias-variance tradeoff.
Our main observation is that the optimum of $\val(v)$ is approximately $1$.
This allows us to simplify the bias and variance bounds significantly.
We define the function
\begin{align}
	\val_\star(v) &= \frac{\rho_1}{\rho_2}\left[d^2 +\left( v-1\right)^2\kappa^2\right]\cdot \tr\left[( v^2X_1^{\top}X_1 +X_2^{\top}X_2)^{-2} (X_2^{\top}X_2)^2\right] \nonumber \\
...
For the isotropic model, using the concentration result for random vectors with i.i.d. entries, Lemma \ref{largedeviation}, we can obtain that $\val(v)= N_2\cdot \val_\star(v)  \left( 1+\OO(p^{-1/2+\e})\right)$. Hence the validation loss in \eqref{approxvalid} reduces to
\be\label{boundv-w}
g(v)=\left[N_2\cdot \val_\star(v) + (N_1\sigma^2_1+N_2\sigma^2_2)\right]\cdot \left( 1+\OO(p^{-(1-\e_0)/2+\e})\right)
\ee
with high probability for any constant $\e>0$. \nc Thus for the following discussions, it suffices to focus on the behavior of $\val_\star (v)$. Let $\hat w$ the minimizer of $\val_\star(v)$. The proof will consist of two steps.
\begin{itemize}
...
For the isotropic model, the minimizer for $\val_\star(v)$ satisfies
...
\end{lemma}
\begin{proof}
To be consistent with the notation $\hat w$, we shall change the name of the argument to $w$ in the proof. First it is easy to observe that $\val_\star(w)< \val_\star(-w)$ for $w> 0$. Hence it suffices to assume that $w\ge 0$.

We first consider the case $w\ge 1$. We write
\begin{align}
	\val_\star(w) &=  \frac{\rho_1}{\rho_2}\left[\frac{d^2}{w^4} +\frac{\left( w-1\right)^2}{w^4}\kappa^2\right]\cdot \tr\left[( X_1^{\top}X_1 +w^{-2}X_2^{\top}X_2)^{-2} (X_2^{\top}X_2)^2\right] \nonumber \\
...
are increasing functions in $w$. Hence taking derivative of $\val_\star(w)$ with respect to $w$, we obtain that
\begin{align*}
\val'_\star(w) \ge &~  \frac{\rho_1}{\rho_2}\left[ \frac{2(w-1)(2-w)}{w^5}\kappa^2 - \frac{4d^2}{w^5}\right] \tr\left[( X_1^{\top}X_1 +w^{-2}X_2^{\top}X_2)^{-2} (X_2^{\top}X_2)^2\right]   \\
...
Hence $\val'_\star(w)>0$ on $(w_1,\infty)$, i.e. $\val_\star(w)$ is strictly increasing for $w>w_1$. This gives $\hat w\le w_1$.

Then we consider the case $w\le 1$, and the proof is similar as above. Taking derivative of $\val_\star(w)$, we obtain that
\begin{align}
	\val'_\star(w) &\le \frac{\rho_1}{\rho_2} \left[2\left( w-1\right) \kappa^2\right]\cdot \tr\left[( w^2X_1^{\top}X_1 +X_2^{\top}X_2)^{-2} (X_2^{\top}X_2)^2\right] \nonumber \\
...
Hence $\val'_\star(w)<0$ on $[0,w_2)$, i.e. $\val_\star(w)$ is strictly decreasing for $w<w_2$. This gives $\hat w\ge w_2$.
...
%\end{lemma}
%\begin{proof}
 In fact, from the proof of Lemma \ref{lem_hat_v} above, one can check that if $C\cal E \le |w-1| \le 2C\cal E$ for a large enough constant $C>1$, then $|\val'_\star(w)|\gtrsim p d^2$. Moreover, under the choice \eqref{choiceofpara} we have
$$\val_\star(w) =\OO(pd^2),\quad \text{for}\quad   |w-1|\le 2C\cal E.$$
Thus we obtain that for $|w-1|\ge 2C\cal E$,
$$\left|\val_\star(w) - \val_\star(\hat w)\right|\ge |\val_\star(w)-\min(\val_\star(w_1),\val_\star(w_2))|\gtrsim pd^2 \cal E \gtrsim \cal E \cdot \val_\star(\hat w),$$
which leads to $g(w) > g(\hat w)$ w.h.p. by \eqref{boundv-w}. Thus $w$ cannot be a minimizer of $g(v)$, and we must have $|\hat v - 1|\le 2C\cal E$. %Together with \eqref{hatw_add1}, we conclude \eqref{hatv_add1}.
%\end{proof}
...
Hence we need to choose that $p d^2 \sim \sigma^2$. On the other hand, we want the error term $\cal E^2 \kappa^2$ to be much smaller than $d^2$, which leads to the condition $p^{-1+\e_0+4\e}\kappa^2  \ll d^2 \ll \kappa^2$. The above considerations lead to the choices of parameters in \eqref{choiceofpara}. Moreover, under \eqref{choiceofpara} we can simplify \eqref{1Liso} to
\begin{align}
...
\be\label{Deltavar2} 
\al_-(\rho_1) - \oo(1)  \le  {\delta_{\vari}^{(2)}}/{ \Delta_{\vari}^{(2)}} \le \al_+(\rho_1) +  \oo(1) , \ee
where 
...
\subsection{Missing Proofs regarding Sample Size} \label{app_proof_32}
...
\begin{align}\label{neg1solv}
 ...
 \rho_1 > \frac{(\rho_2-2) \sigma^2}{\Psi(\beta_1, \beta_2) \cdot  (\rho_2 - 1.5)\left( \al_-^2(\rho_1) -  \oo(1) \right)  - 1}  ,
 \ee
...
\noindent{\bf The upper bound.} From \eqref{reduceproof1} and \eqref{reduceproof2}, we see that $x^\star\le x$ if $x$ satisfies 
\be\nonumber
\begin{split}
