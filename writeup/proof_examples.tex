...
For the isotropic model, we will use Lemma \ref{prop_model_shift_tight} to analyze the bias-variance tradeoff. We define the function
\begin{align}
...
For the isotropic model, using the concentration result, Lemma \ref{largedeviation}, we can obtain that $\val(v)= N_2\cdot h(v)  \left( 1+\OO(p^{-1/2+\e})\right)$. Hence the validation loss in \eqref{approxvalid} reduces to
\be\label{boundv-w}
...
We now show that for the isotropic model, the optimum of $\hat v$ is very close to 1. This allows us to simplify the expression of $\te(\hat{\beta}_2^{\MTL})$ significantly.
...
\begin{lemma}
For the isotropic model, suppose that $\kappa^2$, $d^2$ and $\sigma^2$ satisfy the following relations: for some constant $c_0>0$,
