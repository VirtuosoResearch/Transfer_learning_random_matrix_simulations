...
\todo{check!}In the setting of Proposition \ref{prop_var_transition}, the test loss is given by \eqref{eq_te_mtl_2task}. 
...
\end{align*}
This proves the first claim of Proposition \ref{prop_var_transition} for positive transfer. \cor $\nu = (1+\oo(1)) \cdot (1 - 1/\sqrt{\rho_1})^{-4}$\nc
...
Following the above proof of Proposition \ref{prop_dist_transition}, we see that $\te(\hat{\beta}_2^{\MTL})< \te(\hat{\beta}_2^{\STL})$ w.h.p. if \eqref{pos1} holds, while $\te(\hat{\beta}_2^{\MTL})>\te(\hat{\beta}_2^{\STL})$ w.h.p. if \eqref{neg1} holds. 
%\be\label{pos1}
