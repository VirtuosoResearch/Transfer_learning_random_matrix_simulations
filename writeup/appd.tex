...
\end{equation}
where we denote $n:=n_1+n_2$. Here we have chosen the scaling that is more standard in the random matrix theory literature. Moreover, we assume that the fourth moment exists:
...
\tau^2 \le \sigma_p \le \sigma_1 \le \tau^{-2}. %, \quad \max\left\{\pi_A^{(n)}([0,\tau]), \pi_B^{(N)}([0,\tau])\right\} \le 1 - \tau .
\end{equation}
...
\begin{split}
&b_2+b_3= 1-\gamma_n,\quad r_1 =  b_2 +\frac1n\sum_{i=1}^p \frac{\sigma_i^2 b_2}{ \sigma_i^2 b_2+ (1-\gamma_n - b_2) } .
\end{split}
\ee
where $b_2:=-m_{2c}(0)$ and $b_3:=-m_{3c}(0)$. Note that the function 
$$f(b_2):=b_2 +\frac1n\sum_{i=1}^p \frac{\sigma_i^2 b_2}{ \sigma_i^2 b_2+ (1-\gamma_n - b_2) } $$
is a strictly increasing function on $[0,1-\gamma_n]$, and $f(0)=0<r_1$, $f(1-\gamma_n)=1>r_1$. Hence there exists a unique solution $(b_2,b_3)$ to \eqref{selfomega0}. Moreover, it is easy to check that $f'(a)=\OO(1)$ for $a\in [0,1-\gamma_n]$. Hence there exists a constant $\tau>0$, such that 
\be\label{a23}
\tau<b_2<1-\gamma_n - \tau,\quad \tau<b_3<1-\gamma_n - \tau .
\ee 
...
\be\label{prior1}
|z|\le c_0, \quad \max_{\al=2}^3 |m_{\al c}(z) - m_{\al c}(0)| \le c_0.
\ee
...
Before proving this theorem, we first use it to complete the proof of Lemma \ref{lem_cov_shift} and \cor Lemma \ref{}?estimate \eqref{derivative}. \nc
...
Note that by \eqref{Lipomega}, we have $|m_{2c}+b_2|\lesssim (\log n)^{-1}$ and $|m_{3c}+b_3|\lesssim (\log n)^{-1}$. Together with  \eqref{a23}, we obtain the following basic estimates 
\be\label{Gsim1}
...
\begin{proof}
 By (\ref{resolvent2}), (\ref{Zi}) and (\ref{Zmu}), we obtain that
\begin{align}
...
\be\label{Gmumu0}
\mathbf 1(\Xi)G_{ii}=\mathbf 1(\Xi)\left[-\frac{1}{z + \sigma_i^2 m_2+m_3} +\OO_\prec\left(n^{-1/2}\right)\right].
\ee
Plugging it into the definitions of $m$ and $m_1$ in \eqref{m123}, we get
\be\label{Gmumu}
\mathbf 1(\Xi)m=\mathbf 1(\Xi)\left[-\frac1n\sum_{i\in \cal I_1}\frac{1}{z + \sigma_i^2 m_2+m_3}  +\OO_\prec\left(n^{-1/2}\right)\right],
\ee
and 
\be\label{Gmumu2}
\mathbf 1(\Xi)m_1=\mathbf 1(\Xi)\left[-\frac1n\sum_{i\in \cal I_1}\frac{\sigma_i^2}{z + \sigma_i^2 m_2+m_3}   +\OO_\prec\left(n^{-1/2}\right)\right],
\ee
...
\be\label{Gii0}
\mathbf 1(\Xi)G_{\mu\mu}=\mathbf 1(\Xi)\left[-\frac{1}{1 + m_1}  +\OO_\prec\left(n^{-1/2}\right)\right], \quad \mu \in \cal I_2,
\ee
and 
\be\label{Gii1}
\mathbf 1(\Xi)G_{\nu\nu}=\mathbf 1(\Xi)\left[-\frac{1}{1 + m}  +\OO_\prec\left(n^{-1/2}\right)\right], \quad \nu \in \cal I_3.
\ee
Taking average over $\mu\in \cal I_2$ and $\nu\in \cal I_3$, we get that with high probability,
\be\nonumber
\begin{split}
& \mathbf 1(\Xi)m_2 =\mathbf 1(\Xi)r_1 \left[-\frac{1}{1 + m_1} +\OO_\prec\left(n^{-1/2}\right)\right],\quad  \mathbf 1(\Xi)m_3 =\mathbf 1(\Xi)r_2\left[-\frac{1}{1 + m}   +\OO_\prec\left(n^{-1/2}\right)\right],
\end{split}
\ee
which further implies
\be\label{Gii}
 \mathbf 1(\Xi)\left(\frac{r_1}{m_2} + 1 + m_1\right) \prec  n^{-1/2} ,\quad \mathbf 1(\Xi)\left(\frac{r_2}{m_3} + 1 + m\right) \prec  n^{-1/2} .
\ee
...
\end{split}
\ee
Since $f$ is strictly increasing and has bounded derivatives on $[0,1-\gamma_n]$, by basic calculus the second equation in \eqref{selfcons_lemm000} gives that $|\omega_2-b_2|\prec n^{-1/2}$. Together with the first equation in \eqref{selfcons_lemm000}, we get $|\omega_3-b_3|\prec n^{-1/2}$. This concludes \eqref{Xiz0}.
\end{proof}
...
As explained above, this concludes \eqref{entry_diagonal}, that is, Lemma \ref{prop_entry}. 
...
\subsection{Proof of Lemma \ref{lem_mbehaviorw} and Lemma \ref{lem_stabw}}\label{sec contract}
...
\begin{split}
&m_{2c}=-(1-\gamma_n) - m_{3c} - z\left( \frac{r_2}{m_{3c}}+1\right),\quad g_z(m_{3c}(z))=r_2, 
\end{split}
\ee
where
$$g_z(m_{3c}):= - m_{3c} +\frac1n\sum_{i=1}^p \frac{m_{3c} }{  z -\sigma_i^2(1-\gamma_n)+ (1 - \sigma_i^2) m_{3c} - \sigma_i^2 z\left(  r_2m_{3c}^{-1}+1\right) }.$$
Hence it suffices to show that there exists a unique solution $m_{3c}(z)$ to the equation $g_z(m_{3c}(z))=r_2$ under the conditions in \eqref{prior1}, and the solution satisfies \eqref{Lipomega}. Now we abbreviate $\e(z):= m_{3 c}(z) - m_{3 c}(0)$, and from \eqref{selfalter} we can obtain that 
\begin{equation} \nonumber
0=\left[g_z(m_{3c}(z)) -  g_0(m_{3c}(0)) -g_z'(m_{3c}(0))\e(z)\right] + g_z'(m_{3c}(0))\e(z),
\ee
which implies
\be\label{selfomega1}
g_z'(m_{3c}(0)) \e(z) =- \left[g_z(m_{3c}(0)) - g_0(m_{3c}(0))\right] - \left[g_z(m_{3c}(z)) -  g_z(m_{3c}(0))-g_z'(m_{3c}(0))\e(z) \right].
 \ee
...
\ee
for all $|z|\le \wt c$ and $|\e_1|  \le \wt c$, $|\e_2|  \le \wt c$. Then with \eqref{dust}, it is easy to see that there exists a sufficiently small constant $\delta>0$ depending only on $\wt C$, such that $h$ is a self-mapping 
$$  h: B_r  \to B_r , \quad B_r:=\{\e \in \C: |\e| \le r \},$$
as long as $r\le \delta$ and $|z| \le c_\delta$ for some constant $c_\delta>0$ depending only on $\wt c, \wt C$ and $\delta$. 
...
\ee
Hence as long as $r$ is chosen to be sufficiently small such that $2rC\le 1/2$, then 
...
$h$ is indeed a contraction mapping on $ B_r$, which proves both the existence and uniqueness of the solution $m_{3c}(z)=m_{3c}(0)+\e$ to the second equation of \eqref{selfalter} if we choose $c_0$ in \eqref{prior1} as $c_0=\min\{c_\delta, r\}$. After obtaining $m_{3c}(z)$, we can then find $m_{2c}$ using the first equation in \eqref{selfalter}. 
...
$$|{ \e}| \le \sum_{k=0}^\infty \|{  \e}^{(k+1)}-{ \e}^{(k)}\|_\infty \le 2\wt C|z|.$$
This gives the bound \eqref{Lipomega} for $m_{3c}(z)$. Then using the first equation in \eqref{selfalter}, we immediately obtain the bound \eqref{Lipomega} for $m_{2c}(z)$ as long as $c_0$ is taken sufficiently small. 
...
\be\label{selfalter2}m_{2c}=-(1-\gamma_n) - m_{3c} - z\left( \frac{r_2}{m_{3c}}+1\right) + \cal E'_2(z),\quad g_z(m_{3c}(z))=r_2+ \cal E'_3(z),\ee
