...
%\end{definition}
For simplicity, we will denote $Y_1:=Z_1 U\Lambda $, $Y_2:=Z_2V$ and $W:=(Y_1^\top, Y_2^\top)$. By Schur complement formula, one can find that (recall \eqref{mainG})
\be \label{green2}
\begin{split}
...
\be\label{defm}
\begin{split} 
m(z) :=\frac1n\sum_{i \in \cal I_1}G_{ii}(z)=\frac1n \tr \cal G(z),\quad & m_1(z):=\frac1n\sum_{i \in \cal I_1}\sigma_i^2 G_{ii}(z),\\
 m_2(z):= \frac{1}{n}\sum_{\mu \in \cal I_2}G_{\mu\mu}(z) = \frac1n \tr \cal G_{22}(z),\quad & m_3(z):= \frac{1}{n}\sum_{\mu \in \cal I_3}G_{\mu\mu}(z) = \frac1n \tr \cal G_{33}(z).
\end{split}
\ee
...
\begin{split}
&\frac{n_1}{n}\frac1{m_{2c}} = - 1 +\frac1n\sum_{i=1}^p \frac{\sigma_i^2}{  z+\sigma_i^2m_{2c} + m_{3c}  } ,\quad \frac{n_2}{n}\frac1{m_{3c}} = - 1 +\frac1n\sum_{i=1}^p \frac{1 }{  z+\sigma_i^2 m_{2c} +  m_{3c}  },
\end{split}
\ee
such that $(m_{2 c}(z), m_{3c}(z))\in \C_+$ for $z\in \C_+$. For simplicity, we  introduce the parameters 
\be\label{ratios}
\gamma\equiv \gamma_n :=\frac{p}{n},\quad r_1\equiv r_1(n) :=\frac{n_1}{n},\quad r_2\equiv r_2(n) :=\frac{n_2}{n}.
\ee
We then define the matrix limit of $G(z)$ as
\be \label{defn_piw}
\Pi(z) := \begin{pmatrix} -(z+\sigma_i^2 m_{2c} +  m_{3c})^{-1} & 0 & 0 \\ 0 & r_1^{-1}m_{2c}(z)I_{n_1} & 0 \\ 0 & 0 & r_2^{-1}m_{3c}(z)I_{n_2}  \end{pmatrix}.\ee
In particular, the matrix limit of $\cal G(z)$ is given by $-(z+\sigma_i^2 m_{2c} +  m_{3c})^{-1}$. 

If $z=0$, then the equations \eqref{selfomega} is reduced to 
\begin{equation}\label{selfomega0}
\begin{split}
&a_2+a_3= 1-\gamma_n,\quad r_1 =  a_2 +\frac1n\sum_{i=1}^p \frac{\sigma_i^2 a_2}{ \sigma_i^2 a_2+ (1-\gamma_n - a_2) } .
\end{split}
\ee
where $a_2:=-m_{2c}(0)$ and $a_3:=-m_{3c}(0)$. Note that the function 
$$f(a_2):=a_2 +\frac1n\sum_{i=1}^p \frac{\sigma_i^2 a_2}{ \sigma_i^2 a_2+ (1-\gamma_n - a_2) } $$
is a strictly increasing function on $[0,1-\gamma_n]$, and $f(0)=0<r_1$, $f(1-\gamma_n)=1>r_1$. Hence there exists a unique solution $(a_2,a_3)$ to \eqref{selfomega0}. Moreover, it is easy to check that $f'(a)=\OO(1)$ for $a\in [0,1-\gamma_n]$. Hence there exists a constant $\tau>0$, such that 
\be\label{a23}
\tau<a_2<1-\gamma_n - \tau,\quad \tau<a_3<1-\gamma_n - \tau .
\ee 
...
There exist constants $c_0, C_0>0$ depending only on $\tau$ in \eqref{assm2}, \eqref{assm3} and \eqref{a23} such that the following statements hold. 
%Fix any constants $c, C>0$. If \eqref{assm20} holds, then we have the following estimates.
If $|z|\le c_0$, then there exists a unique solution to \eqref{selfomega} under the condition
\be\label{prior1}
\max_{\al=2}^3 |m_{\al c}(z) - m_{\al c}(0)| \le c_0.
\ee
...
There exist constants $c_0, C_0>0$ depending only on $\tau$ in \eqref{assm2}, \eqref{assm3} and \eqref{a23} such that the self-consistent equations in \eqref{selfomega} are stable in the following sense. Suppose $|z|\le c_0$ and $m_{\al}: \C_+\mapsto \C_+$, $\al=2,3$, are analytic functions of $z$ such that 
\be\label{prior1}
\max_{\al=2}^3 |m_{\al}(z) - m_{\al c}(0)| \le c_0.
\ee
Suppose they satisfy the system of equations
\begin{equation}\label{selfomegaerror}
\begin{split}
&\frac{r_1}{m_{2}} + 1 -\frac1n\sum_{i=1}^p \frac{\sigma_i^2}{  z+\sigma_i^2m_{2} + m_{3}  } =\cal E_2,\quad  \frac{r_2}{m_{3}} + 1 -\frac1n\sum_{i=1}^p \frac{1 }{  z+\sigma_i^2 m_{2} +  m_{3}  }=\cal E_3,
\end{split}
\ee
for some errors satisfying 
...
Before proving this theorem, we first use it to complete the proof of \ref{lem_cov_shift} and \eqref{derivative}. 

\cor


\begin{proof}[Proof of Lemma \ref{lem_cov_shift} and \eqref{derivative}]
Using \eqref{derivJal} and Cauchy's integral formula, we get
\be\label{apply derivlocal}
\begin{split}
\langle \bu, G(\theta_l)J_\al G(\theta_l)\bv\rangle & = \frac{1}{2\pi \ii}\oint_{\cal C} \frac{\langle \bu, \cal R(w,0)\bv\rangle }{w^2}\dd w \\
&=  \frac{1}{2\pi \ii}\oint_{\cal C} \frac{\langle \bu, \Gamma(w)\bv\rangle }{w^2}\dd w +\OO_\prec(n^{-1/2}) = \langle \bu, \Gamma'(0)\bv\rangle+ \OO_\prec(n^{-1/2}),
\end{split}
\ee
where $\cal C$ is the contour $\{w\in \C: |w| \le (\log n)^{-1} \}$ and we used \eqref{aniso_outstrongw} in the second step. Hence it remains to study the derivatives $\Gamma'(0)$, which can be reduced to calculating the derivatives $\dot m_{\al c}(\theta_l):=\omega'_\al(0)$. 

Using \eqref{selfomega} and implicit differentiation, we obtain that
%We first calculate the partial derivative $\dot m_{\al c} \equiv \partial_{t_1}m^{\mathbf t}_{\al c}(\mathbf t=0) $:
\begin{align*}
&  c_1^{-1}\dot m_{1c}=   m_{3c}^{-2}  + \dot m_{1c} + \frac{z^{-1}}{[1 + (1-z)m_{2c} ]^2} \dot m_{2c},\quad  \dot m_{3c} = m_{3c}^2 \left(c_1^{-1}\dot m_{1c} - m_{3c}^{-2}\right) \\
& c_2^{-1}\dot m_{2c} = \dot m_{2c} + \frac{z^{-1}}{[1 + (1-z)m_{1c}  ]^2} \dot m_{1c} , \quad \dot m_{4c} = c_2^{-1}\dot m_{2c} m_{4c}^2.
\end{align*}
Solving the above equations, we get $\dot m_{\al c} = \gamma_{\al}^{(1)} $, $\al=1,2,3,4,$ for $\gamma_{\al}^{(1)}$ defined in \eqref{defgamma}. Moreover, we can check that $g_1'(0)= h_1(z)$. Hence, we have $ \Gamma'(0)=\Gamma_1$, which, together with \eqref{apply derivlocal}, concludes \eqref{derivG}.
\end{proof}
...
\section{Proof of Theorem \ref{LEM_SMALL}}\label{sec_Gauss}
...
In this case, the problem is reduced to proving the anisotropic local law for $G$ with $U=\id$ and $V=\id$, such that the entries of $Y_1$ and $Y_2$ are independent. This can be handled using the standard resolvent methods as in e.g. \cite{isotropic,PY,yang2019spiked}. To go from the Gaussian case to the general $X$ case, we adopt a continuous self-consistent comparison argument developed in \cite{Anisotropic}. 
...
Suppose Assumption \ref{assm_big1} holds, and $Z_1,Z_2$ satisfy the bounded support condition \eqref{eq_support} with $q= n^{-{1}/{2}}$. Suppose $U$ and $V$ are identity. Then the estimates \eqref{aniso_law} and \eqref{aver_in} hold for $G_0(z)$.
...
\be\label{op rough1} (1-\sqrt{c_1})^2 - \e \le  \lambda_p(Z_1^\top Z_1)   \le  \lambda_1(Z_1^\top Z_1) \le (1+\sqrt{c_1})^2 + \e ,
\ee
and
\be\label{op rough2} (1-\sqrt{c_2})^2 - \e \le  \lambda_q(Z_2^\top Z_2)  \le  \lambda_1(Z_2^\top Z_2) \le (1+\sqrt{c_2})^2 + \e .
\ee
\end{lemma}
\begin{proof}
This lemma essentially follows from \cite[Theorem 2.10]{isotropic}, although the authors considered the case with $\phi_n \prec n^{-1/2}$ only. The results for more general $\phi_n$ follows from \cite[Lemma 3.12]{DY}, but only the bounds for the largest eigenvalues are given there in order to avoid the issue with the smallest eigenvalue when $d_{1,2}$ is close to 1. However, under the assumption \eqref{assm2}, the lower bound for the smallest eigenvalue follows from the exactly the same arguments as in \cite{DY}. Hence we omit the details. 
\end{proof}

With this lemma, we can easily obtain the following a priori estimate on the resolvent $G(z)$ for $z\in S(c_0)$.
...
\end{lemma}
\begin{proof}
By Lemma \ref{SxxSyy} and the assumption \eqref{assm2}, we obtain that 
...
Together with using \eqref{spectral}, it implies the estimates \eqref{priorim} and \eqref{priordiff}.
\end{proof}
...
\be\label{m123}
\begin{split} 
m^{(\mathbb T)} := \frac1n \tr \cal G^{(\mathbb T)}(z),\quad  m^{(\mathbb T)}_1:=\frac1n\sum^{(\mathbb T)}_{i \in \cal I_1}\sigma_i^2 G^{(\mathbb T)}_{ii}(z),\quad m_2^{(\mathbb T)}(z):=  \frac1n \tr \cal G^{(\mathbb T)}_{22}(z),\quad  m_3^{(\mathbb T)}(z):=  \frac1n \tr \cal G^{(\mathbb T)}_{33}(z).
\end{split}
\ee
where we abbreviated that $\sum_{a}^{(\mathbb T)} := \sum_{a\notin \mathbb T} $. For convenience, we will adopt the convention that for any minor $\cal A^{(T)}$ defined as above, $\cal A^{(T)}_{ab} = 0$ if $a \in \mathbb T$ or $b \in \mathbb T$. Moreover, we will abbreviate $(\{a\})\equiv (a)$, $(\{a, b\})\equiv (ab)$.
\end{definition}
...
\subsection{Entrywise local law}\label{sec entry}
...
Then \eqref{aver_in} and the following estimate holds uniformly for $z\in \mathbf D$: %there exist constants 
...
%\end{definition}
\begin{proof}
The proof of Lemma \ref{prop_entry} is divided into the following steps. For simplicity, we will still denote $G\equiv G_0$ in the following proof, while keeping in mind that $W$ takes the form in \eqref{diagW}.
...
\be\label{Zi}
\begin{split}
Z_i = (\mathbb E_{i} - 1) \left( {WG^{\left( i \right)} W^\top} \right)_{ii} &= \sigma_i \sum_{\mu ,\nu\in \mathcal I_2}  G^{(i)}_{\mu\nu} \left(\frac{1}{n}\delta_{\mu\nu} - z_{\mu i}z_{\nu i}\right)\\
...
\be\label{Zmu} 
\begin{split}
Z_\mu &= (\mathbb E_{\mu} - 1) \left( {W^\top  G^{\left( \mu  \right)} W} \right)_{\mu \mu } = \sum_{i,j \in \mathcal I_1} \sqrt{\sigma_i \sigma_j}G^{(\mu)}_{ij} \left(\frac{1}{n} \delta_{ij} - z_{\mu i}z_{\mu j}\right), \quad \mu\in \cal I_2,\\
...
$$\sum_{\nu \in \cal I_2\cup \cal I_3 }  \left| G_{\mu\nu}^{(i)}  \right|^2\le \sum_{\fa \in \cal I }  \left| G_{\mu\fa}^{(i)}  \right|^2 =\left[G^{(i)}(G^{(i)})^* \right]_{\mu\mu} =\OO(1)$$
...
Note that by \eqref{Lipomega}, we have $|m_{2c}+a_2|\lesssim (\log n)^{-1}$ and $|m_{3c}+a_3|\lesssim (\log n)^{-1}$. Together with  \eqref{a23}, we obtain the following basic estimates 
\be\label{Gsim1}
 |m_{2c}| \sim 1, \quad |m_{3c}| \sim 1, \quad |z+\sigma_i^2 m_{2c} +  m_{3c}|\sim 1  ,\quad \left|1 + m_c\right|\sim1, \quad |1+m_{1c}|\sim 1  ,
 \ee
 uniformly in $z\in \mathbf D$, where we abbreviate
 $$m_c(z):=-\sum_{i\in \cal I_1}\frac{1}{z+\sigma_i^2 m_{2c} +m_{3c}},\quad m_{1c}(z):=-\sum_{i\in \cal I_1}\frac{\sigma_i^2}{z+\sigma_i^2 m_{2c} +m_{3c}}.$$
...
\begin{split}
&{\mathbf 1}(\Xi) \left|\frac{r_1}{m_{2}} + 1 -\frac1n\sum_{i=1}^p \frac{\sigma_i^2}{  z+\sigma_i^2m_{2} + m_{3}  } \right|\prec n^{-1/2},\quad  {\mathbf 1}(\Xi)\left|\frac{r_2}{m_{3}} + 1 -\frac1n\sum_{i=1}^p \frac{1 }{  z+\sigma_i^2 m_{2} +  m_{3}  }\right|\prec n^{-1/2}.
\end{split}
\ee
...
[Z]:=\frac{1}{n}\sum_{i\in \mathcal I_1}\frac{Z_i}{\left(z + \sigma_i^2 m_2+m_3\right)^2}, \quad [Z]_1:=\frac{1}{n}\sum_{i\in \mathcal I_1}\frac{\sigma_i^2 Z_i}{\left(z + \sigma_i^2 m_2+m_3\right)^2}, \ \ [Z]_\al:=\frac{1}{n}\sum_{\mu \in \mathcal I_\al}  {Z_\mu} ,\ \ \al=2,3  .
\end{equation}
\nc
\end{lemma}

\begin{proof}
It suffices to prove (\ref{selfcons_improved}), from which (\ref{selfcons_lemm}) follows due to (\ref{Zestimate1}). By (\ref{resolvent2}), (\ref{Zi}) and (\ref{Zmu}), we obtain that
\begin{align}
\frac{1}{{G_{ii} }}&=  - z - \frac{\sigma_i^2}{n} \sum_{\mu\in \mathcal I_2} G^{\left( i \right)}_{\mu\mu}- \frac{1}{n} \sum_{\mu\in \mathcal I_3} G^{\left( i \right)}_{\mu\mu} + Z_i =  - z - \sigma_i^2 m_2 - m_3 + \epsilon_i, \quad i \in \cal I_1, \label{self_Gii}\\
\frac{1}{{G_{\mu\mu} }}&=  - 1 - \frac{1}{n} \sum_{i\in \mathcal I_1}\sigma_i^2 G^{\left(\mu\right)}_{ii}+ Z_{\mu} =  - 1  - m_1 + \epsilon_\mu, \quad \mu \in \cal I_2, \label{self_Gmu1}\\
\frac{1}{{G_{\nu\nu} }}&=  - 1 - \frac{1}{n} \sum_{i\in \mathcal I_1} G^{\left(\nu\right)}_{ii}+ Z_{\nu} =   - 1 - m + \epsilon_\nu, \quad \nu \in \cal I_3, \label{self_Gmu2}
\end{align}
where we recall \eqref{m123}, and 
$$\epsilon_i := Z_i + \sigma_i\left(m_2 - m_2^{(i)}\right) + (m_3-m_3^{(i)}) ,\quad \epsilon_\mu := \begin{cases}Z_{\mu} + (m_1-m_1^{(\mu)}) , \ &\text{if } \mu \in \cal I_2\\ Z_{\mu} +(m-m^{(\mu)}) , \ &\text{if } \mu \in \cal I_3\end{cases}.$$
By (\ref{resolvent8}) we can bound that
\begin{equation}\nonumber
  |m_2 - m_2^{(i)}| \le   \sum_{\mu\in \mathcal I_2}  \left|\frac{G_{\mu i} G_{i\mu}}{G_{ii}}\right|  \prec n^{-1},
\end{equation}
...
With \eqref{Gsim1} and the definition of $\Xi$, we get that $\mathbf 1(\Xi)|z + \sigma_i^2 m_2+m_3|\sim1$. Hence using (\ref{self_Gii}), \eqref{erri} and \eqref{Zestimate1}, we obtain that
\be\label{Gmumu0}
\mathbf 1(\Xi)G_{ii}=\mathbf 1(\Xi)\left[-\frac{1}{z + \sigma_i^2 m_2+m_3} - \frac{Z_i}{\left(z + \sigma_i^2 m_2+m_3\right)^2}  +\OO_\prec\left(n^{-1}\right)\right].
\ee
Plugging it into the definitions of $m$ and $m_1$ in \eqref{m123}, we get
\be\label{Gmumu}
\mathbf 1(\Xi)m=\mathbf 1(\Xi)\left[-\frac1n\sum_{i\in \cal I_1}\frac{1}{z + \sigma_i^2 m_2+m_3} - [Z]  +\OO_\prec\left(n^{-1}\right)\right],
\ee
and 
\be\label{Gmumu2}
\mathbf 1(\Xi)m_1=\mathbf 1(\Xi)\left[-\frac1n\sum_{i\in \cal I_1}\frac{\sigma_i^2}{z + \sigma_i^2 m_2+m_3} - [Z]_1  +\OO_\prec\left(n^{-1}\right)\right],
\ee
...
\be\label{Gsim2}
|1+m_1|\sim 1, \quad |1+m|\sim 1, \quad \text{with high probability on } \Xi.
\ee
...
\be\label{Gii0}
\mathbf 1(\Xi)G_{\mu\mu}=\mathbf 1(\Xi)\left[-\frac{1}{1 + m_1} - \frac{Z_\mu}{\left(1 +  m_1\right)^2}  +\OO_\prec\left(n^{-1}\right)\right], \quad \mu \in \cal I_2,
\ee
and 
\be\label{Gii1}
\mathbf 1(\Xi)G_{\nu\nu}=\mathbf 1(\Xi)\left[-\frac{1}{1 + m} - \frac{Z_\nu}{\left(1 +  m\right)^2}  +\OO_\prec\left(n^{-1}\right)\right], \quad \nu \in \cal I_3.
\ee
Taking average over $\mu\in \cal I_2$ and $\nu\in \cal I_3$, we get that with high probability,
\be\nonumber
\begin{split}
& \mathbf 1(\Xi)m_2 =\mathbf 1(\Xi)r_1 \left[-\frac{1}{1 + m_1} - \frac{[Z]_2}{(1+m_1)^2}  +\OO_\prec\left(n^{-1}\right)\right],\\ 
&\mathbf 1(\Xi)m_3 =\mathbf 1(\Xi)r_2\left[-\frac{1}{1 + m} - \frac{[Z]_3}{(1+m)^2}  +\OO_\prec\left(n^{-1}\right)\right],
\end{split}
\ee
which further implies
\be\label{Gii}
 \mathbf 1(\Xi)\left(\frac{r_1}{m_2} + 1 + m_1\right) \prec \mathbf 1(\Xi)|[Z]_2|  + n^{-1} ,\quad \mathbf 1(\Xi)\left(\frac{r_2}{m_3} + 1 + m\right) \prec \mathbf 1(\Xi)|[Z]_3|  + n^{-1} .
\ee
Finally, plugging \eqref{Gmumu} and \eqref{Gmumu2} into \eqref{Gii}, we conclude \eqref{selfcons_improved}. 
\end{proof}
...
Now applying Lemma \ref{lem_stabw} to \eqref{selfcons_lemm}, and using \eqref{roughh1}-\eqref{roughh2}, we obtain that
\be\label{Xizz}
...
It remains to show that \eqref{Xiz0} holds for some $0\in \mathbf D$. In the following lemma, we pick $0=0$.
\begin{lemma}
Under the assumptions in Proposition \ref{prop_diagonal}, the estimate \eqref{Xiz0} holds.
\end{lemma}
\begin{proof}
By \eqref{spectral}, we get
$$ m(0)=\frac1n\sum_{i\in \cal i_1}G_{ii}(0) = \frac1n\sum_{k = 1}^{p} \frac{|\xi_k(i)|^2 }{\lambda_k} \ge \gamma_n \lambda_1^{-1} \gtrsim 1. $$
Similarly, we can also get that $m_1(0)$ is positive and has size $m_1(0)\sim 1$. Hence we have $1+m_1(0)\sim 1$ and $1+m_1(0)\sim 1$, and by \eqref{self_Gmu1}-\eqref{self_Gmu2} and \eqref{erri}, we get that \eqref{Gii} holds at $z=0$ even without the indicator function $\mathbf 1(\Xi)$. Furthermore, it gives that 
$$  \left|\sigma_i^2 m_2(0)+m_3(0)\right|=\left|\frac{\sigma_i^2}{ 1+m_1(0)} +\frac{1}{1+m(0)}+ \OO_\prec (n^{-1/2})\right| \sim 1$$
with high probability. Then using (\ref{self_Gii}), \eqref{erri} and \eqref{Zestimate1}, we obtain that \eqref{Gmumu} and \eqref{Gmumu2} hold at $z=0$ even without the indicator function $\mathbf 1(\Xi)$. Finally, plugging \eqref{Gmumu} and \eqref{Gmumu2} into \eqref{Gii}, we conclude \eqref{selfcons_lemm} holds at $z=0$, that is,
\begin{equation} \label{selfcons_lemma222}
\begin{split}
& \left|\frac{r_1}{m_{2}(0)} + 1 -\frac1n\sum_{i=1}^p \frac{\sigma_i^2}{ \sigma_i^2m_{2}(0) + m_{3}(0)  } \right|\prec n^{-1/2},\quad \left|\frac{r_2}{m_{3}(0)} + 1 -\frac1n\sum_{i=1}^p \frac{1 }{ \sigma_i^2 m_{2}(0) +  m_{3}(0)  }\right|\prec n^{-1/2}.
\end{split}
\ee

Denoting $\omega_{2}=-m_{2c}(0)$ and $\omega_{3}=-m_{2c}(0)$. By \eqref{Gii}, we have
$$\omega_2= \frac{r_1}{1+m_{1}(0)} +\OO_\prec(n^{-1/2}),\quad \omega_3= \frac{r_2}{1+m(0)}+\OO_\prec(n^{-1/2}).$$ 
Hence there exists a sufficiently small constant $c>0$, such that for any fixed $\e>0$,
$$ c \le  \omega_2 \le r_1+n^{-1/2+\e}, \quad  c\le \omega_3\le r_2+n^{-1/2+\e}, \quad \text{with high probability}.$$
...
\ee
where we also used $1-\gamma_n - r_{1,2}\gtrsim 1$ by \eqref{assm2}. Moreover, one can verify from \eqref{selfcons_lemma222} that $(\omega_2,\omega_3)$ satisfy approximately the same equations as in \eqref{selfomega0}:
\be\label{selfcons_lemm000}
\begin{split}
\omega_2+\omega_3 = 1-\gamma_n + \OO_\prec (n^{-1/2}),\quad f(\omega_2)=r_1 + \OO_\prec (n^{-1/2}).
\end{split}
\ee
Since $f$ is strictly increasing and has bounded derivatives on $[0,1-\gamma_n]$, by basic calculus the second equation in \eqref{selfcons_lemm000} gives that $|\omega_2-a_2|\prec n^{-1/2}$. Together with the first equation in \eqref{selfcons_lemm000}, we get $|\omega_3-a_3|\prec n^{-1/2}$. This concludes \eqref{Xiz0}.
\end{proof}
...
So far, we have proved \eqref{entry_diagonal}. It remains to show that \eqref{aver_in} holds.
...
\subsection{Anisotropic local law}\label{sec_comparison}
...
We denote 
Introduce the notations $Z^0:=Z^{Gauss}$ and $Z^1:=Z$. Let $\rho_{i\mu}^0$ and $\rho_{i\mu}^1$ be the laws of $Z_{i\mu}^0$ and $Z_{i\mu}^1$, respectively. For $\theta\in [0,1]$, we define the interpolated law
...
We shall apply Lemma \ref{lemm_comp_3} with $F(X)=F_{\mathbf v}^p(X,z)$ for $F_{\mathbf v}(X,z)$ defined as
\begin{equation}\label{eq_comp_F(X)}
 F_{\mathbf v}(X,z):=\left|G_{\mathbf{vv}}(X,z)-\Pi_{\mathbf {vv}}(z)\right|.
\end{equation}
For simplicity of notations, we introduce the following generalized entries. For $\mathbf v,\mathbf w \in \mathbb R^{\mathcal I}$ and $\fa\in \mathcal I$, we shall denote
\begin{equation}
G_{\mathbf{vw}}:= \mathbf v^\top G\mathbf w , \quad G_{\mathbf{v}\fa}:= \mathbf v^\top G\mathbf e_{\fa} , %\quad G_{a\mathbf{w}}:=\langle \mathbf e_a,G\mathbf w\rangle,
\end{equation}
where $\mathbf e_{\fa}$ is the standard unit vector along $a$-th axis. Given vectors $\mathbf u\in \mathbb \R^{\mathcal I_{1,2,3}}$, we always identify it with its natural embedding in $\mathbb R^{\mathcal I}$.
%$\left( {\begin{array}{*{20}c}
%   {\mathbf x}  \\
%   0 \\
...
The exact meanings will be clear from the context. The main work is devoted to proving the following self-consistent estimate for the right-hand side of (\ref{basic_interp}).
...
\Delta_{(i\mu)}^{\lambda} :=\lambda \left( {\begin{array}{*{20}c}
   { 0 } & \Sig^{1/2} \mathbf u_i \mathbf v_\mu^\top \tilde \Sig^{1/2}   \\
   { \tilde \Sig^{1/2}\mathbf v_\mu \mathbf u_i^\top \Sig^{1/2} } & {0}  \\
...
To illustrate the idea, we assume the extra condition 
\be\label{3moment}
%\mathbb E x_{ij}^3=0,  
\mathbb E z_{\mu i}^3=0, \quad 1\le \mu \le n,\ \  1\le i \le p.
\ee
We will explain where it is needed in the proof, and explain why it is not necessary at the end of the proof.
...
\subsection{Proof of Lemma \ref{lem_mbehaviorw}}\label{sec contract}
 %We first prove the existence and continuity of the solutions to \eqref{falvww}. 
  For $\al=1,2,$ we abbreviate $m_{\al c}\equiv m_{\al c}(0)$ and $\e_\al (z):= m_{\al c}(z) - m_{\al c}(0)$ with $|\e_\al| \le \wt c$ for some sufficiently small constant $\wt c>0$. By \eqref{selfomega}, we obtain the self-consistent equations for $(\omega_{1c},\omega_{2c})$:
 \begin{equation}\label{selfomega1}
\begin{split}
& \frac{c_1}{\omega_{1c} }=- z +   (1-\theta_l ) \frac{1 + (1-\theta_l )\omega_{2c}}{[1 + (1-\theta_l )\omega_{1c} ][1 + (1-\theta_l )\omega_{2c} ]-\theta_l ^{-1}},\\
&\frac{c_2}{\omega_{2c} }=   (1-\theta_l) \frac{1 + (1-\theta_l )\omega_{1c}}{[1 + (1-\theta_l )\omega_{1c} ][1 + (1-\theta_l )\omega_{2c} ]-\theta_l ^{-1}}.
\end{split}
\ee
 On the other hand,  by \eqref{selfm12} and \eqref{selfm3}, the following self-consistent equations hold for $m_{\al c}(\theta_l)$:
  \begin{equation}\label{selfomega2}
\begin{split}
& \frac{c_1}{m_{1c} (\theta_l)}= (1-\theta_l ) \frac{1 + (1-\theta_l )m_{2c} (\theta_l)}{[1 + (1-\theta_l )m_{1c} (\theta_l)][1 + (1-\theta_l )m_{2c} (\theta_l)]-\theta_l ^{-1}},\\
&\frac{c_2}{m_{2c}(\theta_l) }=   (1-\theta_l) \frac{1 + (1-\theta_l )m_{1c} (\theta_l)}{[1 + (1-\theta_l )m_{1c} (\theta_l) ][1 + (1-\theta_l )m_{2c} (\theta_l)]-\theta_l ^{-1}}.
\end{split}
\ee
Subtract \eqref{selfomega2} from \eqref{selfomega1}, we get that
 \begin{equation}\label{selfomega3}
\begin{split}
& \e_{1} \frac{c_1}{(m_{1c} +\e_1) m_{1c} }= z +   (1-\theta_l )^2 \frac{ g(m_{2c} + \e_2)g( m_{2c}) \e_1+\theta_l^{-1} \e_2}{ [g(m_{1c} + \e_1)g( m_{2c} +\e_2)-\theta_l^{-1} ][g(m_{1c} )g( m_{2c} )-\theta_l^{-1} ]},\\
&  \e_2 \frac{c_2}{(m_{2c}+\e_2) m_{2c}}= (1-\theta_l )^2 \frac{ g(m_{1c} + \e_1)g( m_{1c}) \e_2+\theta_l^{-1} \e_1}{ [g(m_{1c} + \e_1)g( m_{2c} +\e_2)-\theta_l^{-1} ][g(m_{1c} )g( m_{2c} )-\theta_l^{-1} ]}.
\end{split}
\ee
where we abbreviate  $g(x):=1 + (1-\theta_l ) x $. Inspired by the above equations, we define iteratively a sequence of vectors ${\e}^{(k)}=(\e_1^{(k)},\e_2^{(k)})\in \C^2$ such that ${\e}^{(0)}=\mathbf 0\in \C^2$, and 
 \begin{equation}\label{selfomega4}
\begin{split}
& \left\{\frac{c_1}{m_{1c}^2  } -  \frac{ (1-\theta_l )^2 g(m_{2c})^2 }{ [g(m_{1c})g( m_{2c} )-\theta_l^{-1} ]^2}\right\} \e^{(k+1)}_{1}  -  \frac{ (1-\theta_l )^2 \theta_l^{-1}}{ [g(m_{1c})g( m_{2c} )-\theta_l^{-1} ]^2} \e_2^{(k+1)}= z + \frac{c_1 [\e_1^{(k)}]^2}{m_{1c}^2 (m_{1c}+\e_1^{(k)})} \\
&+  \frac{ (1-\theta_l )^2}{g(m_{1c} )g( m_{2c})-\theta_l^{-1} } \left\{\frac{ g(m_{2c} + \e_2^{(k)})g( m_{2c}) \e_1^{(k)}+\theta_l^{-1} \e_2^{(k)}}{ g(m_{1c} + \e_1^{(k)})g( m_{2c} +\e_2^{(k)})-\theta_l^{-1} }  - \frac{ g(m_{2c})^2 \e_1^{(k)}+\theta_l^{-1} \e_2^{(k)}}{ g(m_{1c} )g( m_{2c})-\theta_l^{-1} } \right\},\\
& \left\{\frac{c_2}{m_{2c}^2  } -  \frac{ (1-\theta_l )^2 g(m_{1c})^2 }{ [g(m_{1c})g( m_{2c} )-\theta_l^{-1} ]^2}\right\} \e^{(k+1)}_{2}  -  \frac{ (1-\theta_l )^2 \theta_l^{-1}}{ [g(m_{1c})g( m_{2c} )-\theta_l^{-1} ]^2} \e_1^{(k+1)}= \frac{c_2 [\e_2^{(k)}]^2}{m_{2c}^2 (m_{2c}+\e_2^{(k)})} \\
&+  \frac{ (1-\theta_l )^2}{g(m_{1c} )g( m_{2c})-\theta_l^{-1} } \left\{\frac{ g(m_{1c} + \e_1^{(k)})g( m_{1c}) \e_2^{(k)}+\theta_l^{-1} \e_1^{(k)}}{ g(m_{1c} + \e_1^{(k)})g( m_{2c} +\e_2^{(k)})-\theta_l^{-1} }  - \frac{ g(m_{1c})^2 \e_2^{(k)}+\theta_l^{-1} \e_1^{(k)}}{ g(m_{1c} )g( m_{2c})-\theta_l^{-1} } \right\}.
\end{split}
\ee
 In other words, \eqref{selfomega4} defines a mapping $\mathbf f:l^\infty (2)\to l^\infty (2)$,
\be\label{iteration} 
{\e}^{(k+1)}= \mathbf f({{} \e}^{(k)}), \quad \mathbf f(\mathbf x):=S^{-1}\begin{pmatrix} z\\ 0 \end{pmatrix}+ S^{-1} \mathbf e(\mathbf x),
\ee
where 
$$
...
$$
and
$$\mathbf e(\mathbf x):= \begin{pmatrix} \frac{c_1 x_1^2}{m_{1c}^2 (m_{1c}+x_1)} -  \frac{ \theta_l(1-\theta_l )^2}{1-t_l } \left\{\frac{ g(m_{2c} + x_2)g( m_{2c}) x_1 +\theta_l^{-1} x_2}{ g(m_{1c} +x_1)g( m_{2c} +x_2)-\theta_l^{-1} }  - \frac{ g(m_{2c})^2 x_1+\theta_l^{-1} x_2}{ g(m_{1c} )g( m_{2c})-\theta_l^{-1} } \right\} \\ \frac{c_2 x_2^2}{m_{2c}^2 (m_{2c}+x_2)} -  \frac{ \theta_l (1-\theta_l )^2}{1-t_l } \left\{\frac{ g(m_{1c} + x_1)g( m_{1c}) x_2+\theta_l^{-1} x_1}{ g(m_{1c} + x_1)g( m_{2c} +x_2)-\theta_l^{-1} }  - \frac{ g(m_{1c})^2 x_2 +\theta_l^{-1} x_1}{ g(m_{1c} )g( m_{2c})-\theta_l^{-1} } \right\} \end{pmatrix}.$$
Here we have used $\theta_lg(m_{1c})g( m_{2c} ) = f_c(\theta_l) = t_l$ (recall \eqref{fcz}) to simplify the expressions a little bit.

With direct calculation, one can show that under \eqref{tlc}, there exist constants $\wt c, \wt C>0$ depending only on $c_1, c_2$ and $\delta_l$ such that
\be\label{dust}
\|S^{-1}\|_{l^\infty\to l^\infty} \le \wt C, \quad \text{and} \quad \|\mathbf r(\mathbf x)\|_\infty \le \wt C\|\mathbf x\|_\infty^2 \ \ \text{for} \ \ \|\bx\|_\infty \le \wt c.
\ee
With \eqref{dust} and \eqref{iteration}, it is easy to see that there exists a sufficiently small constant $\tau>0$ depending only on $\wt C$, such that $\mathbf f$ is a self-mapping 
$$\mathbf f: B_r \left(l^\infty(\Z_2)\right)\to B_r \left(l^\infty(\Z_2)\right), \quad B_r\left(l^\infty(\Z_2)\right):=\{\bx\in l^\infty(\Z_2): \|\bx\|_\infty \le r \},$$
as long as $r\le \tau$ and $|z| \le c_\tau$ for some constant $c_\tau>0$ depending only on $c_1, c_2,\delta_l$ and $\tau$. 
%\be\label{priori_cond}
%\zeta+\| \b g\|_\infty+ |z-\wt z|\le c_r
%\ee
%for some constant $ c_r>0$ depending on $r$. 
Now it suffices to prove that $h$ restricted to $B_r \left(l^\infty(\Z_N)\right)$ is a contraction, which then implies that ${{} \e}:=\lim_{k\to\infty} {{{} \e}}^{(k)}$ exists and is a unique solution to \eqref{selfomega3} subject to the condition $\|{{} \e}\|_\infty \le r$. 


From the iteration relation \eqref{iteration}, using \eqref{dust} we obtain that
\be\label{k1k}
{{} \e}^{(k+1)} - {{} \e}^{(k)}= S^{-1}\left[\mathbf e({{} \e}^{(k)}) -\mathbf e({{} \e}^{(k-1)})\right] \le \wt C \left\|\mathbf e({{} \e}^{(k)}) -\mathbf e({{} \e}^{(k-1)})\right\|_\infty .
\ee
From the expression of $\mathbf e$, we see that as long as $r$ is chosen to be sufficiently small compared to $\theta_l^{-1}-g(m_{1c})g( m_{2c} )= (1-t_l)\theta_l^{-1}$, we have
...
$$
for some constant $C >0$ depending only on $c_1, c_2$ and $\delta_l$. Thus we can choose a sufficiently small constant $0<r \le \min\{\tau, (2C)^{-1}\}$ such that $Cr \le 1/2$, i.e. $\mathbf f$ is indeed a contraction mapping on $ B_r \left(l^\infty(\Z_2)\right)$, which proves both the existence and uniqueness of the solution to \eqref{selfomega3} if we choose $c_0$ in \eqref{prior1} as $c_0=\min\{c_\tau, r\}$. After obtaining $\omega_{1c}$ and $\omega_{2c}$ with ${{} \e}$, we can then define $\omega_{3c}$ and $\omega_{4c}$.

Note that with \eqref{dust} and ${{} \e}^{(0)}=\mathbf 0$, we get from \eqref{iteration} that
 $$ \|{{} \e}^{(1)}\|_\infty\le \wt C|z| .$$
With the contraction mapping, we have the bound 
$$\|{{} \e}\|_\infty \le \sum_{k=0}^\infty \|{{} \e}^{(k+1)}-{{} \e}^{(k)}\|_\infty \le 2\wt C|z|.$$
This gives the bound \eqref{Lipomega} for $\omega_{1,2c}$. Then using \eqref{selfomega}, we immediately obtain the bound \eqref{Lipomega} for $\omega_{3,4c}$ as long as $c_0$ is taken sufficiently small. 

Finally, for \eqref{Immcw}, we take imaginary parts of the self-consistent equations in \eqref{selfomega1}:  
 \begin{equation} \nonumber
\begin{split}
& \frac{c_1}{|\omega_{1c} |^2} \im\omega_{1c} = \eta +   (1-\theta_l )^2 \frac{1}{|g(\omega_{1c})g(\omega_{2c})-\theta_l ^{-1} |^2} \left( |g(\omega_{2c})|^2\im \omega_{1c} + \theta_l^{-1} \im \omega_{2c}\right),\\
&\frac{c_2}{|\omega_{2c}|^2 } \im\omega_{2c}= (1-\theta_l )^2 \frac{1}{|g(\omega_{1c})g(\omega_{2c})-\theta_l ^{-1} |^2} \left( |g(\omega_{1c})|^2\im \omega_{2c} + \theta_l^{-1} \im \omega_{1c}\right).
\end{split}
\ee
One can then solve the above equations for $\im \omega_{1c}$ and $\im \omega_{2c}$ in terms of $ \omega_{1c}$ and $ \omega_{2c}$ and show that \eqref{Immcw} holds for $\im \omega_{1,2 c}(z) $. In fact, if we replace $ (\omega_{1c}, \omega_{2c})$ with $(m_{1c},m_{2c})$ in the coefficients, one can show with direct calculation that \eqref{Immcw} holds. Then one can use a simple perturbation argument using the bound \eqref{Lipomega}. Finally, with \eqref{selfomega}, we immediately obtain that \eqref{Immcw} holds for $\im \omega_{3,4 c}(z) $. 
%We now prove \eqref{dozy}. We have proved above that both $(M_\zeta^{\b g})_i(z, \wt z)$ and $(M_{\zeta'}^{\b g'})_i(z', \wt z\,')$ exist and satisfy \eqref{bony}. Denote by $m':=m_{\rm sc}(\wt z\,'+ {\rm i} 0^+\, )$ and $x'_i:=(M_{\zeta'}^{\b g'})_i(z', \wt z\,')-m' $. By \eqref{m'}, we have 
%\be\label{m-m'}
%|m'-m|={\rm O}(|\wt z - \wt z\,'|).
%\ee 
%Then using \eqref{mGYY} we can obtain that  
%\begin{align*}
% \|  \bx'-\bx \|_\infty \le & C\|(1-m^2S_0)^{-1}\|_{L^\infty\to L^\infty}\cdot  \left\{ |\wt z-\wt z\,'|\cdot \left[ \|\bx'\|_\infty +  \|\b g'\|_\infty+ |z'-\wt z\,'| + \|\bx'\|_\infty^2 + \zeta' (1+\|\bx'\|_\infty)\right] \right.\\
%& + \left. \left[  \|\b g-\b g'\|_\infty+ |z-z'| + |\wt z-\wt z\,'|+ |\zeta-\zeta'|(1+\|\bx'\|_\infty)+ \left(\zeta+\|\bx \|_\infty +\| \bx' \|_\infty  \right)\cdot \|\bx' - \bx \|_\infty\right]\right\} \\
%\le & C\left(\zeta+\|\bx \|_\infty +\| \bx' \|_\infty  \right)\cdot \|\bx' - \bx \|_\infty + C \left(  \|\b g-\b g'\|_\infty+ |z-z'| + |\wt z-\wt z\,'|+ |\zeta-\zeta'|\right).
%\end{align*}
%Applying \eqref{bony} to both $(M_\zeta^{\b g})_i(z, \wt z)$ and $(M_{\zeta'}^{\b g'})_i(z', \wt z\,')$,
%we see that for small enough $c$, 
%$$ \|  \bx'-\bx \|_\infty \le C \left(  \|\b g-\b g'\|_\infty+ |z-z'| + |\wt z-\wt z\,'|+ |\zeta-\zeta'|\right).$$
%Together with \eqref{m-m'}, we obtain \eqref{dozy} as desired. 
%\end{proof}

\begin{proof}[Proof of Lemma \ref{lem_stabw}]
As in the proof of Lemma \ref{lem_mbehaviorw}, we subtract the equations \eqref{selfomegaerror} from \eqref{selfomega}, and consider the contraction principle for the functions $\e_\al (z):= \omega_{\al }(z) - m_{\al c}(z)$. We omit the details.
\end{proof}