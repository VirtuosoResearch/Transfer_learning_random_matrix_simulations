...
%z\sum_{k = 1}^{n} \frac{\zeta_k(\mu) \zeta_k^*(\nu)}{\lambda_k-z}=
...
\begin{split}
&\frac{n_1}{n}\frac1{a_2} = - 1 +\frac1n\sum_{i=1}^p \frac{\sigma_i^2}{ \sigma_i^2 a_2+ a_3  } ,\quad \frac{n_2}{n}\frac1{a_3} = - 1 +\frac1n\sum_{i=1}^p \frac{1 }{\sigma_i^2 a_2+ a_3 },
\end{split}
\ee
where $a_2:=m_{2c}(0)$ and $a_3:=m_{3c}(0)$.

The existence and uniqueness of the solution $(m_{2 c}(z),m_{3c}(z))$ is given by the following lemma. Moreover, we will also include some basic estimates on it. {\cor(say something about the previous work)}

\cob
\begin{lemma} \label{lem_mbehaviorw}
There exist constants $c_0, C_0>0$ depending only on $c_1, c_2$ and $\delta_l$ in \eqref{tlc} such that the following statements hold. 
...
\be\label{prior1}
\max_{\al=1}^4 |\omega_{\al c}(z) - m_{\al c}(\theta_l)| \le c_0.
\ee
Moreover, the solution satisfies
\be\label{Lipomega}
\max_{\al=1}^4 |\omega_{\al c}(z) - m_{\al c}(\theta_l)| \le C_0 |z|,
\ee
and
\be\label{Immcw}
0\le \im \omega_{\al c}(z) \le C_0\eta ,\quad z= E+ \ii\eta, \ \ \al=1,2,3,4.
\ee
...
\end{lemma}
 ...
 The proof is a standard application of the contraction principle. We abbreviate $m_{\al c}\equiv m_{\al c}(\theta_l)$ and $\e_\al (z):= \omega_{\al c}(z) - m_{\al c}(\theta_l)$ with $|\e_\al| \le \wt c$ for some sufficiently small constant $\wt c>0$. By \eqref{selfomega}, we obtain the self-consistent equations for $(\omega_{1c},\omega_{2c})$:
...
As a byproduct of the contraction mapping argument in the above proof, we also obtain the following stability result that will be useful for our proof of Theorem \ref{thm_localw} below. 


\begin{lemma} \label{lem_stabw}
There exist constants $c_0, C_0>0$ depending only on $c_1, c_2$ and $\delta_l$ in \eqref{tlc} such that the self-consistent equations in \eqref{selfomega} are stable in the following sense. Suppose $|z|\le c_0$ and $\omega_{\al}: \C_+\mapsto \C_+$, $\al=1,2,3,4$, are analytic functions of $z$ such that 
\be\label{prior1}
\max_{\al=1}^4 |\omega_{\al}(z) - m_{\al c}(\theta_l)| \le c_0.
\ee
Suppose they satisfy the system of equations
\begin{equation}\label{selfomegaerror}
\begin{split}
& \frac{c_1}{\omega_{1} }+ z +  \omega_{3}= \mathcal E_1,\quad \omega_{3} - (\theta_l -1) \frac{1 + (1-\theta_l )\omega_{2}}{[1 + (1-\theta_l )\omega_{1} ][1 + (1-\theta_l )\omega_{2} ]-\theta_l ^{-1}}= \mathcal E_2,\\
&\frac{c_2}{\omega_{2} } + \omega_{4} = \mathcal E_3, \quad  \omega_{4} -(\theta_l -1) \frac{1 + (1-\theta_l )\omega_{1}}{[1 + (1-\theta_l )\omega_{1} ][1 + (1-\theta_l )\omega_{2} ]-\theta_l ^{-1}}= \mathcal E_4,
\end{split}
\ee
for some errors satisfying 
$$ \max_{\al=1}^4 |\mathcal E_\al| \le \delta(z),$$
where $\delta(z)$ is any deterministic $z$-dependent function $\delta(z) \le (\log n)^{-1}.$ Then we have 
 \begin{equation}
\max_{\al=1}^4 \left|\omega_\al(z)-\omega_{\al c}(z)\right|\le C_0\delta(z).\label{Stability1}
\end{equation}
\end{lemma}
\begin{proof}
As in the proof of Lemma \ref{lem_mbehaviorw}, we subtract the equations \eqref{selfomegaerror} from \eqref{selfomega}, and consider the contraction principle for the functions $\e_\al (z):= \omega_{\al }(z) - \omega_{\al c}(z)$. We omit the details.
\end{proof}
\nc
...
For any constants $c_0>0$ and $a \le 1$, we define a domain of the spectral parameter $z$ as
\begin{equation}
S(c_0):= \left\{z=E+ \ii \eta \in \C_+: |z|\le c_0 \right\}. \label{SSET1}
\end{equation}
