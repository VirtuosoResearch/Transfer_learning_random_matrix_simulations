...
\section{Proof of Lemma \ref{X1X2}} %\label{sec_maintools}
...
\subsection{Main tools}\label{sec_tools}
...
\section{Proof of of Theorem \ref{main_thm}}\label{sec_cutoff}

In this section, we prove Theorem \ref{main_thm} with the results in Section \ref{sec_tools}. 
%We begin by proving the necessity part.
%\vspace{5pt}
%\begin{proof}[Proof of the sufficiency]
...
%Theorem satisfies the conditions ${\bf A_1, A_2^{\prime}}$ with $q=N^{-\phi}, \ \phi>0$ and ${\bf A_3}$.
%\noindent {\bf Proof of Proposition 2.2:}
%\begin{proof}[Proof of Proposition 2.2]
%By Lemma 5.2 of \cite{LY}, for any $C>0$, if $C \leq \vert A \vert$ and $B \geq A^2+1$, then there exists a random variable $x$ such that
%\begin{equation*}
%\mathbb{E}(x)=0, \ \mathbb{E}(x^2)=1, \ \mathbb{E}(x^3)=C, \ \mathbb{E}(x^4)=B
%\end{equation*}
%with the following support
%\begin{equation*}
%\text{supp}(x) \subset [-DB,DB]
%\end{equation*}
%where $D$ depends only on $C$.  Now we choose $C=B_2, \ A=\mathcal{O}(1), \ B=\mathcal{O}(B_2 \log N), D=B_2(=C)$, then there exists a random variable $\tilde{q}$, s.t
%\begin{equation*}
%\mathbb{E}\tilde{q}=0, \ \mathbb{E}\vert \tilde{q} \vert^2=1, \ \mathbb{E}(\tilde{q})^3=B_2, \ \mathbb{E}(\tilde{q})^4=\mathcal{O}(B_2 \log N)
%\end{equation*}
%Define a matrix $\tilde{X}$ with i.i.d entries $\tilde{x}_{ij}=\frac{1}{\sqrt{N}} \tilde{q}_{ij}$, where $\tilde{q}_{ij} \approx \tilde{q}$. Hence, there exists a matrix $\tilde{X}$, the first four moments match with $X$ and the support becomes
%$$[-B_2 \log N, B_2 \log N]$$
%This implies $q=\mathcal{O}(\frac{\log N}{\sqrt{N}})$.  \end{proof}
%\noindent Therefore, we have constructed matrices $X_1,  \tilde{X_1}$ from $X$ satisfying conditions ${\bf A_1, \ A_2^{\prime}}$ and ${\bf A_3} $, where $X_1$ has a large support while $\tilde{X_1}$ has a small support.  We will leave the proof of Proposition 2.3 to section 4.  The rest is to finish the proof  {\bf step 5}, which is (\ref{BOUNDSBSBSBSBS}).  We firstly provide two facts. Define the matrix $C$ with i.i.d entries $C_{ij}=c_{ij}$.  \\
%By (\ref{DEF1}), (\ref{DEF2}) and the fact that $X^c_{ij}$ is Bernoulli, 
It is easy to check that for independent $X^s$, $X^l$ and $X^c$,
\begin{equation}
X_{ij} \stackrel{d}{=} X^s_{ij}\left(1-X^c_{ij}\right)+X^l_{ij}X^c_{ij} - \frac{1}{\sqrt{N}}\frac{\beta_N}{1-\alpha_N}. \label{T3}
\end{equation}
%where $X_2= (\mathbb{E}\vert q^l_{ij} \vert^2)^{-\frac{1}{2}}X^l$ and $X_{ij}^l=\frac{1}{\sqrt{N}} q_{ij}^l$.
%where by (\ref{BBBOUNDS}), we have
%\begin{equation*}
%\left\vert \frac{1}{\sqrt{N}}\frac{\beta_N}{1-\alpha_N} \right\vert \leq 2\delta N^{-2+3\epsilon}.
%\end{equation*}
If we define the $n\times N$ matrix $Y=(Y_{ij})$ by 
$$Y_{ij}=\frac{1}{\sqrt{N}}\frac{\beta_N}{1-\alpha_N}=\OO(\delta N^{-2+3\epsilon}), \quad 1\le i \le n,\ \ 1\le  j \le N,$$
then we have $ \| Y \| =\OO (N^{-1+3\epsilon}) $. % depending on $\delta$ and $d$. 
%Using the bound (\ref{boundH}), 
...
%it is easy to see that with $\xi_1$-high probability%from the representation (\ref{linearize_block}), it is easy to see that
...
%Together with the estimate for $\mathbb{E}\vert q^s_{11}\vert^2$ in (\ref{estimate_qs}), 
Then by Theorem \ref{lem_comparison}, there exist constants $\epsilon,\delta>0$ such that for any $s\in \mathbb R$,
\be\label{univ_small}
\begin{split}
\mathbb{P}^G \left( N^{{2}/{3}}(\lambda_1-\lambda_{r}) 
 \leq  s-N^{-\epsilon}\right)-N^{-\delta} \leq
\mathbb{P}^s\left(N^{{2}/{3}}(\lambda_1-\lambda_{r})\leq s \right) & \\
\leq \mathbb{P}^G(N^{{2}/{3}}\left(\lambda_1-\lambda_{r}) \leq s+N^{-\epsilon}\right)+N^{-\delta} &,
\end{split}
\ee
...
We first introduce a cutoff on matrix $X^c$ as $\tilde X^c : =\mathbf{1}_{A}X^c$, where
\begin{align*}
...
% and hence
%\begin{equation}
%\mathbb P(X^c = \tilde X^c) \ge 1 - \mathcal \OO(N^{-1+10\epsilon}).
%\end{equation}
%By independence, we can put the matrix $C_{ij}$ into a vector  $C^1 \in \mathbb{R}^{MN}$, then we have
%\begin{equation*}
%\mathbb{P}(\{\text{Rank(C)} \geq N^{5 \epsilon}\}) \leq \mathbb{P}(\sum_{i=1}^{MN} C^1_i \geq N^{5 \epsilon}) \leq \exp(-N^2 I(N^{5\epsilon-2}))
%\end{equation*}
%where $I(x)=x\log x +(1-x)\log (1-x)+ \log 2$ and it is easy to check that, when $N$ is sufficiently large, $I(N^{5\epsilon-2})>0$.
%By definition, we have
%\begin{equation*}
%\mathbb{P}(A) \geq 1- \mathbb{P}(\{\text{Rank(C)} \geq N^{5 \epsilon}\})
%\end{equation*}
...
\end{equation}
for any fixed constant $\omega>0$. Hence if we introduce the matrix 
...
\end{equation}
%where 
%$$\lambda_1^s:=\lambda_1\left(\tilde {\mathcal Q}_1(X^s) \right),\quad  \lambda_1^E:=\lambda_1\left(\tilde {\mathcal Q}_1(E) \right).$$ 
The estimate (\ref{eq_claim}), combined with (\ref{const_err}), (\ref{univ_small}) and (\ref{EneR}), concludes (\ref{SUFFICIENT}).

Now we prove (\ref{eq_claim}). Since $ \tilde{X}^c $ is independent of $X^s$, the positions of the nonzero elements of $E$ are independent of $X^s$. Without loss of generality, we assume the $m$ nonzero entries of $E$ are
\begin{equation}
...
P: &= \left( {\begin{array}{*{20}c}
  ...
  & = \left( {\begin{array}{*{20}c}
   ...
   \end{array}} \right),
   \end{align*}
%Furthermore, we have the eigendecomposition 
%$$\left( {\begin{array}{*{20}c}
%   { 0 } & E  \\
%   { E^*} & {0}  \\
%   \end{array}} \right) = W P_DW^*,$$ 
where $P_D$ is a $2m \times 2m$ diagonal matrix
...
\end{equation}
%\begin{equation}
%\det\left(W^* \left( {\begin{array}{*{20}c}
%   {U\Sig^{1/2} } & 0  \\
%   { 0} & {V\tilde \Sig^{1/2}}  \\
%   \end{array}} \right) G^s (\mu)\left( {\begin{array}{*{20}c}
%   {\Sig^{1/2} U^{*}} & 0  \\
%   { 0} & {\tilde \Sig^{1/2}V^*}  \\
%   \end{array}} \right)W + (\gamma P_D)^{-1}\right)=0,
%\end{equation}
where
...
\end{array}} \right)\right)^{-1}, \quad O:=\left( {\begin{array}{*{20}c}
   ...
   { 0} & {\tilde \Sig^{1/2}V^*}  \\
   \end{array}} \right)W.$$ 
Define $R^\gamma:=O^* G^s O + (\gamma P_D)^{-1}$ for $0<\gamma < 1$. Now let $\mu: = \lambda_1^s \pm N^{-3/4}.$ We claim that 
...
%It has the following $2\times 2$ blocks (recall the definition (\ref{Aij_group})): for $1\le i \le m $,
%\begin{align}\label{Q_group}
%\left( {\begin{array}{*{20}c}
%   {R^\gamma_{i,j}} & {R^\gamma_{i,j+m}}  \\
%   {R^\gamma_{i+m,j}} & {R^\gamma_{i+m,j+m}}  \\
%   \end{array}} \right) & =  \frac{1}{2} \left( {\begin{array}{*{20}c}
%   {1} & {1}  \\
%   {1} & {-1}  \\
%   \end{array}} \right) G_{[ij]} \left( {\begin{array}{*{20}c}
%   {1} & {1}  \\
%   {1} & {-1}  \\
%   \end{array}} \right) \nonumber \\
%   & +  \delta_{ij}\left( {\begin{array}{*{20}c}
%   {(\gamma e_{ii})^{-1}} & {0}  \\
%   {0} & {-(\gamma e_{ii})^{-1}}  \\
%   \end{array}} \right) \ \ .
%\end{align}
...
Finally, we prove (\ref{suff_claim}). Note that $\eta_0=\OO(b_N/N)=\OO(N^{-1+\e})$, hence $z=\lambda_r+\ii N^{-{2}/{3}}$ is in $\tilde S(c_0, C_0,\delta)$ for any small constant $\delta>0$. Now we write
\be\label{Rgamma}
R^\gamma(\mu)=O^* \left( G^s(\mu) - G^s(z)\right) O+ O^* \left( G^s(z) - \Pi(z)\right) O + O^*  \Pi(z) O + (\gamma P_D)^{-1} . 
\ee
With \eqref{psi12}, %\eqref{Piii} and \eqref{assm3}, 
we have 
\be\label{Pi part}
\|O^*  \Pi(z) O\| =\OO(1)
\ee
%for some constant $C>0$ depending only on $\tau$. 
...
\be\label{Gz part}
\left\|O^* \left( G^s(z) - \Pi(z)\right) O\right\| = \OO(mN^{-1/6})= \OO(1) \quad \text{with probability $1- \OO(mN^{-1/3})$}.
\ee
% Then by (\ref{DIAGONAL}) and (\ref{SQUAREROOTBEHAVIOR}), we have with high probability,
%\begin{equation}
%\max_{a} \vert G_{aa}^s (z) - \Pi_{aa}(\lambda_r) \vert \leq N^{-{\epsilon}/{2}}, \label{BOUND1}
%\end{equation}
%where we also used the Assumption \ref{assm_big2} and 
%$$\left|m_{2c}(z) - m_{2c}(\lambda_r)\right| \sim |z-\lambda_r|^{1/2},$$
%which follows from (\ref{SQUAREROOT}). For the off-diagonal terms, we use (\ref{weak_off}), (\ref{SQUAREROOTBEHAVIOR}) and the Markov inequality to conclude that
%\begin{equation}\label{BOUND2}
%\max_{a \ne b \in \{1,\ldots, m\}\cup\{M+1, \ldots, M+m\} } |G_{ab}^s(z)| \le N^{-1/6}
%\end{equation}
%holds with probability $1- \OO(N^{-1/6})$.
%\begin{equation}
%\max_{ i \neq j} \vert G_{ij}^1(z) \vert \leq  N^{-\frac{1}{6}+\epsilon} \label{BOUND2}
%\end{equation}
%where $G^{1}(z)$ is the Green function of $X_1^{*}X_1$.
...
\begin{equation}\label{repulsion_estimate}
\mathbb P\left( \min_{i}|\lambda_i(\tilde Q_1(X^s) ) - \mu| \ge N^{-3/4} \right) = 1-\oo(1).
\end{equation}
On the other hand, the rigidity result (\ref{rigidity}) gives that %with high probability,
\begin{equation}\label{rigid_estimate}
|\mu - \lambda_r | \prec N^{-2/3} + N^{-3/4}.
\end{equation}
Using (\ref{delocal}), (\ref{repulsion_estimate}), (\ref{rigid_estimate}) and the rigidity estimate (\ref{rigidity}), we can get that for any set $\Omega$ of deterministic unit vectors of cardinality $N^{\OO(1)}$, %and any small constant $c>0$,
\begin{equation}
...
\end{align*}
where in the first step we used (\ref{spectral1}), in the second step (\ref{delocal}) and $|\lambda_k - z||\lambda_k - \mu| \gtrsim 1$ for $\gamma_k \le \lambda_r-c_1$ due to (\ref{rigidity}), in the third step the Cauchy-Schwarz inequality, in the fourth step (\ref{repulsion_estimate}), and in the last step $|\lambda_k - z||\lambda_k - \mu| \sim (k/N)^{-4/3}$ for $k>N^\e$ by the rigidity estimate (\ref{rigidity}). %For all the other choices of $\mathbf u$ and $\mathbf v$, we can prove the estimate (\ref{REALCOMPLEX}) in a similar way. 
For the other choices of deterministic unit vectors $\mathbf u ,\mathbf v \in \mathbb C^{\mathcal I_{1,2}}$, we can prove \eqref{REALCOMPLEX} in a similar way. Now with (\ref{REALCOMPLEX}), we can get that %a similar bound as in \eqref{Gz part}:
\be\label{Gmu part}
\left\|O^* \left( G^s(\mu) - G^s(z)\right) O\right\| = \OO(mN^{-1/4+3\e}) \quad \text{with probability $1- \oo(1)$}.
\ee
...
% see that (\ref{BOUND1}) and (\ref{BOUND2}) still hold if we replace $z$ by $\mu=\lambda_1^s \pm N^{-{3}/{4}}$ and double the right hand sides. Then using $\max_{i}|e_{ii}| \le \tau^{-1}\omega$ and (\ref{Q_group}), we get that for any $0<\gamma\le 1$,
%\begin{align*}
%& \min_{1\le i \le m, \gamma} \{|R^\gamma_{ii}|, |R^\gamma_{i+m,i+m}|\} \ge \tau {\omega}^{-1} - \frac{1}{2}\left| \Pi_{ii}(\lambda_r)+ m_{2c}(\lambda_r)\right| -  \OO(N^{-\epsilon/2}), \\
%& \max_{1\le i \le m, \gamma} \{|R^\gamma_{i,i+m}|, |R^\gamma_{i+m,i}|\} \le  \frac{1}{2}\left| \Pi_{ii}(\lambda_r) - m_{2c}(\lambda_r)\right| +  \OO(N^{-\epsilon/2}),
%\end{align*}
%and
%$$\max_{1 \le i \ne j \le m, \gamma} \left( |R^\gamma_{i,j}|+|R^\gamma_{i+m,j}|+|R^\gamma_{i,j+m}|+ |R^\gamma_{i+m,j+m}|\right) = \OO(N^{-1/6}),$$
%hold with probability $1-\OO(1)$. Thus $R^{\gamma}$ is diagonally dominant with probability $1-\OO(1)$ (provided that $\omega$ is chosen to be sufficiently small). This proves the claim (\ref{suff_claim}), which further gives (\ref{eq_claim}) and completes the proof.
%\end{proof}
