...
\begin{split}
&a_2+a_3= 1-\gamma_n,\quad r_2 =  a_2 +\frac1n\sum_{i=1}^p \frac{\sigma_i^2 a_2}{ \sigma_i^2 a_2+ (1-\gamma_n - a_2) } .
\end{split}
\ee
...
\begin{split}
&\frac{r_1}{m_{2c}} + 1 -\frac1n\sum_{i=1}^p \frac{\sigma_i^2}{  z+\sigma_i^2m_{2c} + m_{3c}  } =\cal E_2,\quad  \frac{r_2}{m_{3c}} + 1 -\frac1n\sum_{i=1}^p \frac{1 }{  z+\sigma_i^2 m_{2c} +  m_{3c}  }=\cal E_3,
\end{split}
\ee
...
\begin{equation}
 \vert \frac{1}{p}\sum_{i\in \cal I_1}\left[G_{ii}(z)-\Pi_{ii}(z)\right] \vert \prec n^{-1}. \label{aver_in} %+ q^2 
\end{equation}
%where $m$ is defined in \eqref{defn_m}. Moreover, outside of the spectrum we have the following stronger estimate
%\begin{equation}\label{aver_out1}
% | m(z)-m_{c}(z)|\prec q^2  + \frac{1}{N(\kappa +\eta)} + \frac{1}{(N\eta)^2\sqrt{\kappa +\eta}},
...
with diagonal spatial and temporal covariance matrices, which can be handled using the standard resolvent methods as in e.g. \cite{isotropic,PY}. To go from the Gaussian case to the general $X$ case, we adopt a continuous self-consistent comparison argument developed in \cite{Anisotropic}. 
...
As discussed above, we first prove Theorem \ref{LEM_SMALL} for the case $U=\id$ and $V=\id$, which will imply the local laws in the Gaussian $Z_1$ and $Z_2$ case. Thus we first deal with the following resolvent:
...
In this subsection, we collect some a priori estimates and resolvent identities that will be used in the proof. The estimates in this section work for general $G$, that is, we do not require $U$ and $V$ to be identity.
...
$$  \lambda_p \ge \lambda_p(Z_2^\top Z_2) \ge \e >0$$ 
...
\begin{split}
& G_{i\mu } =  -G_{ii}  \left( WG^{\left( {i} \right)} \right)_{i\mu}. %\left( { - Y_{i\mu }  +  {\left( {YG^{\left( {i\mu } \right)} Y} \right)_{i\mu } } } \right) . %, \ \  G_{\mu i}  = G_{\mu \mu } G_{ii}^{\left( \mu  \right)} \left( { - Y_{\mu i}^\top  + \left( {Y^\top  G^{\left( {\mu i} \right)} Y^\top  } \right)_{\mu i} } \right).
\end{split}
\end{equation}
...
\begin{equation}  \label{eqn_randomerror}
\begin{split}
 \Lambda _o : &=  \max_{i \ne j \in \mathcal I_1\cup \cal I_2} \left| {\cal R_{ij}  } \right| +  \max_{\mu\ne \nu \in \cal I_3} \|\pi^{-1}\cal R_{[\mu\nu]} \|+ \max_{i\in \cal I_1\cup \cal I_2,\mu\in \cal I_3}\left( \left\| \cal R_{i,[\mu]}  \right\| + \left\| \cal R_{[\mu],i} \right\| \right).
% \Lambda: &= \Lambda_o + \max_{i\in \cal I_1 \cup \cal I_2}|G_{ii} - \Pi_{ii}| +  \max_{\mu \in \cal I_3}   \left\|\pi^{-1}\left(G_{[\mu\mu]}  - \pi\right)\pi^{-1}\right\|   ,
\end{split}
\end{equation}
...
\cob
Then we prove the off-diagonal estimates. For $i\ne j \in \mathcal I_1\cup \cal I_2$, using \eqref{resolvent3}, Lemma \ref{largedeviation} and \eqref{priorim}, we obtain that \begin{equation}
 |\cal R_{ij}| \prec \frac{1}{\sqrt{n}} \left( \sum_{\mu \in \cal I_3\cup \cal I_4}  {\left| \cal R_{\mu j}^{(i)}  \right|^2 }  \right)^{1/2} \prec n^{-1/2}. 
 \ee
For $\mu \ne \nu \in \mathcal I_3$, using \eqref{eq_res21}, Lemma \ref{largedeviation} and \eqref{priorim}, we obtain that 
$$ \left\| \cal R_{[\mu\nu]} \right\| \prec    \frac{1}{n} \left( \sum_{i \in \cal I_{1}\cup \cal I_2}  {\left| \cal R_{i\nu}^{[\mu]}  \right|^2 }  \right)^{1/2}+\frac{1}{n} \left(  \sum_{i \in \cal I_{1}\cup \cal I_2}  {\left| \cal R_{i\overline \nu}^{[\mu]}  \right|^2 }   \right)^{1/2} \prec n^{-1/2}.$$
For $i\in \cal I_1\cup \cal I_2$ and $\mu \in \mathcal I_3$, using \eqref{eq_res22}, Lemma \ref{largedeviation} and \eqref{priorim}, we obtain that  
$$ \left\| \cal R_{[\mu],i} \right\| \prec   \frac{1}{n} \left( \sum_{j \in \cal I_{1}\cup \cal I_2 }  {\left| \cal R^{[\mu]}_{ji}  \right|^2 }  \right)^{1/2} \prec  \phi_n + \left(  \frac{\im m_{1}^{(i\mu\overline \mu)} +\im m_{2}^{(i\mu\overline \mu)} }{n\eta}     \right)^{1/2} \prec n^{-1/2}.$$
Thus we conclude \eqref{Zestimate1}.
\nc
\end{proof}
\nc
\begin{proof}
 Applying Lemma \ref{largedeviation} to $Z_{i}$ in (\ref{Zi}), we get that on $\Xi$,
...
%Using (\ref{resolvent3}), Lemma \ref{largedeviation} and the bound $\max_{a,b}|G_{ab}|=\OO(1)$ on $\Xi$, 
Similarly, using (\ref{resolvent3}) and Lemmas \ref{Ward_id}-\ref{largedeviation}, we can prove that %with $\xi$-high probability,
...
the bound $\max_{a,b}|G_{ab}|= \OO(1)$ on $\Xi$, Lemma \ref{Ward_id} and Lemma \ref{largedeviation}, we get that %with $\xi$-high probability,
