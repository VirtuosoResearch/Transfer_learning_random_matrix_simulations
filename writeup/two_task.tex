...
\begin{theorem}[Transfer rate under model shift]\label{thm_model_shift}
	...
	\begin{itemize}
		\item {\bf Positive transfer:} when ${\Delta_{\vari}} \ge \bigbrace{\sqrt{n_1} + \sqrt{{p}}}^2 {\Delta_{\beta}} $, we have that $\te(\hat{\beta}_{s,t}) \le \te(\hat{\beta}_t)$.
...
\begin{align*}
		& \Delta_\beta\left( 1-\sqrt{\frac{p}{n_1}}\right)^4 \le ~ {\bignorm{\Sigma_2^{1/2} (X_1^{\top}X_1 + X_2^{\top}X_2)^{-1} X_1^{\top}X_1 (\beta_s - \beta_t)}}^2 \le ~ \Delta_\beta\left( 1+\sqrt{\frac{p}{n_1}}\right)^4.
		\end{align*}	
	}
...
\[ \Delta_{\var} \le \bigbrace{\sqrt{{n_1}} - \sqrt{p}}^4 \Delta_{\beta}. \]
...
\begin{proposition}\label{prop_model_shift}
	...
	\begin{align*}
		\bigbrace{{n_1^2}{} - {4\sqrt{n_1^{3}p}} - {2n_1 p}} \Delta
		\le \bignorm{\Sigma_2^{1/2} (X_1^{\top}X_1 + X_2^{\top}X_2)^{-1}X_1^{\top}X_1(\beta_s - \beta_t)}^2
		\le \bigbrace{\sqrt{{n_1}{}} + \sqrt{p}}^4 \Delta.
	\end{align*}
\end{proposition}
