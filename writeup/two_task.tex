...
\subsection{Deterministic Conditions for the Case of Two Tasks}
...
\begin{theorem}\label{thm_model_shift}
	...
	\end{align*}
	where $\delta = \norm{Z} \cdot \norm{\Sigma_1^{1/2}(\beta_s - \hat{w}\beta_t)}^2$ with $Z = {M} \frac{(1 + a_3)\id + a_4 {M}^{\top}{M}}{(a_2 + a_1 {M}^{\top}{M})^2} {M}^{\top}$, and
	\begin{align*}
		\Delta_{\vari} &\define \sigma^2 \bigbrace{\frac{1}{n_2 - p}\bigtr{\Sigma_2^{-1}} - \frac{1}{n_1 + n_2}\bigtr{(a_1 M^{\top}M + a_2\id)^{-1}}} \\
		\Delta_{\beta} &\define \frac{n_1^2}{(n_1 + n_2)^2}(\beta_s - \beta_t)^{\top} \Sigma_1^{1/2} Z \Sigma_1^{1/2} (\beta_s - \beta_t),
	\end{align*}
