...
\begin{theorem}[Transfer rate under model shift]\label{thm_model_shift}
	...
	Let
	\[ \Delta_{\beta} \define (\beta_s - \beta_t)^{\top} \Sigma_1^{1/2} M \frac{(1 + a_3)\id + a_4 M^{\top}M}{(a_2 + a_1 M^{\top}M)^2} M^{\top} \Sigma_1^{1/2} (\beta_s - \beta_t), \]
...
\begin{example}[\textbf{Increasing the distance of task models}]
	We consider a simple setting where $\Sigma_1 = \id$.
	Suppose that $\beta_s - \beta_t$ is a vector with mean zero and variance $d^2$. Hence the task models have distance $d^2$ in expectation.
	In this case, we can simplify $\Delta_{\beta}$ as follows
	\[ d^2 \cdot \sum_{i=1}^p \frac{(1 + a_3)\lambda_i^2 + a_4 \lambda_i^4}{(a_1 \lambda_i^2 + a_2)^2}. \]
\end{example}


\begin{example}[\textbf{Increasing the source task data size}]
\end{example}
