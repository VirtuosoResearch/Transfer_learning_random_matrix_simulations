...
\begin{align}
	\Delta_{\vari} \define \sigma^2 p \cdot \bigbrace{\frac{p}{n_2 - p} - \frac{1}{n_1 + n_2} \bigtr{\bigbrace{a_1 M^{\top}M + a_2\id}^{-1}}}
\end{align}
It remains to consider increment from the first term in $\te(\hat{\beta}_{s,t})$, which is bounded using the following lemma.
%We are now interested in the following quantity

\begin{theorem}[Transfer rate under model shift]\label{thm_model_shift}
	Let $a_1, a_2$ be the solutions from equations \eqref{eq_a1} and \eqref{eq_a2}.
	Denote by $M \define \Sigma_1^{1/2}\Sigma_2^{-1/2}$.
	Let
	\[ \Delta_{\beta} \define (\beta_s - \beta_t)^{\top} \Sigma_1^{1/2} M \frac{(1 + a_3)\id + a_4 M^{\top}M}{(a_1 + a_2 M^{\top}M)^2} M^{\top} \Sigma_1^{1/2} (\beta_s - \beta_t), \]
	...
	\end{align}
	We have the following conditions that guarantee when we get positive or negative transfer.
	\begin{itemize}
		\item {\bf Positive transfer:} when ${\Delta_{\vari}} \ge \bigbrace{\frac{n_1}p + 2\sqrt{\frac{n_1}{p}} + 1}^2 {\Delta_{\beta}} $, we have that $\te(\hat{\beta}_{s,t}) \le \te(\hat{\beta}_t)$.
		\item {\bf Negative transfer:} when $\Delta_{\vari} \le \bigbrace{\frac{n_1^2}{p^2} - {\frac{4n_1^{1.5}}{p^{1.5}}} - \frac{2n_1}{p}} {\Delta_{\beta}} $, we have that $\te(\hat{\beta}_{s,t}) \ge \te(\hat{\beta}_t)$.
	\end{itemize}
\end{theorem}
...
\[ \Delta_{\var} \le \bigbrace{\sqrt{\frac{n_1}p} - 1}^4 \Delta_{\beta}. \]
We describe examples based on Theorem \ref{thm_model_shift}.


\begin{example}[\textbf{Phase transition as we increase the distance of task models}]
\end{example}


\begin{example}[\textbf{Phase transition as we increase the source task data size}]
\end{example}
...
\begin{proposition}\label{prop_model_shift}
	...
	\begin{align*}
		\bigbrace{\frac{n_1^2}{p^2} - {\frac{4n_1^{1.5}}{p^{1.5}}} - \frac{2n_1}{p}} \Delta
		\le \bignorm{\Sigma_2^{1/2} (X_1^{\top}X_1 + X_2^{\top}X_2)^{-1}X_1^{\top}X_1(\beta_s - \beta_t)}^2
		\le \bigbrace{\sqrt{\frac{n_1}{p}} + 1}^4 \Delta.
	\end{align*}
\end{proposition}
...
\begin{proof}
	...
	Denote by $\cE = Z_1^{\top}Z_1 - \frac{n_1}p\id$.
	Let $K = (X_1^{\top}X_1 + X_2^{\top}X_1)^{-1}$.
	We have
	\begin{align}
		\bignorm{\Sigma_2^{1/2}(X_1^{\top}X_1 + X_2^{\top}X_2)^{-1}X_1^{\top}X_1(\beta_s - \beta_t)}^2
		= &\frac{n_1^2}{p^2}\Delta + \frac{2n_1}p(\beta_s - \beta_t)^{\top}\Sigma_1^{1/2}\cE\Sigma_1^{1/2}K \Sigma_2 K \Sigma_1 (\beta_s - \beta_t) \\
		&+ \bignorm{\Sigma_2^{1/2} K \Sigma_1^{1/2}\cE \Sigma_1^{1/2}(\beta_s - \beta_t)}^2 \label{eq_lem_model_shift_1}\\
		\le& \bigbrace{\frac{n_1^2}{p^2} + \frac{2n_1}p(1 + 2\sqrt{\frac{n_1}p}) + (1 + 2\sqrt{\frac{n_1}p})^2} \Delta. \nonumber
	\end{align}
...
\paragraph{Part II: The limit of $\Delta_{\beta}$ using random matrix theory.}
...
\begin{align}
-\frac{n_1}{p}\left(2\sqrt{\frac{p}{n_1}} - {\frac{p}{n_1}}\right)  \le {Z_1^T Z_1 -  \frac{n_1}{p} \id}  \le \frac{n_1}{p}\left(2\sqrt{\frac{p}{n_1}} + {\frac{p}{n_1}}\right) . \label{eq_isometric}
\end{align}
}
