...
\subsection{Tight Bounds for the Case of Two Tasks}
...
\begin{theorem}\label{thm_model_shift}
	...
	Let $\hat{w}$ denote the optimal solution for the ratio $W_1/W_2$ in equation \eqref{eq_te_mtl}.
	...
	The information transfer is solely determined by two deterministic quantities $\Delta_{\beta}$ and $\Delta_{\vari}$, which show the change of model shift bias and variace, respectively. With high probability we have
	\begin{align*}
	 	\te(\hat{\beta}_{t}^{\MTL}) \le \te(\hat{\beta}_t^{\STL}) \text{ when: } &\Delta_{\vari} - \Delta_{\beta} \ge \left(2\sqrt{\frac{p} {n_1}} + \frac{p}{n_1}\right) \delta \\
		\te(\hat{\beta}_t^{\MTL}) \ge \te(\hat{\beta}_t^{\STL}) \text{ when: } &\Delta_{\vari} - \Delta_{\beta} \le -\bigbrace{2\sqrt{\frac {p}{n_1}} + \frac{p}{n_1}} \delta,
	\end{align*}
	...
	\begin{align*}
		\Delta_{\vari} &\define {\frac{\sigma^2 }{n_2 - p}\bigtr{\Sigma_2^{-1}} - \frac{\sigma^2}{n_1 + n_2}\bigtr{(a_1 M^{\top}M + a_2\id)^{-1}}} \\
...
\subsection{The Effect of Covariate Shift on the Rate of Transfer}
