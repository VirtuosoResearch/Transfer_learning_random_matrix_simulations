...
\begin{lemma}\label{lem_model_shift}
	...
	\begin{gather}
		 ...
		 a_1 +\frac1{n_1 + n_2}\sum_{i=1}^p \frac{a_1}{a_1 + \lambda_i^2[(1-\gamma_n)-a_1]} = \frac{n_1}{n_1 + n_2}. \label{m35reduced_2}
	\end{gather}
\end{lemma}
...
We consider the same setting as in previous subsection. Assume that $p/n_1$ is a small number such that $Z_1^TZ_1$ is roughly an isometry, that is, under \eqref{eq_12moment}, 
$$ \left\| Z_1^T Z_1 -  \frac{n_1}{n} I \right\| \le 2\sqrt{\frac{p}{n}} + {\frac{p}{n}} .$$ 
In the previous subsection, we have seen that ({\color{red}may need to assume that $\Sigma_1$ and $\Sigma_2$ are in the same eigenspace})
