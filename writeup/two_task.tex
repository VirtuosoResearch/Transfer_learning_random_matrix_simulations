...
\subsection{Positive versus Negative Transfer with Multi-Task Training}
...
\subsection{The Effect of Covariate Shift on Transfer Rate}



\subsubsection{Warm Up: When There is No Model Shift}
...
\begin{theorem}[Transfer rate without model shift]\label{thm_cov_shift}
	When $\beta_s = \beta_t$, we have that
	\todo{figure out the constants} $\hat{w} = 1 \pm \bigo{\frac 1 {n_1+n_2}}$.
	And we have
	\begin{align*}
		%\err(\hat{\beta}^{\TL}_{s,t}) &= \sigma^2 \cdot \bigtr{\frac 1 {(n_1 + n_2)a_1\Sigma_1 + (n_1 + n_2)a_2\Sigma_2}} \\
		\te(\hat{\beta}^{\TL}_{t}) &= \sigma^2 \cdot \bigtr{\frac 1 {(n_1 + n_2)a_1\Sigma_2^{-1/2}\Sigma_1\Sigma_2^{-1/2} + (n_1 + n_2)a_2\id}} + \bigo{\frac 1 {(n_1 + n_2)^2}},
	\end{align*}
...
\smallskip
\begin{example}[\textbf{When $\Sigma_1$ and $\Sigma_2$ have perpendicular eigenspaces}]
...
\subsubsection{The General Case: Gains over Single-Task Training}


\subsubsection{Gains over Multi-Task Training}
