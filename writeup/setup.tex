...
\textbf{Problem setup.}
Suppose we have $t$ training datasets, where $t$ is a fixed value that does not grow with the feature dimension $p$.
...
We focus on the commonly used hard parameter sharing model for multi-task learning \cite{R17,MTDNN19}.
...
Concretely, considering the following objective
\begin{align}
	\label{eq_mtl}
...
In general, the above objective $f(\cdot)$ is non-convex in $B$ and the $W_k$'s.
