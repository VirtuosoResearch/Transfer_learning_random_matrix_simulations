...
\begin{align*}
	\te(w_2\hat{B}(w)) = w^2 \bignorm{\Sigma^{1/2}(w^2 X_1^{\top}X_1 + X_2^{\top}X_2)^{-1} X_1^{\top}X_1 (\beta_s - w \beta_t)}^2 + \sigma^2 \cdot \bigtr{(w^2 X_1^{\top}X_1 + X_2^{\top}X_2)^{-1} \Sigma_2}.
\end{align*}
...
This setting captures a wide range of applications of multi-task learning where we would like to use auxillary task data to help train tasks with limited labeled data.
