...
\squishlist
	...
	\item Fine-tuning on the target task: we learn the output layer $W_t\in\real^{t-1}$ for the target task similar to equation \eqref{eq_mtl}
		\begin{align}
			g(W_t) = \bignorm{X_t B W_t - Y_t}^2.
		\end{align}
\squishend
After solving $W_t$, we use $\hat{\beta}_t^{\TL} = B W_t$ as the transfer learning estimator for the target task.
