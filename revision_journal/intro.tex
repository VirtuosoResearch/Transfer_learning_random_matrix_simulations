...
On the other hand, the bias limit is much harder to study, and its exact dependence on the sample ratio and the covariate shift matrix is still an open problem in random matrix theory in the most general setting. However, when the data is Gaussian distributed and there is no covariante shift, we can use recent results in \cite{BES_free1,BES_free2} to derive the bias limit. To the best of our knowledge, we are not aware of any previous result in the high-dimensional setting, even for two sample covariance matrices with isotropic population covariances and the same sample size. 
