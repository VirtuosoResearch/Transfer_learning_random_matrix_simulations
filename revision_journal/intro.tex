...
These previous works mainly consider cases where there is only one high-dimensional sample covariance matrix involved. ......... In the single task case, if the data is Gaussian distributed, then precise variance can be derived immediately from properties of Wishart distribution. In particular, in the high dimensional setting, the eigenvalues of Wishart matrices satisfy the famous Marchenko–Pastur (MP) law \cite{MP}. It well-known that the MP law holds universally regardless of the detailed data distribution \cite{bai2010spectral}. 
...
\cite{Anisotropic} later extended this result to sample covariance matrices with arbitrary covariance.
...
There exists another method to derive the asymptotic variance limit using free addition in free probability theory \cite{nica2006lectures}. We do not take this procedure because this method is not completely justified in the general non-Gaussian setting (in the sense that it is hard to establish rigorously the free independence between two sample covariance matrices with non-diagonal covariate shift). Furthermore, our techniques provide almost sharp convergence rates to the asymptotic limit, which cannot be given by free probability theory. To the best of our knowledge, we are not aware of any previous results in the multi-task case, even for two tasks (with arbitrary covariate shifts).

On the other hand, the bias limit is much harder to derive. However, when the data is Gaussian distributed and there is no covariante shift, we can use recent results in ... to derive the bias limit. 
