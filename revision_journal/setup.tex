...
Consider the transfer learning setting with two data sets. We denote their sample sizes by $n_1$ and $n_2$.
For $i=1,2,$ let $X^{(i)} \in \real^{n_i \times p}$ denote dataset $i$'s feature covariates. We assume that the label vector $Y^{(i)} \in \real^{n_i}$ for $X^{(i)}$ follows a linear model plus random noise:
$$Y^{(i)}= X^{(i)}\beta^{(i)} + \varepsilon^{(i)},\quad i=1,2.$$
Throughout the paper, we make the following assumptions on the feature covariates $X^{(i)}$ and the noise vectors $\varepsilon^{(i)}$, following the work of \citet{hastie2019surprises} (see also \citet{wu2020optimal,richards2021asymptotics} and the references therein). %We remark that all these assumptions are natural in high-dimensional statistics.
