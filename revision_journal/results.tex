...
\begin{example}[Does covariate shift help or hurt information transfer?]
    ...
    \begin{claim}\label{claim_dichotomy}
        ...
        Within the set of all possible covariate shift matrix $M$ described above, the following dichotomy holds:
        \begin{enumerate}
...
\begin{proof}
    ...
    where we abbreviate $\gamma:=p/(n_1+n_2)$. Then using $\alpha_1 + \alpha_2 = 1-\gamma$, through a direct calculation we find that %we can calculate that
    \begin{align*}
...
%	Let $g(M)$ denote the prediction loss of $\hat{\beta}_t^{\MTL}$ when $M = \Sigma_1^{1/2}\Sigma_2^{-1/2} \in\cS_{\mu}$.
	...
	\begin{align} g(\id_{p\times p}) \le \bigbrace{1+ {\frac{n_2}{\tau^2\cdot n_1}  }} g(M), \text{ for any } M \in \cS. \label{eq_claim_id}
	\end{align}
\end{claim}
\begin{proof}
    ...
    \begin{align*}
        ...
        &\ge \sum_{i=1}^p \frac{1} {\alpha_1 \lambda_i^2 + \alpha_2 \cdot {\lambda_i^2} / {\tau^2}} \tag{since $\lambda_i \le \tau$, for any $i$} \\
