\section{Precise asymptotics under distribution shift}\label{sec_main}

We show precise estimates of the bias and variance of HPS and almost sharp convergence rates.
In Section \ref{sec3_cov}, we characterize the variance of HPS under covariate shift, which extends the classical result from Lemma \ref{fact_tr} to the sum of two sample covariance matrices with covariate shift.
Based on the characterization, We provide a detailed analysis of the impact of covariate shift on the excess risk of HPS.
...
Finally, Section \ref{sec3_combined} extends these results to settings under covariate and model shifts.
...
Our first example illustrates that the effect of covariate shift depends on the sample sizes of each dataset in an intricate manner.
    ...
    \begin{proposition}[Sample sizes vs. covariate shift]\label{claim_dichotomy}
...
\begin{proposition}[Identical covariance provides approximately optimal transfer under imbalanced dataset sizes]\label{prop_covariate}
...
\begin{proposition}[Sample sizes vs. model shift]\label{claim_model_shift}
...
\subsection{Covariate and model shifts}\label{sec3_combined}
\todo{write down the formal statement}
...
\begin{figure}[!t]
	...
	\end{subfigure}
	\caption{We further show that our observations in Figures \ref{fig_sec31} and \ref{fig_sec3_model_shift} extend to settings under covariate and model shifts.
...
\begin{theorem}[Bias estimates under covariate and model shifts]\label{prop_main_RMT}
...
\end{theorem}
\todo{consider changing $a_3, a_4$ to $\alpha_3, \alpha_4$ because of conflict with $a$}
